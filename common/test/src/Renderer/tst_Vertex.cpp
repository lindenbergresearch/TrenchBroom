...
REQUIRE(std::memcmp(&expected, &actual, sizeof(expected))
== 0);
}
...
expected.
emplace_back(TestVertex{pos, uv, color}
);
actual.
emplace_back(pos, uv, color
);
}
...
REQUIRE(std::memcmp(expected.data(), actual.data(), sizeof(TestVertex) * 3)
== 0);
}} // namespace Renderer
} // namespace TrenchBroom
