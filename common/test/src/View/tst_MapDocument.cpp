...
Model::PatchNode *MapDocumentTest::createPatchNode(const std::string &textureName) const {
    ...
    return new Model::PatchNode{
        Model::BezierPatch{
            3, 3, {{0, 0, 0}, {1, 0, 1}, {2, 0, 0}, {0, 1, 1}, {1, 1, 2}, {2, 1, 1}, {0, 2, 0}, {1, 2, 1}, {2, 2, 0}}, textureName
        }};
    // clang-format on
}
...
{
    auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/valveFormatMapWithoutFormatTag.map", "Quake",
...
{
    auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/standardFormatMapWithoutFormatTag.map", "Quake",
        Model::MapFormat::Unknown
    );
...
{
    auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/emptyMapWithoutFormatTag.map", "Quake",
        Model::MapFormat::Unknown
    );
...
{
    auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/reloadTextureCollectionsQ2.map", "Quake2",
    ...
    REQUIRE(kdl::vec_transform(faces, [](const auto *face) { return face->attributes().textureName(); }) == std::vector<std::string>{
...
TEST_CASE_METHOD(MapDocumentTest, "Brush Node Selection")
{
    ...
    document->addNodes({{document->world()->defaultLayer(), {brushNodeInDefaultLayer, brushEntityNode, pointEntityNode, outerGroupNode}},
                        {document->world(),                 {customLayerNode}}}
    );

    document->addNodes({{customLayerNode, {brushNodeInCustomLayer}},
    ...
    SECTION("hasAnySelectedBrushNodes")
    {
        ...
        // clang-format off
        const auto [pathsToSelect, expectedResult] = GENERATE_COPY(values<T>({{std::vector<Model::NodePath>{},     false},
...
TEST_CASE_METHOD(MapDocumentTest, "selectByLineNumber")
{
    ...
    */
    ...
    const auto map = std::map<const Model::Node *, std::string>{{brush,             "brush"},
    ...
    document->addNodes({{document->world()->defaultLayer(), {brush, pointEntity, patch, brushEntity, outerGroup}},});

    document->addNodes({{brushEntity, {brushInEntity1,    brushInEntity2}},
    ...
    {
        const auto [lineNumbers, expectedNodeNames] = GENERATE(values<T>({{{0},                {}},
                                                                          {{4},                {"brush"}},
                                                                          {{5},                {"brush"}},
                                                                          {{4, 5},             {"brush"}},
                                                                          {{6},                {}},
    ...
    SECTION("outer group is open")
    {
        ...
        const auto [lineNumbers, expectedNodeNames] = GENERATE(values<T>({{{31}, {}},
...
TEST_CASE_METHOD(MapDocumentTest, "canUpdateLinkedGroups")
{
    ...
    REQUIRE_THAT(entityNodes, Catch::UnorderedEquals(std::vector<Model::EntityNodeBase *>{
            entityNode, linkedEntityNode
        }
    ));
...
TEST_CASE_METHOD(MapDocumentTest, "createPointEntity")
{
    ...
    SECTION("Default entity properties")
    {
        ...
        document->loadDocument(Model::MapFormat::Standard, document->worldBounds(), game, "").transform_error([](auto e) {
              throw std::runtime_error{e.msg};
            }
        );
        ...
        CHECK_THAT(entityNode->entity().properties(), Catch::Matchers::UnorderedEquals(
            std::vector<Model::EntityProperty>{{Model::EntityPropertyKeys::Classname, "some_name"},
...
TEST_CASE_METHOD(MapDocumentTest, "resetDefaultProperties")
{
    ...
    REQUIRE(entityNodeWithPropAWithValueChanged->entity().definition() == definitionWithDefaults);
    ...
    REQUIRE_THAT(entityNodeWithoutDefinition->entity().properties(), Catch::Matchers::UnorderedEquals(
        std::vector<Model::EntityProperty>{{"classname", "some_class"},
                                           {"some_prop", "some_value"},
        }
    ));
    REQUIRE_THAT(entityNodeWithProp->entity().properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"},
                                                                                                                                {"some_prop", "some_value"},
        }
    ));
    REQUIRE_THAT(entityNodeWithPropA->entity().properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname",      "some_name"},
                                                                                                                                 {"some_prop",      "some_value"},
                                                                                                                                 {"default_prop_a", "default_value_a"},
        }
    ));
    REQUIRE_THAT(entityNodeWithPropAWithValueChanged->entity().properties(), Catch::Matchers::UnorderedEquals(
        std::vector<Model::EntityProperty>{{"classname",      "some_name"},
                                           {"default_prop_a", "some_other_value"},
        }
    ));
    REQUIRE_THAT(entityNodeWithPropsAB->entity().properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname",      "some_name"},
                                                                                                                                   {"some_prop",      "some_value"},
                                                                                                                                   {"default_prop_a", "default_value_a"},
    ...
    SECTION("Set Existing Default Properties")
    {
        ...
        CHECK_THAT(entityNodeWithoutDefinition->entity().properties(), Catch::Matchers::UnorderedEquals(
            std::vector<Model::EntityProperty>{{"classname", "some_class"},
                                               {"some_prop", "some_value"},
            }
        ));
        CHECK_THAT(entityNodeWithProp->entity().properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"},
                                                                                                                                  {"some_prop", "some_value"},
            }
        ));
        CHECK_THAT(entityNodeWithPropA->entity().properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname",      "some_name"},
                                                                                                                                   {"some_prop",      "some_value"},
                                                                                                                                   {"default_prop_a", "default_value_a"},
            }
        ));
        CHECK_THAT(entityNodeWithPropAWithValueChanged->entity().properties(), Catch::Matchers::UnorderedEquals(
            std::vector<Model::EntityProperty>{{"classname",      "some_name"},
                                               {"default_prop_a", "default_value_a"},
            }
        ));
        CHECK_THAT(entityNodeWithPropsAB->entity().properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname",      "some_name"},
                                                                                                                                     {"some_prop",      "some_value"},
                                                                                                                                     {"default_prop_a", "default_value_a"},
