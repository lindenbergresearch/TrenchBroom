...
void MapDocumentTest::SetUp() {
  ...
  document->newDocument(m_mapFormat, vm::bbox3(8192.0), game).transform_error(
      [](auto e) {
...
{
auto [document, game, gameConfig] = View::loadMapDocument(
    "fixture/test/View/MapDocumentTest/standardFormatMapWithoutFormatTag.map", "Quake", Model::MapFormat::Unknown
);
CHECK(document
...
{
auto [document, game, gameConfig] = View::loadMapDocument(
    "fixture/test/View/MapDocumentTest/emptyMapWithoutFormatTag.map", "Quake", Model::MapFormat::Unknown
);
...
CHECK_THROWS_AS(View::loadMapDocument("fixture/test/View/MapDocumentTest/mixedFormats.map", "Quake", Model::MapFormat::Unknown), IO::WorldReaderException
);
}
...
{
auto [document, game, gameConfig] = View::loadMapDocument(
    "fixture/test/View/MapDocumentTest/reloadTextureCollectionsQ2.map", "Quake2", Model::MapFormat::Quake2
);
...
== 4);
REQUIRE(kdl::vec_transform(faces, [](const auto *face) { return face->attributes().textureName(); })
== std::vector<std::string>{
"b_pv_v1a1", "e1m1/b_pv_v1a2", "e1m1/f1/b_rc_v4", "lavatest"
});

REQUIRE(kdl::none_of(faces, [](const auto *face) { return face->texture() == nullptr; })
);
...
// clang-format off
const auto paths = GENERATE_COPY(
    values<T>(
        {{}, {getPath(brushNodeInDefaultLayer)}, {getPath(brushNodeInDefaultLayer), getPath(brushNodeInCustomLayer)},
         {getPath(brushNodeInDefaultLayer), getPath(brushNodeInCustomLayer), getPath(brushNodeInEntity)}, {getPath(brushNodeInGroup)},
...
// clang-format off
const auto [pathsToSelect, expectedResult] = GENERATE_COPY(
    values<T>(
        {{std::vector<Model::NodePath>{}, false}, {{getPath(pointEntityNode)}, false}, {{getPath(brushEntityNode)}, true}, {{getPath(outerGroupNode)}, true},
         {{getPath(brushNodeInDefaultLayer)}, true}, {{getPath(brushNodeInCustomLayer)}, true}, {{getPath(brushNodeInEntity)}, true},
         {{getPath(brushNodeInGroup)}, true}, {{getPath(brushNodeInNestedGroup)}, true},
        }
    ));
// clang-format on
...
const auto map = std::map<const Model::Node *, std::string>{{brush, "brush"}, {pointEntity, "pointEntity"}, {patch, "patch"}, {brushEntity, "brushEntity"},
                                                            {brushInEntity1, "brushInEntity1"}, {brushInEntity2, "brushInEntity2"}, {outerGroup, "outerGroup"},
                                                            {brushInOuterGroup, "brushInOuterGroup"}, {innerGroup, "innerGroup"},
                                                            {brushInInnerGroup, "brushInInnerGroup"},
};

const auto mapNodeNames = [&](const auto &nodes) {
  return kdl::vec_transform(
      nodes, [&](const Model::Node *node) {
...
{
const auto [lineNumbers, expectedNodeNames] = GENERATE(
    values<T>(
        {{{0}, {}}, {{4}, {"brush"}}, {{5}, {"brush"}}, {{4, 5}, {"brush"}}, {{6}, {}}, {{7}, {}}, {{12}, {"pointEntity"}}, {{16}, {"patch"}},
         {{20}, {"brushInEntity1", "brushInEntity2"}}, {{24}, {"brushInEntity1"}}, {{26}, {"brushInEntity2"}}, {{31}, {"outerGroup"}}, {{32}, {"outerGroup"}},
         {{39}, {"outerGroup"}}, {{43}, {"outerGroup"}}, {{0, 4, 12, 24, 32}, {"brush", "pointEntity", "brushInEntity1", "outerGroup"}},
        }
    ));
...
CHECK_THAT(mapNodeNames(document->selectedNodes().nodes()), Catch::Matchers::UnorderedEquals(expectedNodeNames)
);
}
...
const auto [lineNumbers, expectedNodeNames] = GENERATE(
    values<T>(
        {{{31}, {}}, {{32}, {"brushInOuterGroup"}}, {{39}, {"innerGroup"}}, {{43}, {"innerGroup"}},}
    ));
...
REQUIRE_THAT(entityNodes, Catch::UnorderedEquals(
...
game->
setWorldNodeToLoad(
    std::make_unique<Model::WorldNode>(Model::EntityPropertyConfig{{}, true(setDefaultProperties)}, Model::Entity{}, Model::MapFormat::Standard)
);
document->
loadDocument(Model::MapFormat::Standard, document
->
...
auto *definitionWithDefaults = new Assets::PointEntityDefinition{
    "some_name", Color{}, vm::bbox3{32.0}, "", {std::make_shared<Assets::StringPropertyDefinition>("some_default_prop", "", "", ! true(readOnly), "value"),},
    {}};
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{Model::EntityPropertyKeys::Classname, "some_name"}, {"some_default_prop", "value"},}
)
...
auto *definitionWithDefaults = new Assets::BrushEntityDefinition{
    "some_name", Color{}, "", {std::make_shared<Assets::StringPropertyDefinition>("some_default_prop", "", "", ! true(readOnly), "value"),}};
document->setEntityDefinitions({
definitionWithDefaults});
...
auto *definitionWithDefaults = new Assets::PointEntityDefinition{
    "some_name", Color{}, vm::bbox3{32.0}, "", {
        ...
        std::make_shared<Assets::StringPropertyDefinition>("default_prop_a", "", "", ! true(readOnly), "default_value_a"),
        std::make_shared<Assets::StringPropertyDefinition>("default_prop_b", "", "", ! true(readOnly), "default_value_b"),
    }, {}};
document->setEntityDefinitions({
definitionWithDefaults});

auto *entityNodeWithoutDefinition = new Model::EntityNode{document->world()->entityPropertyConfig(), {{"classname", "some_class"},}};
document->addNodes({
{
document->
...
REQUIRE_THAT(entityNodeWithoutDefinition
->
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_class"}, {"some_prop", "some_value"},}
)

);
REQUIRE_THAT(entityNodeWithProp
->
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"some_prop", "some_value"},}
)

);
REQUIRE_THAT(entityNodeWithPropA
->
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"some_prop", "some_value"}, {"default_prop_a", "default_value_a"},}
)

);
REQUIRE_THAT(entityNodeWithPropAWithValueChanged
->
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "some_other_value"},}
)

);
REQUIRE_THAT(entityNodeWithPropsAB
->
...
document->selectNodes({
entityNodeWithoutDefinition, entityNodeWithProp, entityNodeWithPropA, entityNodeWithPropAWithValueChanged, entityNodeWithPropsAB
}
);
...
CHECK_THAT(entityNodeWithoutDefinition
->
...
CHECK_THAT(entityNodeWithPropA
->
...
CHECK_THAT(entityNodeWithPropAWithValueChanged
->
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "default_value_a"},}
)

);
CHECK_THAT(entityNodeWithPropsAB
->
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "some_other_value"}, {"default_prop_b", "default_value_b"},}
)
...
properties(), Catch::Matchers::UnorderedEquals(
    std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "default_value_a"}, {"default_prop_b", "default_value_b"},}
)
