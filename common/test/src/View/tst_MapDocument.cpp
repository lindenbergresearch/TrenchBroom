...
namespace View {
MapDocumentTest::MapDocumentTest() : MapDocumentTest(Model::MapFormat::Standard) {
}

MapDocumentTest::MapDocumentTest(const Model::MapFormat mapFormat) : m_mapFormat(mapFormat), m_pointEntityDef(nullptr), m_brushEntityDef(nullptr) {
    SetUp();
}
...
Model::PatchNode *MapDocumentTest::createPatchNode(const std::string &textureName) const {
    // clang-format off
    return new Model::PatchNode{
        Model::BezierPatch{3, 3, {{0, 0, 0}, {1, 0, 1}, {2, 0, 0}, {0, 1, 1}, {1, 1, 2}, {2, 1, 1}, {0, 2, 0}, {1, 2, 1}, {2, 2, 0}}, textureName}};
    // clang-format on
}

ValveMapDocumentTest::ValveMapDocumentTest() : MapDocumentTest(Model::MapFormat::Valve) {
}

Quake3MapDocumentTest::Quake3MapDocumentTest() : MapDocumentTest{Model::MapFormat::Quake3} {
}

TEST_CASE_METHOD(MapDocumentTest,
"MapDocumentTest.throwExceptionDuringCommand") {
CHECK_THROWS_AS(
    document
->

throwExceptionDuringCommand(), CommandProcessorException
...
CHECK(document
->world()->mapFormat() == Model::MapFormat::Valve);
CHECK(
    document
->world()->defaultLayer()->
...
{
auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/standardFormatMapWithoutFormatTag.map", "Quake", Model::MapFormat::Unknown);
CHECK(document
->world()->mapFormat() == Model::MapFormat::Standard);
CHECK(
    document
->world()->defaultLayer()->
...
{
auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/emptyMapWithoutFormatTag.map", "Quake", Model::MapFormat::Unknown);
...
CHECK_THROWS_AS(
    View::loadMapDocument("fixture/test/View/MapDocumentTest/mixedFormats.map", "Quake", Model::MapFormat::Unknown), IO::WorldReaderException
);
}

TEST_CASE("MapDocument.reloadTextureCollections")
{
auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/MapDocumentTest/reloadTextureCollectionsQ2.map", "Quake2", Model::MapFormat::Quake2);

const auto faces = kdl::vec_transform(document->world()->defaultLayer()->children(), [&](const auto *node) {
...
== 4);
REQUIRE(
    kdl::vec_transform(faces, [](const auto *face) { return face->attributes().textureName(); })
...
{
customLayerNode, {
brushNodeInCustomLayer}},
{
outerGroupNode, {
innerGroupNode, brushNodeInGroup}},
{
brushEntityNode, {
brushNodeInEntity}},
});
...
// clang-format off
const auto paths = GENERATE_COPY(values<T>({{}, {getPath(brushNodeInDefaultLayer)}, {getPath(brushNodeInDefaultLayer), getPath(brushNodeInCustomLayer)}, {getPath(brushNodeInDefaultLayer), getPath(brushNodeInCustomLayer), getPath(brushNodeInEntity)}, {getPath(brushNodeInGroup)}, {getPath(brushNodeInGroup), getPath(brushNodeInNestedGroup)},
                                           }));
// clang-format on
...
allSelectedBrushNodes(), Catch::Matchers::UnorderedEquals(brushNodes)
...
// clang-format off
const auto [pathsToSelect, expectedResult] = GENERATE_COPY(values<T>({{std::vector<Model::NodePath>{}, false}, {{getPath(pointEntityNode)}, false}, {{getPath(brushEntityNode)}, true}, {{getPath(outerGroupNode)}, true}, {{getPath(brushNodeInDefaultLayer)}, true}, {{getPath(brushNodeInCustomLayer)}, true}, {{getPath(brushNodeInEntity)}, true}, {{getPath(brushNodeInGroup)}, true}, {{getPath(brushNodeInNestedGroup)}, true},
                                                                     }));
// clang-format on
...
const auto map = std::map<const Model::Node *, std::string>{{brush, "brush"}, {pointEntity, "pointEntity"}, {patch, "patch"}, {brushEntity, "brushEntity"}, {brushInEntity1, "brushInEntity1"}, {brushInEntity2, "brushInEntity2"}, {outerGroup, "outerGroup"}, {brushInOuterGroup, "brushInOuterGroup"}, {innerGroup, "innerGroup"}, {brushInInnerGroup, "brushInInnerGroup"},
};
...
{
brushEntity, {
brushInEntity1, brushInEntity2}},
{
outerGroup, {
brushInOuterGroup, innerGroup}},
});
...
{
const auto [lineNumbers, expectedNodeNames] = GENERATE(values<T>({{{0}, {}}, {{4}, {"brush"}}, {{5}, {"brush"}}, {{4, 5}, {"brush"}}, {{6}, {}}, {{7}, {}}, {{12}, {"pointEntity"}}, {{16}, {"patch"}}, {{20}, {"brushInEntity1", "brushInEntity2"}}, {{24}, {"brushInEntity1"}}, {{26}, {"brushInEntity2"}}, {{31}, {"outerGroup"}}, {{32}, {"outerGroup"}}, {{39}, {"outerGroup"}}, {{43}, {"outerGroup"}}, {{0, 4, 12, 24, 32}, {"brush", "pointEntity", "brushInEntity1", "outerGroup"}},
                                                                 }));

CAPTURE(lineNumbers);

document->
selectNodesWithFilePosition(lineNumbers);
CHECK_THAT(
    mapNodeNames(document->selectedNodes().nodes()), Catch::Matchers::UnorderedEquals(expectedNodeNames)
);
}
...
const auto [lineNumbers, expectedNodeNames] = GENERATE(values<T>({{{31}, {}}, {{32}, {"brushInOuterGroup"}}, {{39}, {"innerGroup"}}, {{43}, {"innerGroup"}},
                                                                 }));
...
const auto [lineNumbers, expectedNodeNames] = GENERATE(values<T>({{{31}, {}}, {{32}, {}}, {{39}, {}}, {{43}, {"brushInInnerGroup"}},
                                                                 }));
...
auto *linkedEntityNode = dynamic_cast<Model::EntityNode *>(linkedInnerGroupNode->children().front());
...
REQUIRE_THAT(
    entityNodes, Catch::UnorderedEquals(std::vector<Model::EntityNodeBase *>{entityNode, linkedEntityNode})
);
...
CHECK_FALSE(
    document
->
canUpdateLinkedGroups(kdl::vec_element_cast<Model::Node *>(entityNodes)
));
}
...
game->
setWorldNodeToLoad(std::make_unique<Model::WorldNode>(Model::EntityPropertyConfig{{}, true(setDefaultProperties)}, Model::Entity{}, Model::MapFormat::Standard)
);
document
->
loadDocument(
    Model::MapFormat::Standard, document
->
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{Model::EntityPropertyKeys::Classname, "some_name"}, {"some_default_prop", "value"},
})
...
{
brushNode1, brushNode2, brushNode3}}});
...
auto *definitionWithDefaults = new Assets::PointEntityDefinition{
    "some_name", Color{}, vm::bbox3{32.0}, "", {
        std::make_shared<Assets::StringPropertyDefinition>("some_prop", "", "", !true(readOnly)), std::make_shared<Assets::StringPropertyDefinition>("default_prop_a", "", "", !true(readOnly), "default_value_a"), std::make_shared<Assets::StringPropertyDefinition>("default_prop_b", "", "", !true(readOnly), "default_value_b"),
    }, {}};
document->setEntityDefinitions({
definitionWithDefaults});

auto *entityNodeWithoutDefinition = new Model::EntityNode{
    document->world()->entityPropertyConfig(), {{"classname", "some_class"},
    }};
document->addNodes(
{
{
...
auto *entityNodeWithProp = document->createPointEntity(definitionWithDefaults, {0, 0, 0});
REQUIRE(entityNodeWithProp
!= nullptr);
REQUIRE(
    entityNodeWithProp
->
...
auto *entityNodeWithPropA = document->createPointEntity(definitionWithDefaults, {0, 0, 0});
REQUIRE(entityNodeWithPropA
!= nullptr);
REQUIRE(
    entityNodeWithPropA
->
...
auto *entityNodeWithPropAWithValueChanged = document->createPointEntity(definitionWithDefaults, {0, 0, 0});
REQUIRE(entityNodeWithPropAWithValueChanged
!= nullptr);
REQUIRE(
...
auto *entityNodeWithPropsAB = document->createPointEntity(definitionWithDefaults, {0, 0, 0});
REQUIRE(entityNodeWithPropsAB
!= nullptr);
REQUIRE(
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_class"}, {"some_prop", "some_value"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"some_prop", "some_value"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"some_prop", "some_value"}, {"default_prop_a", "default_value_a"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "some_other_value"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"some_prop", "some_value"}, {"default_prop_a", "default_value_a"}, {"default_prop_b", "yet_another_value"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "default_value_a"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"some_prop", "some_value"}, {"default_prop_a", "default_value_a"}, {"default_prop_b", "default_value_b"},
})
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "some_other_value"}, {"default_prop_b", "default_value_b"},
})
...
{
document->
setDefaultProperties(Model::SetDefaultPropertyMode::SetAll);
...
properties(), Catch::Matchers::UnorderedEquals(std::vector<Model::EntityProperty>{{"classname", "some_name"}, {"default_prop_a", "default_value_a"}, {"default_prop_b", "default_value_b"},
})
