...
TEST_CASE_METHOD(ValveMapDocumentTest, "SetEntityPropertiesTest.setProtectedProperty")
{
    ...
    {
        document->setProtectedProperty("some_key", true);
        REQUIRE_THAT(entityNode->entity().protectedProperties(), Catch::UnorderedEquals(std::vector<std::string>{
                "some_key"
            }
        ));
...
TEST_CASE_METHOD(ValveMapDocumentTest, "SetEntityPropertiesTest.setProtectedPropertyRestoresValue")
{
    ...
    REQUIRE_THAT(linkedEntityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "some_value"}}));
    ...
    REQUIRE_THAT(linkedEntityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "another_value"}}));

    // the value in the original entity remains unchanged
    entityNode = dynamic_cast<Model::EntityNode *>(groupNode->children().front());
    REQUIRE_THAT(entityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "some_value"}}));
    ...
    SECTION("When there is an unprotected property in the corresponding entity")
    {
        ...
        entityNode = dynamic_cast<Model::EntityNode *>(groupNode->children().front());
        CHECK_THAT(linkedEntityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "some_value"}}));
        CHECK_THAT(entityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "some_value"}}));
    }
    ...
    SECTION("When no corresponding entity with an unprotected property can be found")
    {
        ...
        entityNode = dynamic_cast<Model::EntityNode *>(groupNode->children().front());
        CHECK_THAT(entityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "some_value"}}));
        CHECK_THAT(linkedEntityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "another_value"}}));

        SECTION("Setting the property to unprotected in the original entity will fetch the new "
                "value now")
        {
            document->deselectAll();
            document->selectNodes({entityNode});
            ...
            linkedEntityNode = dynamic_cast<Model::EntityNode *>(linkedGroupNode->children().front());
            CHECK_THAT(entityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "another_value"}}));
            CHECK_THAT(linkedEntityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "another_value"}}));
        }
    }
...
TEST_CASE_METHOD(ValveMapDocumentTest, "SetEntityPropertiesTest.clearProtectedProperties")
{
    ...
    REQUIRE_THAT(entityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key",    "some_other_value"},
                                                                                                              {"another_key", "another_value"}}
    ));

    REQUIRE_THAT(linkedEntityNode->entity().protectedProperties(), Catch::UnorderedEquals(std::vector<std::string>{
    ...
    CHECK_THAT(entityNode->entity().properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key",        "some_other_value"},
                                                                                                            {"another_key",     "another_value"},
                                                                                                            {"yet_another_key", "and_yet_another_value"}}
    ));
