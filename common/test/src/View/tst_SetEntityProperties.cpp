...
"SetEntityPropertiesTest.changeClassname") {
...
== Model::EntityNode::DefaultBounds.
...
TEST_CASE_METHOD(ValveMapDocumentTest,
"SetEntityPropertiesTest.setProtectedProperty")
{
...
TEST_CASE_METHOD(ValveMapDocumentTest,
"SetEntityPropertiesTest.setProtectedPropertyRestoresValue")
{
...
properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "some_value"}})
...
properties(), Catch::UnorderedEquals(std::vector<Model::EntityProperty>{{"some_key", "another_value"}})
...
TEST_CASE_METHOD(ValveMapDocumentTest,
"SetEntityPropertiesTest.clearProtectedProperties")
{
auto *entityNode = new Model::EntityNode{{}, {{"some_key", "some_value"}, {"another_key", "another_value"}}};
document->addNodes({
{
document->
...
"EntityNodesTest.updateSpawnflagOnBrushEntity")
{
...
nodes(), Catch::UnorderedEquals(std::vector<Model::Node *>{brushNode})

);

REQUIRE(!brushEntNode->

entity()

.hasProperty("spawnflags"));
CHECK(document
->updateSpawnflag("spawnflags", 1, true));
