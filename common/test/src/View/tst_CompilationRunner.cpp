...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationRunToolTaskRunner.toolReturnsZeroExitCode")
{
...
auto task = Model::CompilationRunTool{true, RETURN_EXITCODE_PATH, "--exit 0", treatNonZeroResultCodeAsError};
auto runner = CompilationRunToolTaskRunner{context, task};
...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationRunToolTaskRunner.toolReturnsNonZeroExitCode")
{
auto variables = EL::NullVariableStore{};
auto output = QTextEdit{};
auto outputAdapter = TextOutputAdapter{&output};
...
const auto treatNonZeroResultCodeAsError = GENERATE(true, false);
auto task = Model::CompilationRunTool{true, RETURN_EXITCODE_PATH, "--exit 1", treatNonZeroResultCodeAsError};
auto runner = CompilationRunToolTaskRunner{context, task};
...
.started);
CHECK(exec
.errored == treatNonZeroResultCodeAsError);
CHECK(exec
...
const auto treatNonZeroResultCodeAsError = GENERATE(true, false);
auto task = Model::CompilationRunTool{true, RETURN_EXITCODE_PATH, "--abort", treatNonZeroResultCodeAsError};
auto runner = CompilationRunToolTaskRunner{context, task};
...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationRunToolTaskRunner.toolCrashes")
{
auto variables = EL::NullVariableStore{};
auto output = QTextEdit{};
auto outputAdapter = TextOutputAdapter{&output};
...
const auto treatNonZeroResultCodeAsError = GENERATE(true, false);
auto task = Model::CompilationRunTool{true, RETURN_EXITCODE_PATH, "--crash", treatNonZeroResultCodeAsError};
auto runner = CompilationRunToolTaskRunner{context, task};
...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationCopyFilesTaskRunner.createTargetDirectories")
{
auto variables = EL::NullVariableStore{};
auto output = QTextEdit{};
auto outputAdapter = TextOutputAdapter{&output};
...
auto task = Model::CompilationCopyFiles{
    true, (testEnvironment.dir() / sourcePath).string(), (testEnvironment.dir() / targetPath).string()
};
...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationRenameFileTaskRunner.renameFile")
{
...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationDeleteFilesTaskRunner.deleteTargetPattern")
{
auto variables = EL::NullVariableStore{};
auto output = QTextEdit{};
auto outputAdapter = TextOutputAdapter{&output};
...
auto task = Model::CompilationDeleteFiles{true, (testEnvironment.dir() / "*.lit").string()};
...
TEST_CASE_METHOD(MapDocumentTest,
"CompilationRunner.stopAfterFirstError")
{
auto variables = EL::NullVariableStore{};
auto output = QTextEdit{};
auto outputAdapter = TextOutputAdapter{&output};
...
auto testEnvironment = IO::TestEnvironment{};
testEnvironment.
createFile(does_exist,
"");

auto compilationProfile = Model::CompilationProfile{
    "name", testEnvironment.dir().string(), {
        Model::CompilationCopyFiles{
            true, does_not_exist, "does_not_matter.map"
        }, Model::CompilationCopyFiles{
            true, does_exist, should_not_exist
        },
    }};

auto runner = CompilationRunner{CompilationContext{document, variables, outputAdapter, false}, compilationProfile};
...
REQUIRE(compilationStartedSpy
.

isValid()

);
REQUIRE(compilationEndedSpy
.
...
CHECK_FALSE(testEnvironment
.
fileExists(should_not_exist)
);
}
...
const auto toInterpolate = startSubstr + std::string{"${MAP_DIR_PATH}"} + midSubstr + std::string{
    "${WORK_DIR_PATH}"
};
const auto expected = startSubstr + document->path().parent_path().string() + midSubstr + testWorkDir;
