...
TEST_CASE_METHOD(ValveMapDocumentTest, "ChangeBrushFaceAttributesTest.setAll")
{
    ...
    CHECK(brushNode->brush().face(thirdFaceIndex).attributes() == brushNode->brush().face(secondFaceIndex).attributes());
    ...
    CHECK(brushNode->brush().face(secondFaceIndex).attributes() == brushNode->brush().face(firstFaceIndex).attributes());
...
TEST_CASE("ChangeBrushFaceAttributesTest.Quake2IntegrationTest")
{
    ...
    auto [document, game, gameConfig] = View::loadMapDocument("fixture/test/View/ChangeBrushFaceAttributesTest/lavaAndWater.map", "Quake2",
