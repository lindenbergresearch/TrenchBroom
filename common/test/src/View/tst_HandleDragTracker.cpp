...
struct TestDelegateData {
  ...
  TestDelegateData(HandlePositionProposer i_initialGetHandlePositionToReturn) : initialGetHandlePositionToReturn{
      std::move(i_initialGetHandlePositionToReturn)
  } {
  }
};
...
struct TestDelegate : public HandleDragTrackerDelegate {
  ...
  TestDelegate(TestDelegateData &i_data) : data{i_data} {
  }
...
CHECK(data
.initializeArguments == std::vector<std::tuple<vm::vec3, vm::vec3>> {{
initialHandlePosition, handleOffset
}
}
);

AND_THEN("The initial handle position is passed to drag for the initial and the last "
"handle "
"position")
{
tracker.
...
auto data = TestDelegateData{
    makeHandlePositionProposer(
        ...
        [&](const auto &) { return handlePositionToReturn; }, makeIdentityHandleSnapper())
};
...
{
CHECK(data
.dragArguments == std::vector<std::tuple<DragState, vm::vec3>>{
{
{
vm::vec3{
1, 1, 1
...
handlePositionToReturn = vm::vec3{2, 2, 2};
data.
dragStatusToReturn = DragStatus::End;
...
{
CHECK(getHandlePositionArguments
==
std::vector<std::tuple<DragState, vm::vec3>>{
{
{
...
REQUIRE(initialGetHandlePositionArguments
.
...
{
tracker.
modifierKeyChange(InputState{}
);

THEN("The drag state are passed to the delegate")
{
CHECK(data
.modifierKeyChangeArguments == std::vector<DragState>{
{
vm::vec3{
1, 1, 1}, vm::vec3{
2, 2, 2}, vm::vec3{
...
CHECK(initialGetHandlePositionArguments
.
...
data.
updateDragConfigToReturn = UpdateDragConfig{
    makeHandlePositionProposer(
        // returns a constant hit position
...
AND_THEN("A synthetic drag to the new handle position happens using the other drag "
"config")
{
CHECK(initialGetHandlePositionArguments
.
...
== 1);
CHECK(otherGetHandlePositionArguments
.
...
// clang-format off
const auto [line, handleOffset, pickRay, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::line3{
                 vm::vec3{0, 0, 0}, vm::vec3{
                     0, 0, 1
                 }}, vm::vec3{
                0, 0, 0
            }, vm::ray3{
                vm::vec3{0, - 1, 0}, vm::vec3{0, 1, 0}}, vm::vec3{
                0, 0, 0
            }}, {
             vm::line3{
                 vm::vec3{0, 0, 0}, vm::vec3{
                ...
                vm::vec3{1, - 1, 1}, vm::vec3{
                    0, 1, 0
                }}, vm::vec3{
                0, 0, 0
            }}, // hitPoint is at {1 1 1}
         {
             vm::line3{
                 vm::vec3{0, 0, 0}, vm::vec3{
                     0, 0, 1
...
inputState.
setPickRequest(PickRequest{pickRay, camera}
);

CHECK(makeLineHandlePicker(line, handleOffset)
(inputState) == expectedHandlePosition);
}
...
// clang-format off
const auto [plane, handleOffset, pickRay, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::plane3{
                 vm::vec3{0, 0, 0}, vm::vec3{0, 0, 1}}, vm::vec3{
                0, 0, 0
            }, vm::ray3{
                vm::vec3{0, 0, 1}, vm::vec3{0, 0, - 1}}, vm::vec3{
                0, 0, 0
            }}, {
             vm::plane3{
                 vm::vec3{0, 0, 0}, vm::vec3{
...
CHECK(makePlaneHandlePicker(plane, handleOffset)
(inputState) == expectedHandlePosition);
}
...
// clang-format off
const auto [center, normal, radius, handleOffset, pickRay, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{
                 0, 0, 0
                ...
                vm::vec3{5, 0, 1}, vm::vec3{0, 0, - 1}}, 10.0 * vm::normalize(
                vm::vec3{
                    1, 0, 0
                }
            )
...
CHECK(makeCircleHandlePicker(center, normal, radius, handleOffset)
(inputState) == vm::approx{
...
// clang-format off
const auto [hitFilter, handleOffset, pickRay, expectedHandlePosition] = GENERATE_REF(
    values<T>(
        {{
             type(HitType), vm::vec3{
                0, 0, 0
            }, vm::ray3{
                vm::vec3{0, 0, 20}, vm::vec3{0, 0, - 1}}, vm::vec3{
                hit.hitPoint()
            }}, {
             type(OtherHitType), vm::vec3{
                0, 0, 0
            }, vm::ray3{
                vm::vec3{
                    0, 0, 20
...
addHit(otherHit);
inputState.
setPickResult(std::move(pickResult)
);

CHECK(makeSurfaceHandlePicker(hitFilter, handleOffset)
(inputState) == expectedHandlePosition);
}
...
// clang-format off
const auto [proposedHandlePosition, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{0, 0, 0}, vm::vec3{
                0, 0, 0
            }}, {
             vm::vec3{1, 2, 3}, vm::vec3{
                1, 2, 3
            }},
        }
    ));
...
CHECK (makeIdentityHandleSnapper()
...
// clang-format off
const auto [initialHandlePosition, proposedHandlePosition, gridSize, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{
                 3, 1, 2
...
CHECK(makeRelativeHandleSnapper(grid)
(InputState{
}, DragState{
initialHandlePosition, vm::vec3{
}, vm::vec3{
...
// clang-format off
const auto [proposedHandlePosition, gridSize, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{
                 0, 0, 0
...
const auto grid = Grid{gridSize};
CHECK(makeAbsoluteHandleSnapper(grid)
(InputState{
}, DragState{
vm::vec3{
}, vm::vec3{
...
// clang-format off
const auto [initialHandlePosition, proposedHandlePosition, gridSize, line, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{
                 0, 0, 0
...
const auto grid = Grid{gridSize};
CHECK(makeRelativeLineHandleSnapper(grid, line)
(InputState{
}, DragState{
initialHandlePosition, vm::vec3{
}, vm::vec3{
...
// clang-format off
const auto [proposedHandlePosition, gridSize, line, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{
                 0, 0, 0
...
const auto grid = Grid{gridSize};
CHECK(makeAbsoluteLineHandleSnapper(grid, line)
(InputState{
}, DragState{
vm::vec3{
}, vm::vec3{
...
// clang-format off
const auto [initialHandlePosition, proposedHandlePosition, snapAngle, expectedHandlePosition] = GENERATE(
    values<T>(
        {{
             vm::vec3{
                 1, 0, 0
...
CHECK(makeCircleHandleSnapper(grid, vm::to_radians(snapAngle), center, normal, radius)
(InputState{
}, DragState{
initialHandlePosition, vm::vec3{
0, 0, 0}, vm::vec3{
0, 0, 0}}, proposedHandlePosition
) == vm::approx{
radius *expectedHandlePosition
});
}} // namespace View
} // namespace TrenchBroom
