...
const auto createNode = GENERATE_COPY(CreateNode{
    ...
    [](const auto &) -> Model::Node * { return new Model::EntityNode{Model::Entity{}}; }}, CreateNode{
    [](const auto &test) -> Model::Node * {
        auto *entityNode = new Model::EntityNode{Model::Entity{}};
        auto *brushNode = test.createBrushNode();
        auto *patchNode = test.createPatchNode();
        ...
        return entityNode;
    }}, CreateNode{[](const auto &test) -> Model::Node * { return test.createBrushNode(); }}, CreateNode{[](const auto &test) -> Model::Node * { return test.createPatchNode(); }});
...
vertexPositions(), Catch::UnorderedEquals(std::vector<vm::vec3>{
    // bottom face
