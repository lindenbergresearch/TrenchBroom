...
enum class CommandNotif {
  CommandDo, CommandDone, CommandDoFailed, CommandUndo, CommandUndone, CommandUndoFailed, TransactionDone, TransactionUndone
};
...
public:
  explicit TestCommand(const std::string &name) : UndoableCommand(name, false) {
  }
...
private:
  ...
  bool doCollateWith(UndoableCommand &otherCommand) override {
    ...
    REQUIRE((expectedCall.expectedOtherCommand == nullptr || &otherCommand == expectedCall.expectedOtherCommand));
...
public:
  explicit NullCommand(std::string name) : UndoableCommand{std::move(name), true} {
  }
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, commandName}, {CommandNotif::CommandDone, commandName},
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandUndo, commandName}, {CommandNotif::CommandUndone, commandName},
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandUndo, commandName}, {CommandNotif::CommandUndoFailed, commandName},
    }
)
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, commandName}, {CommandNotif::CommandDoFailed, commandName},
    }
)
...
commandProcessor.
startTransaction(transactionName, TransactionScope::Oneshot
);
CHECK(commandProcessor
.
executeAndStore(std::move(command1)
)->
...
CHECK(commandProcessor
.
executeAndStore(std::move(command2)
)->
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, commandName1}, {CommandNotif::CommandDone, commandName1}, {CommandNotif::CommandDo, commandName2},
                                   {CommandNotif::CommandDone, commandName2}, {CommandNotif::TransactionDone, transactionName},
    }
)
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandUndo, commandName2}, {CommandNotif::CommandUndone, commandName2},
                                   {CommandNotif::CommandUndo, commandName1}, {CommandNotif::CommandUndone, commandName1},
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, commandName1}, {CommandNotif::CommandDone, commandName1},
    }
)
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, commandName2}, {CommandNotif::CommandDone, commandName2},
    }
)
...
commandProcessor.
startTransaction(outerTransactionName, TransactionScope::Oneshot
);
CHECK(commandProcessor
.
executeAndStore(std::move(outerCommand)
)->
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, outerCommandName}, {CommandNotif::CommandDone, outerCommandName},
    }
)
...
commandProcessor.
startTransaction(innerTransactionName, TransactionScope::Oneshot
);
CHECK(commandProcessor
.
executeAndStore(std::move(innerCommand)
)->
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandDo, innerCommandName}, {CommandNotif::CommandDone, innerCommandName},
    }
)
...
popNotifications(), Catch::Equals(std::vector<NotificationTuple>{{CommandNotif::TransactionDone, innerTransactionName},})
...
popNotifications(), Catch::Equals(std::vector<NotificationTuple>{{CommandNotif::TransactionDone, outerTransactionName},})
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandUndo, innerCommandName}, {CommandNotif::CommandUndone, innerCommandName},
                                   {CommandNotif::CommandUndo, outerCommandName}, {CommandNotif::CommandUndone, outerCommandName},
...
isCurrentDocumentStateObservable()

);

commandProcessor.
executeAndStore(std::make_unique<NullCommand>("command")
);
CHECK(commandProcessor
.
...
popNotifications(), Catch::Equals(
    std::vector<NotificationTuple>{{CommandNotif::CommandUndo, commandName1}, {CommandNotif::CommandUndone, commandName1},
...
transaction1_command1->
expectCollate(transaction1_command2
.
...
transaction2_command1->
expectCollate(transaction2_command2
.
...
transaction1_command2->
expectCollate(transaction2_command1
.
...
commandProcessor.
executeAndStore(std::move(transaction1_command1)
);
commandProcessor.
executeAndStore(std::move(transaction1_command2)
);
commandProcessor.
...
commandProcessor.
executeAndStore(std::move(transaction2_command1)
);
commandProcessor.
executeAndStore(std::move(transaction2_command2)
);
commandProcessor.
