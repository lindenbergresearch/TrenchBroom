...
saveRecentDocuments({
"1.map", "2.map", "filter.map",});
CHECK(loadRecentDocuments(1)
== std::vector<std::filesystem::path>{
"1.map"});
}
...
recentDocuments.
addMenu(menu1);
CHECK(getTexts(menu1.actions())
== std::vector<QString>{
"1.map", "2.map", "3.map",});

recentDocuments.
addMenu(menu2);
CHECK(getTexts(menu2.actions())
== std::vector<QString>{
"1.map", "2.map", "3.map",});
