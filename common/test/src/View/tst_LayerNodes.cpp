...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.defaultLayerSortIndexImmutable", "[LayerNodesTest]") {
Model::LayerNode *defaultLayerNode = document->world()->defaultLayer();
setLayerSortIndex(*defaultLayerNode,
555);
...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.duplicateObjectGoesIntoSourceLayer", "[LayerNodesTest]")
{
// delete default brush
document->
...
document->
setCurrentLayer(layerNode2);
document->selectNodes({
entity});
document->

duplicateObjects(); // the duplicate should stay in layer1
...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.newGroupGoesIntoSourceLayer", "[LayerNodesTest]")
{
// delete default brush
document->
...
== newGroup);
CHECK(Model::findContainingLayer(entity)
== layerNode1);
CHECK(Model::findContainingLayer(newGroup)
== layerNode1);
CHECK(
    document
->
...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.newObjectsInHiddenLayerAreVisible", "[LayerNodesTest]")
{
// delete default brush
document->
...
== Model::VisibilityState::Inherited);
CHECK(entity1
->
...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.newObjectsInLockedLayerAreUnlocked", "[LayerNodesTest]")
{
// delete default brush
document->
...
setLayerSortIndex(*layerNode0,
0);
setLayerSortIndex(*layerNode1,
1);
setLayerSortIndex(*layerNode2,
2);
...
CHECK(!document->
canMoveLayer(document
->world()->defaultLayer(), 1));
CHECK(document
->
canMoveLayer(layerNode0,
0));
CHECK(!document->
canMoveLayer(layerNode0,
-1));
...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.moveSelectionToLayer", "[LayerNodesTest]")
{
// delete default brush
document->
...
const auto createNode = GENERATE_COPY(CreateNode{
    [](const auto &test) {
        ...
        return groupNode;
    }}, CreateNode{[](const auto &) { return new Model::EntityNode{Model::Entity{}}; }}, CreateNode{[](const auto &test) { return test.createBrushNode(); }}, CreateNode{[](const auto &test) { return test.createPatchNode(); }});
...
REQUIRE(Model::findContainingLayer(node)
== defaultLayer);
...
node});
document->
moveSelectionToLayer(customLayer);

THEN("The group node is in the target layer")
{
CHECK(Model::findContainingLayer(node)
== customLayer);
...
REQUIRE(Model::findContainingLayer(entityNode)
== defaultLayer);

WHEN("Any child node is selected and moved to another layer")
{
// clang-format off
const auto [selectChild1, selectChild2] = GENERATE(std::make_tuple(true, true), std::make_tuple(true, false), std::make_tuple(false, true));
// clang-format on
...
{
CHECK(Model::findContainingLayer(entityNode)
== customLayer);
CHECK(
    childNode1
->
...
nodes(), Catch::Matchers::UnorderedEquals(selectedNodes)
...
TEST_CASE_METHOD(
    MapDocumentTest,
"LayerNodeTest.setCurrentLayerCollation", "[LayerNodesTest]")
{
// delete default brush
document->
