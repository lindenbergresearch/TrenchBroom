...
namespace View {
template<typename Move, typename End, typename Cancel, typename Render, typename MakeHandleSnapper>
struct TestDelegate : public MoveHandleDragTrackerDelegate {
  ...
  void
  render(const InputState &inputState, const DragState &dragState, Renderer::RenderContext &renderContext, Renderer::RenderBatch &renderBatch) const override {
...
static auto
makeInputState(const vm::vec3 &rayOrigin, const vm::vec3 &rayDirection, Renderer::Camera &camera, ModifierKeyState modifierKeys = ModifierKeys::MKNone) {
...
TEST_CASE("MoveDragTracker.constructor")
{
    ...
    GIVEN("A 3D camera")
    {
        ...
        WHEN("A tracker is created without any modifier keys pressed")
        {
            ...
            {
                CHECK(tracker.dragState() == DragState{initialHandlePosition, initialHandlePosition, handleOffset});
                ...
                AND_THEN("The tracker is using a default hit finder")
                {
                    ...
                    CHECK(tracker.dragState() == DragState{initialHandlePosition, vm::vec3{16, 80, 0}, handleOffset});
                }
            }
        }
        ...
        WHEN("A tracker is created with the alt modifier pressed")
        {
            ...
            THEN("The tracker is using a vertical hit finder")
            {
                ...
                // dragging
                REQUIRE(tracker.drag(makeInputState(vm::vec3{16, 16, 64}, vm::vec3{0, 1, -1}, camera3d)));
                CHECK(tracker.dragState() == vm::approx{
...
TEST_CASE("MoveDragTracker.modifierKeyChange")
{
    ...
    GIVEN("A tracker created with a 3D camera")
    {
        ...
        REQUIRE(tracker.dragState() == DragState{initialHandlePosition, initialHandlePosition, handleOffset});

        WHEN("The alt modifier is pressed")
        {
            tracker.modifierKeyChange(makeInputState(vm::vec3{0, 0, 64}, vm::vec3{
                    0, 1, -1
                }, camera3d, ModifierKeys::MKAlt
            ));
            ...
            AND_WHEN("The alt modifier is released")
            {
                ...
                THEN("The tracker switches to a default hit finder")
                {
                    ...
                    // when dragging
                    REQUIRE(tracker.drag(makeInputState(vm::vec3{16, 16, 64}, vm::vec3{0, 1, -1}, camera3d)));
                    CHECK(tracker.dragState() == DragState{vm::vec3{0, 64, 0}, vm::vec3{16, 80, 0}, handleOffset});
                }
            }
        }
        ...
        {
            REQUIRE(tracker.drag(makeInputState(vm::vec3{16, 16, 64}, vm::vec3{0, 1, -1}, camera3d)));
            REQUIRE(tracker.dragState() == DragState{initialHandlePosition, vm::vec3{16, 80, 0}, handleOffset});

            tracker.modifierKeyChange(makeInputState(vm::vec3{16, 16, 64}, vm::vec3{
                    0, 1, -1
                }, camera3d, ModifierKeys::MKShift
            ));
        ...
        WHEN("The shift modifier is pressed after the handle is moved non-diagonally")
        {
            ...
            REQUIRE(tracker.dragState() == DragState{initialHandlePosition, vm::vec3{16, 96, 0}, handleOffset});

            tracker.modifierKeyChange(makeInputState(vm::vec3{16, 32, 64}, vm::vec3{
                    0, 1, -1
                }, camera3d, ModifierKeys::MKShift
            ));
            ...
            THEN("The tracker has a constricted hit finder")
            {
                // we check this indirectly by observing how the move handle position changes when
                // dragging
                CHECK(tracker.dragState() == DragState{vm::vec3{0, 64, 0}, vm::vec3{0, 96, 0}, handleOffset});
            }
            ...
            AND_WHEN("The shift modifier is released")
            {
                ...
                THEN("The tracker switches back to a default hit finder")
                {
                    // we check this indirectly by observing how the move handle position changes
                    // when dragging
                    CHECK(tracker.dragState() == DragState{vm::vec3{0, 64, 0}, vm::vec3{16, 96, 0}, handleOffset});
                }
            }
        }
    }
    ...
    GIVEN("A tracker created with a 3D camera")
    {
        ...
        WHEN("The alt modifier is pressed")
        {
            tracker.modifierKeyChange(makeInputState(vm::vec3{0, 64, 64}, vm::vec3{
                    0, 0, -1
                }, camera2d, ModifierKeys::MKAlt
            ));
