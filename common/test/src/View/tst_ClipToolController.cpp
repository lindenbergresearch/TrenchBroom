...
"ClipToolControllerTest.testTwoPointsCreateClipPlane") {
const auto data = R"(
// entity 0
{
...
.setSize(2); // 2^2, so this sets it to grid 4
...
CHECK(tool
.
canAddPoint(clipPoint2)
);

// HACK: bias the points towards the center of the screen a bit
inputState.
mouseMove(clipPoint2ScreenSpace
.

x()

- 2.0f, clipPoint2ScreenSpace.
