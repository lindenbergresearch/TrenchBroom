...
bool texCoordsEqual(const vm::vec2f &tc1, const vm::vec2f &tc2) {
    for (size_t i = 0; i < 2; ++i) {
        ...
        if (!(vm::is_equal(distRemainder, 0.0f, vm::Cf::almost_zero()) || vm::is_equal(distRemainder, 1.0f, vm::Cf::almost_zero())))
            return false;
    }
    return true;
}
 ...
 */
bool UVListsEqual(const std::vector<vm::vec2f> &uvs, const std::vector<vm::vec2f> &transformedVertUVs) {
...
GameAndConfig loadGame(const std::string &gameName) {
    TestLogger logger;
    const auto configPath = std::filesystem::current_path() / "fixture/games" / gameName / "GameConfig.cfg";
    const auto gamePath = std::filesystem::current_path() / "fixture/test/Model/Game" / gameName;
...
namespace View {
DocumentGameConfig loadMapDocument(const std::filesystem::path &mapPath, const std::string &gameName, const Model::MapFormat mapFormat) {
