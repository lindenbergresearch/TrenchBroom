...
public:
  ...
  bool match(const kdl::result<T, E...> &in) const override {
    return m_expected.visit(
        kdl::overload(
            [&](const T &lhs) {
              return in.visit(
                  kdl::overload(
                      [&](const T &rhs) { return M{lhs}.match(rhs); }, [](const auto &) { return false; }
                  ));
            }, (
                [&](const E &lhs) {
                  return in.visit(
...
template<typename T, std::size_t S> UnorderedApproxVecMatcher<T, S> UnorderedApproxVecMatches(
