  ...
  public:
    ...
    bool match(const kdl::result<T, E...> &in) const override {
        return m_expected.visit(kdl::overload([&](const T &lhs) {
            return in.visit(kdl::overload([&](const T &rhs) { return M{lhs}.match(rhs); }, [](const auto &) { return false; }));
        }, (
                                                  [&](const E &lhs) {
...
inline auto MatchesPathsResult(std::vector<std::filesystem::path> paths) {
    return MatchesResult<decltype(Catch::UnorderedEquals(paths))>(Result<std::vector<std::filesystem::path>>{std::move(paths)});
}
...
template<typename T> class AnyOfMatcher : public Catch::MatcherBase<T> {
  ...
  public:
    ...
    bool match(const T &in) const override {
        return std::any_of(m_expected.begin(), m_expected.end(), [&](const auto &e) { return in == e; });
    }
...
template<typename T> class NoneOfMatcher : public Catch::MatcherBase<T> {
    std::vector<T> m_expected;

  public:
    explicit NoneOfMatcher(std::vector<T> expected) : m_expected{std::move(expected)} {
    }

    bool match(const T &in) const override {
        return std::none_of(m_expected.begin(), m_expected.end(), [&](const auto &e) { return in == e; });
    }
...
template<typename T> NoneOfMatcher<T> MatchesNoneOf(std::vector<T> expected) {
    return NoneOfMatcher<T>{std::move(expected)};
}

template<typename T> NoneOfMatcher<T> MatchesNoneOf(std::initializer_list<T> expected) {
    return NoneOfMatcher<T>(std::vector<T>{expected});
}

template<typename T> class AllDifferentMatcher : public Catch::MatcherBase<T> {
  public:
    bool match(const T &in) const override {
...
template<typename T> AllDifferentMatcher<T> AllDifferent() {
