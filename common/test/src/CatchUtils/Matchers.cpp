  ...
  return lhs.accept(
      kdl::overload(
          [&](const WorldNode *expectedWorldNode) {
            ...
            return inGroupNode && inGroupNode->group() == expectedGroupNode->group() && inGroupNode->linkId() == expectedGroupNode->linkId()
            ...
            return inEntityNode && inEntityNode->entity() == expectedEntityNode->entity() && inEntityNode->linkId() == expectedEntityNode->linkId()
...
bool nodesMatch(const std::vector<Node *> &lhs, const std::vector<Node *> &rhs) {
  return std::equal(
      lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), [](const auto *lhsChild, const auto *rhsChild) {
...
bool NodeVectorMatcher::match(const std::vector<Node *> &in) const {
  return std::equal(
      in.begin(), in.end(), m_expected.begin(), m_expected.end(), [](const auto &lhs, const auto &rhs) { return nodesMatch(*lhs, *rhs); }
  );
}
