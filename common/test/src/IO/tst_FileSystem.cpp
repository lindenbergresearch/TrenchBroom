            ...
            DirectoryEntry{
                "some_dir", {
                    DirectoryEntry{
                        ...
                        }}, FileEntry{"some_dir_file_1.TXT", makeObjectFile(3)}, FileEntry{"some_dir_file_2.doc", makeObjectFile(4)},
...
CHECK(fs
.makeAbsolute("/foo") == Result<std::filesystem::path> {
"/foo"
}
);
}
...
CHECK(fs
.find("root_file.map", TraversalMode::Flat) == Result <std::vector<std::filesystem::path>>{
Error{
"Path does not denote a directory: 'root_file.map'"}});

CHECK(fs
.find("", TraversalMode::Flat) == Result <std::vector<std::filesystem::path>>{
std::vector<std::filesystem::path>{
"some_dir", "root_file.map", "root_file.jpg",}});
...
CHECK(fs
.find("some_dir", TraversalMode::Recursive) == Result <std::vector<std::filesystem::path>>{
std::vector<std::filesystem::path>{
...
CHECK(fs
.find("", TraversalMode::Recursive, makeExtensionPathMatcher({
".txt", ".map"
}
...
#endif
CHECK(fs
.openFile("does_not_exist") == Result <std::shared_ptr<File>>{
Error{
