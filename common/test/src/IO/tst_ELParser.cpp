...
// the expression
CHECK(evaluate("\"asdf\\\" \\\"asdf\"")
== EL::Value(R"(asdf" "asdf)"));
}
...
array.
push_back(EL::Value(1.0)
);
array.
push_back(EL::Value("test")
);
nestedArray.
push_back(EL::Value(true)
);
array.
push_back(EL::Value(nestedArray)
);
...
CHECK(evaluate("[1..1]")
==
EL::Value(EL::ArrayType({EL::Value(1.0)})
));
CHECK(evaluate("[1..0]")
==
EL::Value(EL::ArrayType({EL::Value(1.0), EL::Value(0.0)})
));
CHECK(evaluate("[-2..1]")
==
EL::Value(EL::ArrayType(
...
map.
insert(std::make_pair("testkey1", EL::Value(1.0))
);
map.
insert(std::make_pair("testkey2", EL::Value("asdf"))
);
nestedMap.
insert(std::make_pair("nestedKey", EL::Value(true))
);
map.
insert(std::make_pair("testkey3", EL::Value(nestedMap))
);
...
CHECK(evaluate(" { \"testkey1\": 1, \"testkey2\"   :\"asdf\", \"testkey3\":{\"nestedKey\":true} }")
==
EL::Value(map)
);
}
...
EL::MapType map;
map.
insert(std::make_pair("key", EL::Value("value"))
);

EL::ArrayType array;
array.
push_back(EL::Value(map)
);

CHECK(evaluate(R"([ { "key": "value" } ])")
==
EL::Value(array)
);
}
...
EL::ArrayType array;
array.
push_back(EL::Value(inner)
);

EL::MapType outer;
outer.
insert(std::make_pair("outerkey1", EL::Value(array))
);
outer.
insert(std::make_pair("outerkey2", EL::Value("asdf"))
);

CHECK(evaluate(R"({ "outerkey1": [ { "key": "value" } ], "outerkey2": "asdf" })")
==
EL::Value(outer)
);
}
...
{
CHECK(evaluate("2 - 3.0")
== EL::Value(-1.0));
CHECK(evaluate("2 - 3 - 4")
== EL::Value(-5.0));
CHECK(evaluate("2 - 3 - 4 - 2")
== EL::Value(-7.0));
}
...
{
CHECK(evaluate("12 / 2.0")
== EL::Value(6.0));
CHECK(evaluate("12 / 2 / 2")
== EL::Value(3.0));
CHECK(evaluate("12 / 2 / 2 / 3")
== EL::Value(1.0));
}
...
CHECK(evaluate("12 % 2.0")
== EL::Value(0.0));
CHECK(evaluate("12 % 5 % 3")
== EL::Value(2.0));
CHECK(evaluate("12 % 5 % 3 % 3")
== EL::Value(2.0));
}
...
{
CHECK(evaluate("[ 1.0, 2.0, \"test\" ][0]")
== EL::Value(1.0));
CHECK(evaluate("[ 1.0, 2.0, \"test\" ][1]")
== EL::Value(2.0));
CHECK(evaluate("[ 1.0, 2.0, \"test\" ][2]")
== EL::Value("test"));
CHECK(evaluate("[ 1.0, 2.0, \"test\" ][-1]")
== EL::Value("test"));
CHECK(evaluate("[ 1.0, 2.0, \"test\" ][-2]")
== EL::Value(2.0));
CHECK(evaluate("[ 1.0, 2.0, \"test\" ][-3]")
== EL::Value(1.0));

CHECK(evaluate("[ 1.0, 2.0, \"test\" ][1 + 1]")
== EL::Value("test"));

CHECK(evaluate("{ \"key1\":1, \"key2\":2, \"key3\":\"test\"}[\"key1\"]")
== EL::Value(1.0));
CHECK(evaluate("{ \"key1\":1, \"key2\":2, \"key3\":\"test\"}[\"key2\"]")
== EL::Value(2.0));
CHECK(evaluate("{ \"key1\":1, \"key2\":2, \"key3\":\"test\"}[\"key3\"]")
== EL::Value("test"));

CHECK(evaluate("[ 1.0, [ 2.0, \"test\"] ][0]")
== EL::Value(1.0));
CHECK(evaluate("[ 1.0, [ 2.0, \"test\"] ][1][0]")
== EL::Value(2.0));
CHECK(evaluate("[ 1.0, [ 2.0, \"test\"] ][1][1]")
== EL::Value("test"));

CHECK(evaluate("{ \"key1\":1, \"key2\":2, \"key3\":[ 1, 2]}[\"key3\"][1]")
== EL::Value(2.0));
...
CHECK(evaluate("\"test\"[3,2,1,0]")
== EL::Value("tset"));
CHECK(evaluate("\"test\"[2,1,0]")
== EL::Value("set"));
CHECK(evaluate("\"test\"[2..1]")
== EL::Value("se"));

CHECK(evaluate("\"test\"[..0]")
== EL::Value("tset"));
CHECK(evaluate("\"test\"[1..]")
== EL::Value("est"));
}
...
CHECK(evaluate("true -> false")
== EL::Value(false));
CHECK(evaluate("true -> true && true")
== EL::Value(true));
CHECK(evaluate("1 < 3 -> 2 + 3")
== EL::Value(5));
CHECK(evaluate("false -> true")
==
...
CHECK(evaluate("{{'asdf'}}")
== EL::Value("asdf"));
CHECK(evaluate("{{'fdsa', 'asdf'}}")
== EL::Value("fdsa"));
CHECK(evaluate("{{false -> 'fdsa', 'asdf'}}")
== EL::Value("asdf"));
CHECK(evaluate("{{false -> false}}")
== EL::Value::Undefined);
}
...
CHECK(evaluate("\"test\" == \"test\"")
== EL::Value(true));
CHECK(evaluate("\"test1\" == \"test\"")
== EL::Value(false));
CHECK(evaluate("\"test\" != \"test\"")
== EL::Value(false));
CHECK(evaluate("\"test1\" != \"test\"")
...
{
CHECK(evaluate("7 + 2 * 3")
== evaluate("2 * 3 + 7"));
CHECK(evaluate("7 + 2 * 3 + 2")
== evaluate("2 * 3 + 7 + 2"));
CHECK(evaluate("7 + 2 * 3 + 2 * 2")
== evaluate("2 * 3 + 7 + 2 * 2"));
CHECK(evaluate("7 + 2 / 3 + 2 * 2")
== evaluate("2 / 3 + 7 + 2 * 2"));

CHECK(evaluate("3 + 2 < 3 + 3")
== evaluate("(3 + 2) < (3 + 3)"));
CHECK(evaluate("3 + 2 < 3 + 3 + 0 && true")
== evaluate("((3 + 2) < (3 + 3 + 0)) && true"));
CHECK(evaluate("false && false || true")
== EL::Value(true));
CHECK(evaluate("false && (false || true)")
== EL::Value(false));
}
...
CHECK(evaluate("(1)")
== EL::Value(1.0));
CHECK(evaluate("(2+1)*3")
== EL::Value(9.0));
CHECK(evaluate("(2+1)*(2+1)")
== EL::Value(9.0));
CHECK(evaluate("(2+1)*((1+1)*2)")
== EL::Value(12.0));
}} // namespace IO
} // namespace TrenchBroom
