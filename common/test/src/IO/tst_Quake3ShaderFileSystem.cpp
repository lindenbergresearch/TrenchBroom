...
TEST_CASE("Quake3ShaderFileSystemTest.testShaderLinking")
{
    ...
    CHECK_THAT(fs.find(texturePrefix / "test", TraversalMode::Flat, makeExtensionPathMatcher({""})), MatchesPathsResult({
    ...
    CHECK_THAT(fs.find(texturePrefix, TraversalMode::Recursive, makeExtensionPathMatcher({""})), MatchesPathsResult({
