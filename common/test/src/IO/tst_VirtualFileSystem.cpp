...
TEST_CASE("VirtualFileSystem")
{
    ...
    SECTION("if nothing is mounted")
    {
        ...
        {
            CHECK(vfs.makeAbsolute("") == Result<std::filesystem::path>{Error{"Failed to make absolute path of ''"}});
            CHECK(vfs.makeAbsolute("foo/bar") == Result<std::filesystem::path>{
        ...
        SECTION("find")
        {
            CHECK(vfs.find("", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                Error{"Path does not denote a directory: ''"}});
            CHECK(vfs.find("foo/bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
        ...
        SECTION("openFile")
        {
            ...
            CHECK(vfs.openFile("foo") == Result<std::shared_ptr<File>>{Error{"'foo' not found"}});
            CHECK(vfs.openFile("foo/bar") == Result<std::shared_ptr<File>>{Error{"'foo/bar' not found"}});
        }
    }
    ...
    SECTION("with a file system mounted at the root")
    {
        ...
        vfs.mount("", std::make_unique<TestFileSystem>(Entry{
            DirectoryEntry{
                "", {
                    DirectoryEntry{
                        "foo", {
        ...
        {
            CHECK(vfs.find("", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"foo", "bar",}});
            CHECK(vfs.find("foo", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"foo/bar",}});
            CHECK(vfs.find("foo/bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
    ...
    SECTION("with two file systems mounted at the root")
    {
        ...
        SECTION("find")
        {
            ...
            CHECK(vfs.find("foo/bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"foo/bar/baz"}});
            CHECK(vfs.find("bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"bar/foo", "bar/bat", "bar/baz", "bar/cat",}});
        }
        ...
        SECTION("openFile")
        {
            ...
            CHECK(vfs.openFile("bar/cat") == Result<std::shared_ptr<File>>{Error{"'bar/cat' not found"}});
        }
    }
    ...
    SECTION("with two file systems mounted at different mount points")
    {
        ...
        vfs.mount("foo", std::make_unique<TestFileSystem>(Entry{
            DirectoryEntry{"", {DirectoryEntry{"bar", {FileEntry{"baz", foo_bar_baz},}},}}}, "/fs1"
        ));
        vfs.mount("bar", std::make_unique<TestFileSystem>(Entry{
            DirectoryEntry{"", {FileEntry{"foo", bar_foo},}}}, "/fs2"
        ));
        ...
        SECTION("find")
        {
            ...
            CHECK(vfs.find("foo/bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"foo/bar/baz",}});
            CHECK(vfs.find("bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"bar/foo",}});
        }
    ...
    SECTION("with two file systems mounted at nested mount points")
    {
        ...
        vfs.mount("foo/bar", std::make_unique<TestFileSystem>(Entry{
            DirectoryEntry{"", {FileEntry{"foo", foo_bar_foo},}}}, "/fs2"
        ));
        ...
        SECTION("makeAbsolute")
        {
            ...
            CHECK(vfs.makeAbsolute("foo/bar/baz") == Result<std::filesystem::path>{"/fs1/bar/baz"});
        }
        ...
        SECTION("find")
        {
            ...
            CHECK(vfs.find("foo", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"foo/bar",}});
            CHECK(vfs.find("foo/bar", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
                std::vector<std::filesystem::path>{"foo/bar/baz", "foo/bar/foo",}});
        }
    ...
    SECTION("with two file systems mounted at nested mount points and overriding")
    {
                    ...
                    DirectoryEntry{
                        "bar", {
                            FileEntry{
                                ...
                                "c", fs1_foo_bar_c
                            }, // overridden by fs2_foo_bar_c
                            FileEntry{
                                "e", fs1_foo_bar_e
                            }, FileEntry{
                                "f", fs1_foo_bar_f
                            }, // overridden by directory in fs2
                            DirectoryEntry{
                                "g", {}},       // overridden by fs2_foo_bar_g
                        }},
                }}}, "/fs1"
        ));
        vfs.mount("foo/bar", std::make_unique<TestFileSystem>(Entry{
        ...
        SECTION("openFile")
        {
            ...
            CHECK(vfs.openFile("foo/bar/f") == Result<std::shared_ptr<File>>{Error{"'foo/bar/f' not found"}});
