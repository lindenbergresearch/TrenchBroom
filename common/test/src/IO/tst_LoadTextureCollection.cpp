...
std::optional<TextureCollectionInfo> makeInfo(const Result<Assets::TextureCollection> &result) {
  return result.transform(
      [](const auto &textureCollection) -> std::optional<TextureCollectionInfo> {
        return TextureCollectionInfo{
            textureCollection.path(), kdl::vec_transform(
                textureCollection.textures(), [](const auto &texture) {
...
fs.mount("textures" / wadPath.

filename(), openFS<WadFileSystem>(wadPath)
...
CHECK(makeInfo(loadTextureCollection("textures/cr8_czg.wad", fs, textureConfig, logger))
...
{
const auto textureConfig = Model::TextureConfig{"textures", {".D"}, "fixture/test/palette.lmp", "wad", "", {"*-jam", "coffin2", "czg_*"},};
