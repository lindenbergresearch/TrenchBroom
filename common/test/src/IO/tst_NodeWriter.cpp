...
const std::string expected = R"(// entity 0
{
...
Model::WorldNode map({}, {}, Model::MapFormat::Standard);
map.defaultLayer()->
setVisibilityState(Model::VisibilityState::Hidden);
map.defaultLayer()->
setLockState(Model::LockState::Locked);

auto layer = map.defaultLayer()->layer();
layer.
setColor(Color(0.25f, 0.75f, 1.0f)
);
...
const std::string actual = str.str();
const std::string expected = fmt::format(R"(// entity 0
{{
"classname" "worldspawn"
}}
// entity 1
...
map.defaultLayer()->
setLayer(std::move(defaultLayer)
);

auto *defaultLayerPointEntityNode = new Model::EntityNode(Model::Entity({}, {{"classname", "defaultLayerPointEntity"}}));

auto *defaultLayerBrushNode = new Model::BrushNode(builder.createCube(64.0, "defaultTexture").value());
map.defaultLayer()->
addChild(defaultLayerPointEntityNode);
map.defaultLayer()->
addChild(defaultLayerBrushNode);
...
layerNode1->
setLayer(std::move(layer1)
);

map.
addChild(layerNode1);

auto *layer1PointEntityNode = new Model::EntityNode(Model::Entity({}, {{"classname", "layer1PointEntity"}}));
layerNode1->
addChild(layer1PointEntityNode);

auto *layer1BrushNode = new Model::BrushNode(builder.createCube(64.0, "layer1Texture").value());
layerNode1->
addChild(layer1BrushNode);
...
auto *layer2PointEntityNode = new Model::EntityNode(Model::Entity({}, {{"classname", "layer2PointEntity"}}));
layerNode2->
addChild(layer2PointEntityNode);

auto *layer2BrushNode = new Model::BrushNode(builder.createCube(64.0, "layer2Texture").value());
layerNode2->
addChild(layer2BrushNode);
...
layerNode->
setLockState(Model::LockState::Inherited);
...
innerGroupNode->
addChild(innerBrushNode);
outerGroupNode->
addChild(innerGroupNode);
map.defaultLayer()->
addChild(worldBrushNode);
map.defaultLayer()->
addChild(outerGroupNode);

std::vector<Model::Node *> nodes;
nodes.
push_back(innerGroupNode);
nodes.
push_back(worldBrushNode);
...
group.
transform(vm::translation_matrix(vm::vec3(32.0, 0.0, 0.0))
);
...
groupClone.
transform(vm::translation_matrix(vm::vec3(0.0, 16.0, 0.0))
);
groupNodeClone->
setGroup(std::move(groupClone)
);

worldNode.defaultLayer()->
addChild(groupNodeClone);
REQUIRE(
    groupNodeClone
->
...
NodeWriter writer(worldNode, str);
writer.
writeNodes(std::vector<Model::Node *>{groupNode}
);
...
NodeWriter writer(worldNode, str);
writer.
writeNodes(std::vector<Model::Node *>{entityNode}
);
...
const std::string actual = str.str();
const std::string expected = R"(( -32 -32 -32 ) ( -32 -31 -32 ) ( -32 -32 -31 ) none 0 0 0 1 1
( -32 -32 -32 ) ( -32 -32 -31 ) ( -31 -32 -32 ) none 0 0 0 1 1
( -32 -32 -32 ) ( -31 -32 -32 ) ( -32 -31 -32 ) none 0 0 0 1 1
( 32 32 32 ) ( 32 33 32 ) ( 33 32 32 ) none 0 0 0 1 1
( 32 32 32 ) ( 33 32 32 ) ( 32 32 33 ) none 0 0 0 1 1
...
const auto expected = R"(// entity 0
{
...
{
Model::WorldNode map({}, {{"message\\", "holy damn\\"}, {"message2", "holy damn\\\\"}, {"message3", "holy damn\\\\\\"},
...
.
transform(
    worldBounds, vm::rotation_matrix(vm::to_radians(15.0), vm::to_radians(22.0), vm::to_radians(89.0)),
false)
.
...
face.
setAttributes(std::move(faceAttributes)
);
...
const auto [mapFormat, expectedSerializationTemplate] = GENERATE(values<FormatInfo>({{
                                                                                         Model::MapFormat::Standard, R"(// entity 0
{{
"classname" "worldspawn"
// brush 0
{{
...
( 32 32 32 ) ( 32 32 33 ) ( 32 33 32 ) {0} 0 0 0 1 1
                                                                                         ...
                                                                                         Model::MapFormat::Valve, R"(// entity 0
{{
"classname" "worldspawn"
// brush 0
{{
...
map.defaultLayer()->addChild(new Model::BrushNode{
std::move(brush)
});
...
==
fmt::format(expectedSerializationTemplate, expectedName
));
}
...
auto patch = Model::BezierPatch{
    5, 3, {{-64, -64, 4, 0, 0}, {-64, 0, 4, 0, -0.25}, {-64, 64, 4, 0, -0.5}, {0, -64, 4, 0.2, 0}, {0, 0, 4, 0.2, -0.25}, {0, 64, 4, 0.2, -0.5}, {64, -64, 4, 0.4, 0}, {64, 0, 4, 0.4, -0.25}, {64, 64, 4, 0.4, -0.5}, {128, -64, 4, 0.6, 0}, {128, 0, 4, 0.6, -0.25}, {128, 64, 4, 0.6, -0.5}, {192, -64, 4, 0.8, 0}, {192, 0, 4, 0.8, -0.25}, {192, 64, 4, 0.8, -0.5}}, "common/caulk"
};

auto map = Model::WorldNode{{}, {}, Model::MapFormat::Standard};
map.defaultLayer()->addChild(new Model::PatchNode{
std::move(patch)
});
