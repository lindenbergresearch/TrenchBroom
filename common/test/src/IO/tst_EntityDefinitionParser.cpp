...
CHECK(status
.
countStatus(LogLevel::Warn)
== 0u);
CHECK(status
.
countStatus(LogLevel::Error)
== 0u);
}
...
CHECK_THAT(options, Catch::Equals(
    std::vector<Assets::FlagsPropertyOption>(
        {{1 << 1, "a1_1", "", true}, {1 << 2, "a2_2", "", true}, {1 << 4, "a2_4", "", false},}
    ))
);
}
...
auto mergedModelDef = pointModelDef;
mergedModelDef.
append(base2ModelDef);
mergedModelDef.
append(base1ModelDef);
...
const auto expected = std::vector<EntityDefinitionClassInfo>(
    {{EntityDefinitionClassType::PointClass, 0, 0, "point", "base2", Color(1, 2, 3), vm::bbox3(- 2, 2), mergedModelDef, {a1_2, a3, a2}, {"base2"}},}
);
...
const auto expected = std::vector<EntityDefinitionClassInfo>(
    {{
         EntityDefinitionClassType::PointClass, 0, 0, "point1", "base2_1", Color(
            1, 2, 3
        ), vm::bbox3(
            - 2, 2
        ), std::nullopt, {a3, a2_1, a1, a2_2}, {"base2_1", "base2_2"}}, {
         EntityDefinitionClassType::PointClass, 0, 0, "point2", "base2_2", Color(
            1, 2, 3
