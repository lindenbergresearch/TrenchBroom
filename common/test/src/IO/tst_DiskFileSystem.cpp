...
TEST_CASE("DiskFileSystemTest")
{
    ...
    SECTION("makeAbsolute")
    {
        ...
        CHECK(fs.makeAbsolute("dir1/does_not_exist.txt") == env.dir() / "dir1/does_not_exist.txt");
    ...
    SECTION("pathInfo")
    {
        ...
        CHECK(fs.pathInfo("anotherDir/./subDirTest/../subDirTest/test2.map") == PathInfo::File);
        CHECK(fs.pathInfo("ANOtherDir/test3.MAP") == PathInfo::File);
    ...
    SECTION("find")
    {
        ...
        CHECK(fs.find("asdf/bleh", TraversalMode::Flat) == Result<std::vector<std::filesystem::path>>{
        ...
        CHECK_THAT(fs.find("anotherDir", TraversalMode::Flat), MatchesPathsResult({
...
#else
        CHECK(fs.openFile("/hopefully_nothing.here") == Result<std::shared_ptr<File>>{
            Error{"Path '/hopefully_nothing.here' is absolute"}});
#endif
        CHECK(fs.openFile("..") == Result<std::shared_ptr<File>>{Error{"'..' not found"}});
        CHECK(fs.openFile(".") == Result<std::shared_ptr<File>>{Error{"'.' not found"}});
        CHECK(fs.openFile("anotherDir") == Result<std::shared_ptr<File>>{Error{"'anotherDir' not found"}});
...
TEST_CASE("WritableDiskFileSystemTest")
{
    ...
    SECTION("createDirectory")
    {
...
#else
        CHECK(fs.createDirectory("/hopefully_nothing_here") == Result<bool>{
            Error{"Path '/hopefully_nothing_here' is absolute"}});
#endif
        CHECK(fs.createDirectory("..") == Result<bool>{Error{"Failed to make absolute path of '..'"}});
        CHECK_THAT(fs.createDirectory("test.txt"), MatchesAnyOf({
                // macOS
                Result<bool>{
                    Error{
        ...
        CHECK(fs.createDirectory("someDir/someOtherDir/.././yetAnotherDir") == Result<bool>{true});
    ...
    SECTION("deleteFile")
    {
...
#else
        CHECK(fs.deleteFile("/hopefully_nothing_here.txt") == Result<bool>{
            Error{"Path '/hopefully_nothing_here.txt' is absolute"}});
#endif
        CHECK(fs.deleteFile("") == Result<bool>{
            Error{"Failed to delete '" + (env.dir()).string() + "': path denotes a directory"}});
        CHECK(fs.deleteFile(".") == Result<bool>{
            Error{"Failed to delete '" + (env.dir() / "").string() + "': path denotes a directory"}});
        CHECK(fs.deleteFile("..") == Result<bool>{Error{"Failed to make absolute path of '..'"}});
        CHECK(fs.deleteFile("dir1") == Result<bool>{
            Error{"Failed to delete '" + (env.dir() / "dir1").string() + "': path denotes a directory"}});
        ...
        CHECK(fs.deleteFile("anotherDir/subDirTest/.././subDirTest/./test2.map") == Result<bool>{true});
...
#else
        CHECK(fs.moveFile("/hopefully_nothing_here.txt", "dest.txt") == Result<void>{
            Error{"'/hopefully_nothing_here.txt' is absolute"}});
        CHECK(fs.moveFile("test.txt", "/dest.txt") == Result<void>{Error{"'/dest.txt' is absolute"}});
#endif
    ...
    SECTION("copyFile")
    {
        ...
        CHECK(
          ...
          == Result<void>{Error{"'C:\\dest.txt' is absolute"}});
#else
        CHECK(fs.copyFile("/hopefully_nothing_here.txt", "dest.txt") == Result<void>{
            Error{"'/hopefully_nothing_here.txt' is absolute"}});
        CHECK(fs.copyFile("test.txt", "/dest.txt") == Result<void>{Error{"'/dest.txt' is absolute"}});
#endif
