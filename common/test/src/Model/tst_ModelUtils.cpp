...
CHECK(findContainingLayer(&worldNode)
== nullptr);
CHECK(findContainingLayer(layerNode)
== layerNode);

CHECK (findContainingLayer(groupNode)

== layerNode);

CHECK (findContainingLayer(entityNode)

== layerNode);

CHECK (findContainingLayer(brushNode)

== layerNode);

CHECK (findContainingLayer(patchNode)
...
innerGroupNode->addChildren({
entityNode, brushNode});
outerGroupNode->addChildren({
innerGroupNode, patchNode});
worldNode.defaultLayer()->
addChild(outerGroupNode);

CHECK(findContainingGroup(&worldNode)
== nullptr);
CHECK(findContainingGroup(layerNode)
== nullptr);
CHECK(findContainingGroup(outerGroupNode)
== nullptr);
CHECK(findContainingGroup(innerGroupNode)
== outerGroupNode);
CHECK(findContainingGroup(entityNode)
== innerGroupNode);
CHECK(findContainingGroup(brushNode)
== innerGroupNode);
CHECK(findContainingGroup(patchNode)
== outerGroupNode);
}
...
{
CHECK(findOutermostClosedGroup(&worldNode)
== nullptr);
CHECK(findOutermostClosedGroup(layerNode)
== nullptr);
CHECK(findOutermostClosedGroup(outerGroupNode)
== nullptr);
CHECK(findOutermostClosedGroup(innerGroupNode)
== outerGroupNode);
CHECK(findOutermostClosedGroup(entityNode)
== outerGroupNode);
CHECK(findOutermostClosedGroup(brushNode)
== outerGroupNode);
CHECK(findOutermostClosedGroup(patchNode)
== outerGroupNode);
}
...
setLinkedGroupId(*groupNode1,
"group1");
setLinkedGroupId(*groupNode2,
"group2");
...
addChild(groupNode3);
worldNode.defaultLayer()->
addChild(linkedGroupNode1_1);
worldNode.defaultLayer()->
addChild(linkedGroupNode2_1);
worldNode.defaultLayer()->
addChild(linkedGroupNode2_2);
...
groupNode.addChild(new EntityNode{
Entity{
}});

auto touchesAll = BrushNode{BrushBuilder{mapFormat, worldBounds}.createCube(24.0, "texture").value()};
REQUIRE_FALSE(touchesAll
.
intersects(&worldNode)
);
REQUIRE_FALSE(touchesAll
.
intersects(&layerNode)
);
REQUIRE(touchesAll
.
intersects(&groupNode)
);
REQUIRE(touchesAll
.
intersects(&entityNode)
);
REQUIRE(touchesAll
.
intersects(&brushNode)
);
REQUIRE(touchesAll
.
intersects(&patchNode)
);
...
auto containsAll = BrushNode{BrushBuilder{mapFormat, worldBounds}.createCube(128.0, "texture").value()};
REQUIRE_FALSE(containsAll
.
contains(&worldNode)
);
REQUIRE_FALSE(containsAll
...
auto containsPatch = BrushNode{BrushBuilder{mapFormat, worldBounds}.createCube(8.0, "texture").value()};
REQUIRE_FALSE(containsPatch
.
contains(&worldNode)
);
REQUIRE_FALSE(containsPatch
...
CHECK_THAT(collectBrushFaces({&worldNode}), Catch::Matchers::UnorderedEquals(toHandles(brushNode))
);
}
...
worldNode.defaultLayer()->
addChild(selectedBrushNode);
worldNode.defaultLayer()->
addChild(unselectedBrushNode);
selectedBrushNode->

select();

CHECK(collectSelectedBrushFaces({&worldNode})
.
...
auto worldNode = WorldNode{{}, {}, mapFormat};
auto *selectableBrushNode = new BrushNode{BrushBuilder{mapFormat, worldBounds}.createCube(64.0, "texture").value()};
auto *unselectableBrushNode = new BrushNode{
    BrushBuilder{mapFormat, worldBounds}.createCube(64.0, "texture").value()
};

worldNode.defaultLayer()->
addChild(selectableBrushNode);
worldNode.defaultLayer()->
addChild(unselectableBrushNode);
unselectableBrushNode->
setLockState(LockState::Locked);

auto editorContext = EditorContext{};

CHECK_THAT(collectSelectableBrushFaces({&worldNode}, editorContext), Catch::Matchers::UnorderedEquals(toHandles(selectableBrushNode))
);
}
...
CHECK(computeLogicalBounds({&worldNode})
== vm::bbox3d{
});
CHECK(computeLogicalBounds({layerNode})
== vm::bbox3d{
});
CHECK(computeLogicalBounds({entityNode})
== vm::bbox3d{
vm::vec3d{
-8, -8, -8}, vm::vec3d{
8, 8, 8}});
CHECK(computeLogicalBounds({brushNode})
== vm::bbox3d{
vm::vec3d{
32, -32, -32}, vm::vec3d{
96, 32, 32}});
CHECK(computeLogicalBounds({patchNode})
== vm::bbox3d{
vm::vec3d{
0, 0, 0}, vm::vec3d{
2, 2, 2}});
CHECK(computeLogicalBounds({entityNode, brushNode})
== vm::bbox3d{
vm::vec3d{
-8, -32, -32}, vm::vec3d{
96, 32, 32}});
...
CHECK(computePhysicalBounds({&worldNode})
== vm::bbox3d{
});
CHECK(computePhysicalBounds({layerNode})
== vm::bbox3d{
});
CHECK(computePhysicalBounds({entityNode})
== vm::bbox3d{
vm::vec3d{
-8, -8, -8}, vm::vec3d{
8, 8, 8}});
CHECK(computePhysicalBounds({brushNode})
== vm::bbox3d{
vm::vec3d{
32, -32, -32}, vm::vec3d{
96, 32, 32}});
CHECK(computePhysicalBounds({patchNode})
== vm::bbox3d{
vm::vec3d{
0, 0, 0}, vm::vec3d{
2, 2, 1}});
CHECK(computePhysicalBounds({entityNode, brushNode})
== vm::bbox3d{
vm::vec3d{
-8, -32, -32}, vm::vec3d{
96, 32, 32}});
