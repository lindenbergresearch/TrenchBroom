...
const auto games = std::vector<std::filesystem::path>{"Quake", "Daikatana", "Quake3",};

for (
const auto &game
: games) {
const auto configPath = std::filesystem::current_path() / "fixture/games/" / game / "GameConfig.cfg";
...
const auto gamePath = std::filesystem::current_path() / "fixture/test/Model/Game/CorruptPak";
...
{
const auto configPath = std::filesystem::current_path() / "fixture/games/Quake3/GameConfig.cfg";
const auto configStr = IO::readTextFile(configPath);
auto configParser = IO::GameConfigParser{configStr, configPath};
auto config = configParser.parse();

const auto gamePath = std::filesystem::current_path() / "fixture/test/Model/Game/Quake3";
auto logger = NullLogger{};
...
game.
loadTextureCollections(textureManager);
