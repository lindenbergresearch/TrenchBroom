...
const BrushFaceAttributes attribs("");
CHECK_FALSE(BrushFace::create(p0, p1, p2, attribs, std::make_unique<ParaxialTexCoordSystem>(p0, p1, p2, attribs))
.
...
if (
vm::get_abs_max_component_axis(face
.
...
CHECK(dot(textureNormal, vm::vec3(negXFace->boundary().normal))
> 0.0);
...
posXFace->
copyTexCoordSystemFromFace(*snapshot, negYFace
->
...
{
face.
flipTexture(cameraUp, cameraRight, vm::direction::left
);
CHECK(face
.
...
{
face.
flipTexture(cameraUp, cameraRight, vm::direction::up
);
CHECK(face
.
