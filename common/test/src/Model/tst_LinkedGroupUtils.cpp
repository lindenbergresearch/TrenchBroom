...
setLinkId(*groupNode1,
"group1");
setLinkId(*groupNode2,
"group2");
...
REQUIRE(
static_cast<EntityNode*>(groupNodeClone->
...
updateLinkedGroups(*innerGroupNode,
{
innerGroupNodeClone.
...
REQUIRE(innerGroupEntityNodeClone
!= nullptr);

updateLinkedGroups(outerGroupNode,
{
outerGroupNodeClone.
...
CHECK(newInnerGroupEntityNodeClone
!= nullptr);
CHECK(newInnerGroupEntityNodeClone
->
...
CHECK(e
== Error{
"Updating a linked node would exceed world bounds"});
});
}
...
setGroupName(*innerGroupNodeNestedClone,
...
properties(), Catch::Equals(sourceEntityNode->entity().properties())
...
entity.
setProtectedProperties(srcProtProperties);
sourceEntityNode->
...
entity.
setProtectedProperties(trgtProtProperties);
targetEntityNode->
setEntity(std::move(entity)
);
}
...
updateLinkedGroups(sourceGroupNode,
{
targetGroupNode.
...
properties(), Catch::UnorderedEquals(expectedTargetProperties)
...
protectedProperties(), Catch::UnorderedEquals(targetEntityNode->entity().protectedProperties())
...
sourceGroupNode.addChildren({
sourceBrushNode, sourceEntityNode});
...
targetEntity.setProtectedProperties({
"light"});
targetEntity.addOrUpdateProperty({
}, "light", "500");
targetEntityNode->
setEntity(std::move(targetEntity)
);
}

auto *sourceBrushEntity = new EntityNode{Entity{}};
sourceGroupNode.
removeChild(sourceBrushNode);
sourceBrushEntity->addChildren({
sourceBrushNode});
sourceGroupNode.addChildren({
sourceBrushEntity});
...
protectedProperties(), Catch::UnorderedEquals(std::vector<std::string>{"light"})
...
innerPatchNode, innerEntityNode});
outerGroupNode->addChildren({
outerEntityNode, outerBrushNode, innerGroupNode});

auto *linkedOuterGroupNode = new GroupNode{Group{"outer"}};
auto *linkedOuterEntityNode = new EntityNode{Entity{}};
auto *linkedOuterBrushNode = new BrushNode{brushBuilder.createCube(64.0, "texture").value()};
...
setLinkId(*outerGroupNode,
"outerGroupLinkId");
setLinkId(*linkedOuterGroupNode,
"outerGroupLinkId");
setLinkId(*innerGroupNode,
"innerGroupLinkId");
setLinkId(*linkedInnerGroupNode,
"innerGroupLinkId");
...
{
linkedInnerGroupNode->addChildren({
linkedInnerPatchNode, linkedInnerEntityNode});
...
auto *linkedOuterBrushNode2 = new BrushNode{brushBuilder.createCube(64.0, "texture").value()};
...
linkedInnerGroupNode2->addChildren({
linkedInnerPatchNode2, linkedInnerEntityNode2});
linkedOuterGroupNode2->addChildren(
{
linkedOuterEntityNode2, linkedOuterBrushNode2, linkedInnerGroupNode2});
layerNode.addChildren({
linkedOuterGroupNode2});

setLinkId(*linkedOuterGroupNode2,
"outerGroupLinkId");
setLinkId(*linkedInnerGroupNode2,
"innerGroupLinkId");
...
setLinkId(*topLevelLinkedInnerGroupNode,
"innerGroupLinkId");
layerNode.addChildren({
...
{
linkedOuterGroupNode->addChildren({
linkedOuterEntityNode, linkedOuterBrushNode});
...
{
linkedInnerGroupNode->addChildren({
linkedInnerEntityNode, linkedInnerPatchNode});
linkedOuterGroupNode->addChildren(
{
linkedOuterEntityNode, linkedOuterBrushNode, linkedInnerGroupNode});
