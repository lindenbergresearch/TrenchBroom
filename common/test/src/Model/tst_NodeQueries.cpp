...
CHECK(collectNodes({&worldNode})
== std::vector<Node *>{
&worldNode});
CHECK(
    collectNodes({outerGroupNode, entityNode})
== std::vector<Node *>{
outerGroupNode, entityNode});
CHECK(
...
CHECK_THAT(
    collectAncestors({layerNode}), Catch::UnorderedEquals(std::vector<Node *>{&worldNode})
);
CHECK_THAT(
    collectAncestors({outerGroupNode}), Catch::UnorderedEquals(std::vector<Node *>{&worldNode, layerNode})
);
CHECK_THAT(
...
{
CHECK_THAT(
    collectNodesAndDescendants({innerGroupNode}), Catch::UnorderedEquals(std::vector<Node *>{innerGroupNode, entityNode, brushNode})
);
CHECK_THAT(
...
CHECK_THAT(
    collectBrushFaces({brushNode, brushNode}), Catch::Matchers::UnorderedEquals(toHandles(brushNode))
);
}
