...
TEST_CASE("WorldNodeTest.nodeTreeUpdates")
{
    ...
    SECTION("Updating a descendant updates it in node tree")
    {
        ...
        REQUIRE(nodeTree.contains(brushNode));
        REQUIRE(nodeTree.contains(patchNode));
        REQUIRE_THAT(nodeTree.find_containers(vm::vec3d::zero()), Catch::UnorderedEquals(std::vector<Node *>{
                entityNode, brushNode, patchNode
            }
        ));
        REQUIRE_THAT(nodeTree.find_containers(vm::vec3d{384, 384, 384}), Catch::UnorderedEquals(std::vector<Node *>{}));
        ...
        CHECK(nodeTree.contains(brushNode));
        CHECK(nodeTree.contains(patchNode));
        CHECK_THAT(nodeTree.find_containers(vm::vec3d::zero()), Catch::UnorderedEquals(std::vector<Node *>{}));
        CHECK_THAT(nodeTree.find_containers(vm::vec3d{384, 384, 384}), Catch::UnorderedEquals(std::vector<Node *>{
                entityNode, brushNode, patchNode
            }
        ));
    }
