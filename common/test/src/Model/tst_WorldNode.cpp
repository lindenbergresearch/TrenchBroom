...
REQUIRE_THAT(nodeTree
.
find_containers(vm::vec3d{384, 384, 384}
),
Catch::UnorderedEquals(std::vector<Node *>{}
));

transformNode(*entityNode, vm::translation_matrix(vm::vec3d(384, 384, 384)), worldBounds
);
transformNode(*brushNode, vm::translation_matrix(vm::vec3d(384, 384, 384)), worldBounds
);
transformNode(*patchNode, vm::translation_matrix(vm::vec3d(384, 384, 384)), worldBounds
);
...
== std::nullopt);
worldNode.
addChild(initialLayerNode);
CHECK(initialLayerNode
->
...
worldNode.
addChild(layerNodeWithId);
CHECK(layerNodeWithId
->
...
worldNode.
addChild(layerNodeWithoutId);
REQUIRE(layerNodeWithoutId
->

persistentId()

!= std::nullopt);
CHECK(*layerNodeWithoutId
->
...
layerNodeWithLowerId->
setPersistentId(lowerId);

worldNode.
addChild(layerNodeWithLowerId);
REQUIRE(layerNodeWithLowerId
->
...
== std::nullopt);
worldNode.defaultLayer()->
addChild(initialGroupNode);
CHECK(initialGroupNode
->
...
worldNode.defaultLayer()->
addChild(groupNodeWithId);
CHECK(groupNodeWithId
->
...
worldNode.defaultLayer()->
addChild(groupNodeWithoutId);
REQUIRE(groupNodeWithoutId
->

persistentId()

!= std::nullopt);
CHECK(*groupNodeWithoutId
->
...
groupNodeWithLowerId->
setPersistentId(lowerId);

worldNode.defaultLayer()->
addChild(groupNodeWithLowerId);
REQUIRE(groupNodeWithLowerId
->
