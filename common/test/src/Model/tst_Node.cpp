...
#endif
grandChild1->
expectCall(DoParentWillChange{}
);
grandChild1->
expectCall(DoAncestorWillChange{}
);
grandChild1->
expectCall(DoParentDidChange{}
);
grandChild1->
expectCall(DoAncestorDidChange{}
);
child->
...
children(), Catch::UnorderedEquals(std::vector<Node *>{child3})
...
CHECK(root
.
isAncestorOf(grandChild1_1)
);
CHECK(root
.
isAncestorOf(grandChild1_2)
);
...
CHECK(root
.
isAncestorOf(std::vector<Node *>{&root, child1, child2, grandChild1_1, grandChild1_2}
));
CHECK(child1
->
isAncestorOf(std::vector<Node *>{&root, child1, child2, grandChild1_1, grandChild1_2}
...
CHECK_FALSE(root
.
isDescendantOf(child1)
);
CHECK_FALSE(root
.
isDescendantOf(child2)
);
CHECK_FALSE(root
.
isDescendantOf(grandChild1_1)
);
CHECK_FALSE(root
.
isDescendantOf(grandChild1_2)
);
...
CHECK_FALSE(root
.
isDescendantOf(std::vector<Node *>{&root, child1, child2, grandChild1_1, grandChild1_2}
));
CHECK(child1
->
isDescendantOf(std::vector<Node *>{&root, child1, child2, grandChild1_1, grandChild1_2}
...
enum class Visited {
  World, Layer, Group, Entity, Brush, Patch,
};
...
layer->
addChild(entityNode1);
layer->
addChild(entityNode2);
layer->
addChild(groupNode);
groupNode->
...
CHECK(world
.
visitParent(nodeTestVisitor)
== std::nullopt);
CHECK(world
.
visitParent(constNodeTestVisitor)
== std::nullopt);
...
auto visited = std::vector<Node *>{};
world.
visitChildren(makeCollectVisitedNodesVisitor(visited)
);
...
CHECK(root
.
resolvePath(NodePath{{0}}
) == &child1);
CHECK(root
.
resolvePath(NodePath{{1}}
) == &child2);
CHECK(root
.
resolvePath(NodePath{{2}}
) == nullptr);
CHECK(root
.
resolvePath(NodePath{{0, 0}}
) == &child1_1);
CHECK(root
.
resolvePath(NodePath{{0, 0, 0}}
) == &child1_1_1);
CHECK(root
.
resolvePath(NodePath{{0, 1}}
) == &child1_2);
CHECK(child1
.
resolvePath(NodePath{{0, 0}}
