...
Result<std::unique_ptr<WorldNode>> TestGame::doLoadMap(
    const MapFormat format, const vm::bbox3 & /* worldBounds */, const std::filesystem::path & /* path */, Logger & /* logger */) const {
...
std::vector<Node *> TestGame::doParseNodes(
    const std::string &str, const MapFormat mapFormat, const vm::bbox3 &worldBounds, const std::vector<std::string> &linkedGroupsToKeep,
    Logger & /* logger */) const {
...
std::vector<BrushFace> TestGame::doParseBrushFaces(
    const std::string &str, const MapFormat mapFormat, const vm::bbox3 &worldBounds, Logger & /* logger */) const {
  IO::TestParserStatus status;
...
std::filesystem::path TestGame::doFindEntityDefinitionFile(
    const Assets::EntityDefinitionFileSpec & /* spec */, const std::vector<std::filesystem::path> & /* searchPaths */) const {
  return {};
}
