...
properties(), Catch::Matchers::UnorderedEquals(expectedProperties)
...
auto config = EntityPropertyConfig{{{EL::LiteralExpression{EL::Value{2.0}}, 0, 0}}};
auto definition = Assets::PointEntityDefinition{
    "some_name", Color(), vm::bbox3(32.0), "", {}, Assets::ModelDefinition{{EL::MapExpression{{}}, 0, 0}}};
...
== Assets::ModelSpecification{
"maps/b_shell0.bsp", 0, 0});

entity.addOrUpdateProperty({
}, EntityPropertyKeys::Spawnflags, "1");
CHECK(entity
.
...
protectedProperties(), Catch::UnorderedEquals(std::vector<std::string>{"newKey"})

);

entity.addOrUpdateProperty({
}, "test", "anotherValue", true);
CHECK_THAT(entity
.
...
REQUIRE(*entity
.property("originalKey") == "originalValue");
...
entity.
renameProperty(config,
"something", "modelscale");
CHECK(entity
.
...
CHECK(entity
.hasPropertyWithPrefix("somename", "somevalue"));
CHECK(entity
.hasPropertyWithPrefix("some", "somevalue"));
CHECK(entity
.hasPropertyWithPrefix("some", "someothervalue"));
CHECK(entity
.hasPropertyWithPrefix("someother", "someothervalue"));
...
CHECK(entity
.hasNumberedProperty("target", "value"));
CHECK(entity
.hasNumberedProperty("target", "value1"));
CHECK(entity
.hasNumberedProperty("target", "value2"));
...
Entity entity;
REQUIRE(!entity.
hasProperty(EntityPropertyKeys::Classname)
);
...
entity.addOrUpdateProperty({
}, EntityPropertyKeys::Classname, "testclass");
SECTION("Entities with a classname property return the value")
{
CHECK(*entity
.
property(EntityPropertyKeys::Classname)
== "testclass");
CHECK(entity
.
...
Entity entity;
REQUIRE(!entity.
hasProperty(EntityPropertyKeys::Origin)
);
...
CHECK(*entity
.
property(EntityPropertyKeys::Origin)
== "1 2 3");
CHECK(entity
.
...
vm::scaling_matrix(vm::vec3{
    2, 2, 2
}
));

entity.
setOrigin(config, vm::vec3{9, 8, 7}
);
REQUIRE(entity
.
...
vm::translation_matrix(vm::vec3(100, 0, 0)
));
CHECK(entity
.
...
entity.
setDefinition(config, &otherDefinition
);
REQUIRE(entity
.
...
entity.
transform(config, vm::translation_matrix(vm::vec3{8, 7, 6})
);
CHECK(entity
.
...
entity.
setClassname(entityPropertyConfig,
"light");
entity.
addOrUpdateProperty(entityPropertyConfig, EntityPropertyKeys::Angle,
"0");
...
{
entity.
transform(entityPropertyConfig, rotation
);
CHECK(*entity
.
property(EntityPropertyKeys::Angle)
== "90");
}
...
updateAnglePropertyAfterTransform = false;
