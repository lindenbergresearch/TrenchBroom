    ...
    SECTION("Updates cached model transformation")
    {
        ...
        auto definition = Assets::PointEntityDefinition{
            "some_name", Color{}, vm::bbox3{32.0}, "", {}, Assets::ModelDefinition{{
                                                                                       EL::MapExpression{
                                                                                           {{"scale", {EL::VariableExpression{"modelscale"}, 0, 0}}}}, 0, 0
                                                                                   }}};
...
TEST_CASE("EntityTest.setDefaultProperties")
{
    ...
    const auto [initialProperties, mode, expectedProperties] = GENERATE(values<T>({{{},                                     SetDefaultPropertyMode::SetExisting, std::vector<EntityProperty>{}},
...
TEST_CASE("EntityTest.modelSpecification")
{
    ...
    CHECK(entity.modelSpecification() == Assets::ModelSpecification{"maps/b_shell0.bsp", 0, 0});

    entity.addOrUpdateProperty({}, EntityPropertyKeys::Spawnflags, "1");
    CHECK(entity.modelSpecification() == Assets::ModelSpecification{"maps/b_shell1.bsp", 0, 0});
}
...
{
    Entity entity;
    entity.setProperties({}, {{"somename",      "somevalue"},
...
{
    Entity entity;
    entity.setProperties({}, {{"target",  "value"},
                              {"target1", "value1"},
                              {"target2", "value2"},
        }
    );
...
TEST_CASE("EntityTest.setOrigin")
{
    ...
    SECTION("Updates cached model transformation")
    {
        ...
        entity.setDefinition(config, &definition);
        REQUIRE(entity.modelTransformation() == vm::translation_matrix(vm::vec3{1, 2, 3}) * vm::scaling_matrix(vm::vec3{
                2, 2, 2
            }
        ));

        entity.setOrigin(config, vm::vec3{9, 8, 7});
        REQUIRE(entity.modelTransformation() == vm::translation_matrix(vm::vec3{9, 8, 7}) * vm::scaling_matrix(vm::vec3{
                2, 2, 2
            }
        ));
    }
...
TEST_CASE("EntityTest.transform")
{
    ...
    SECTION("Updates cached model transformation")
    {
        ...
        CHECK(entity.modelTransformation() == vm::translation_matrix(vm::vec3{8, 7, 6}) * vm::scaling_matrix(vm::vec3{
                2, 2, 2
            }
        ));
    }
