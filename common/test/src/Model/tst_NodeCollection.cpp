...
TEST_CASE("NodeCollection.collections")
{
    ...
    CHECK_THAT(nodeCollection.nodes(), Catch::Matchers::UnorderedEquals(std::vector<Node *>{
            &layerNode, &groupNode, &entityNode, &brushNode, &patchNode
        }
    ));

    CHECK_THAT(nodeCollection.layers(), Catch::Matchers::UnorderedEquals(std::vector<LayerNode *>{&layerNode}));

    CHECK_THAT(nodeCollection.groups(), Catch::Matchers::UnorderedEquals(std::vector<GroupNode *>{&groupNode}));

    CHECK_THAT(nodeCollection.entities(), Catch::Matchers::UnorderedEquals(std::vector<EntityNode *>{&entityNode}));

    CHECK_THAT(nodeCollection.brushes(), Catch::Matchers::UnorderedEquals(std::vector<BrushNode *>{&brushNode}));

    CHECK_THAT(nodeCollection.patches(), Catch::Matchers::UnorderedEquals(std::vector<PatchNode *>{&patchNode}));
