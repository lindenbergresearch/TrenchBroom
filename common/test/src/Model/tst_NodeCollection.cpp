...
nodeCollection.
addNode(&entityNode);
REQUIRE_THAT(nodeCollection
.

nodes(), Catch::Matchers::UnorderedEquals(std::vector<Node *>{&entityNode})

);

CHECK_FALSE(nodeCollection
.
...
REQUIRE_THAT(std::vector<Node *>(nodeCollection.begin(), nodeCollection.end()), Catch::Matchers::UnorderedEquals(std::vector<Node *>{})
);
...
layers(), Catch::Matchers::UnorderedEquals(std::vector<LayerNode *>{&layerNode})
...
groups(), Catch::Matchers::UnorderedEquals(std::vector<GroupNode *>{&groupNode})
...
entities(), Catch::Matchers::UnorderedEquals(std::vector<EntityNode *>{&entityNode})
...
brushes(), Catch::Matchers::UnorderedEquals(std::vector<BrushNode *>{&brushNode})
...
patches(), Catch::Matchers::UnorderedEquals(std::vector<PatchNode *>{&patchNode})
...
layerNode.
addChild(brushInLayer);
groupNode.
addChild(brushInGroup);
entityNode.
addChild(brushInEntity);
}}
...
== std::vector<Node *>{
&layerNode, &entityNode, &brushNode, &patchNode});
CHECK(nodeCollection
.
...
== std::vector<Node *>{
&layerNode, &groupNode, &brushNode, &patchNode});
CHECK(nodeCollection
.
...
== std::vector<Node *>{
&layerNode, &groupNode, &entityNode, &patchNode});
CHECK(nodeCollection
.
...
== std::vector<Node *>{
&layerNode, &groupNode, &entityNode, &brushNode});
CHECK(nodeCollection
.
