...
TEST_CASE("GroupNodeTest.updateLinkedGroups")
{
    ...
    {
        updateLinkedGroups(groupNode, {}, worldBounds).transform([&](const UpdateLinkedGroupsResult &r) { CHECK(r.empty()); }).transform_error(
            [](const auto &) { FAIL(); }
        );
    }
    ...
    SECTION("Target group list contains only source group")
    {
        ...
        ).transform([&](const UpdateLinkedGroupsResult &r) { CHECK(r.empty()); }).transform_error([](const auto &) { FAIL(); });
    }
...
TEST_CASE("GroupNodeTest.updateLinkedGroupsExceedsWorldBounds")
{
    ...
    ).transform([](auto) { FAIL(); }).transform_error([](auto e) {
