...
grandParentGroupNode.
addChild(parentGroupNode);
parentGroupNode->
addChild(groupNode);
groupNode->
addChild(childGroupNode);

REQUIRE_FALSE(grandParentGroupNode
.

opened()

);
REQUIRE(grandParentGroupNode
.

closed()

);
REQUIRE_FALSE(parentGroupNode
->

opened()

);
REQUIRE(parentGroupNode
->

closed()

);
REQUIRE_FALSE(groupNode
->
...
REQUIRE_FALSE(childGroupNode
->

opened()

);
REQUIRE(childGroupNode
->
...
CHECK_FALSE(grandParentGroupNode
.
...
CHECK_FALSE(childGroupNode
->

opened()

);
CHECK(childGroupNode
->
...
CHECK(grandParentGroupNode
.

hasOpenedDescendant()

);
CHECK(parentGroupNode
->
...
CHECK_FALSE(groupNode
.
canAddChild(&worldNode)
);
CHECK_FALSE(groupNode
.
canAddChild(&layerNode)
);
CHECK_FALSE(groupNode
.
canAddChild(&groupNode)
);
CHECK(groupNode
.
canAddChild(&entityNode)
);
CHECK(groupNode
.
canAddChild(&brushNode)
);
CHECK(groupNode
.
canAddChild(&patchNode)
);
...
"linked_group_id");
setLinkedGroupId(*linkedGroupNode, *groupNode
.
...
CHECK(groupNode
.
canRemoveChild(&worldNode)
);
CHECK(groupNode
.
canRemoveChild(&layerNode)
);
CHECK(groupNode
.
canRemoveChild(&groupNode)
);
CHECK(groupNode
.
canRemoveChild(&entityNode)
);
CHECK(groupNode
.
canRemoveChild(&brushNode)
);
CHECK(groupNode
.
canRemoveChild(&patchNode)
);
}
...
REQUIRE(static_cast<EntityNode *>(groupNodeClone->
...
SECTION("Updating outerGroupNode retains the names of its linked group and the nested linked "
"group")
{
updateLinkedGroups(outerGroupNode,
{
