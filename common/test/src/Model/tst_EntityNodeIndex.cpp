...
CHECK(findExactExact(index, "test", "notfound")
.
...
EntityNode *entity2 = new EntityNode({}, {{"test", "somevalue"},});
...
CHECK(findExactExact(index, "other", "someothervalue")
.
...
CHECK(findNumberedExact(index, "test", "notfound")
.
...
allKeys(), Catch::UnorderedEquals(std::vector<std::string>{"test", "other"})
...
CHECK_THAT(index
.
allValuesForKeys(EntityNodeIndexQuery::exact("test")
),
