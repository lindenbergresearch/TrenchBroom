...
REQUIRE(d1
.
decalSpecification(EL::NullVariableStore{}
) == DecalSpecification {
"decal1"
}
);
...
d1.
append(makeDecalDefinition(R"("decal2")")
);
CHECK(d1
.
decalSpecification(EL::NullVariableStore{}
...
CHECK(
    decalDefinition
.
decalSpecification(EL::VariableTable{variables}
)
...
const auto decalDefinition = makeDecalDefinition(expression);
CHECK(decalDefinition
.

defaultDecalSpecification()
