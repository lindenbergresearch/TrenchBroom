...
Autosaver::Autosaver(std::weak_ptr<MapDocument> document, const std::chrono::milliseconds saveInterval, const size_t maxBackups) :
    m_document{std::move(document)}, m_saveInterval{saveInterval}, m_maxBackups{maxBackups}, m_lastSaveTime{
    Clock::now()
...
Result<IO::WritableDiskFileSystem> createBackupFileSystem(const std::filesystem::path &mapPath) {
...
Result<std::vector<std::filesystem::path>> collectBackups(const IO::FileSystem &fs, const std::filesystem::path &mapBasename) {
...
Result<std::vector<std::filesystem::path>> thinBackups(
    Logger &logger, IO::WritableDiskFileSystem &fs, const std::vector<std::filesystem::path> &backups, const size_t maxBackups
) {
...
std::filesystem::path makeBackupName(const std::filesystem::path &mapBasename, const size_t index) {
  return kdl::path_add_extension(mapBasename, "." + kdl::str_to_string(index) + ".map");
}

Result<void> cleanBackups(IO::WritableDiskFileSystem &fs, std::vector<std::filesystem::path> &backups, const std::filesystem::path &mapBasename) {
  return kdl::fold_results(
      kdl::vec_transform(
...
void Autosaver::autosave(Logger &logger, std::shared_ptr<MapDocument> document) {
  ...
  createBackupFileSystem(mapPath).and_then(
      [&](auto fs) {
        return collectBackups(fs, mapBasename).and_then(
