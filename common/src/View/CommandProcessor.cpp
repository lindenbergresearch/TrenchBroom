...
namespace View {
template<typename Ignore, typename T, typename C> void notifyCommandIfNotType(T &notifier, C &command) {
...
class CommandProcessor::TransactionCommand : public UndoableCommand {
  ...
  public:
    TransactionCommand(std::string name, std::vector<std::unique_ptr<UndoableCommand>> commands, Notifier<Command &> &i_commandDoNotifier, Notifier<Command &> &i_commandDoneNotifier, Notifier<UndoableCommand &> &i_commandUndoNotifier, Notifier<UndoableCommand &> &i_commandUndoneNotifier)
        : UndoableCommand(std::move(name), false), m_commands{std::move(commands)}, m_commandDoNotifier{i_commandDoNotifier}, m_commandDoneNotifier{i_commandDoneNotifier}, m_commandUndoNotifier{i_commandUndoNotifier}, m_commandUndoneNotifier{i_commandUndoneNotifier} {
    }
...
std::unique_ptr<UndoableCommand> CommandProcessor::createTransaction(std::string name, std::vector<std::unique_ptr<UndoableCommand>> commands) {
    return std::make_unique<TransactionCommand>(std::move(name), std::move(commands), commandDoNotifier, commandDoneNotifier, commandUndoNotifier, commandUndoneNotifier);
}
