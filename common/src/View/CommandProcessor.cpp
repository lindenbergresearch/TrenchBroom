...
struct CommandProcessor::TransactionState {
    ...
    TransactionState(std::string i_name, const TransactionScope i_scope) : name{std::move(i_name)}, scope{i_scope} {
    }
};
...
struct CommandProcessor::SubmitAndStoreResult {
    ...
    SubmitAndStoreResult(std::unique_ptr<CommandResult> i_commandResult, const bool i_commandStored) : commandResult{std::move(i_commandResult)}, commandStored{i_commandStored} {
    }
};
...
public:
    TransactionCommand(
        std::string name, std::vector<std::unique_ptr<UndoableCommand>> commands, Notifier<Command &> &i_commandDoNotifier,
        Notifier<Command &> &i_commandDoneNotifier, Notifier<UndoableCommand &> &i_commandUndoNotifier, Notifier<UndoableCommand &> &i_commandUndoneNotifier) : UndoableCommand(std::move(name), false), m_commands{
                                                                                                                                                                                                             std::move(commands)},
                                                                                                                                                                m_commandDoNotifier{i_commandDoNotifier}, m_commandDoneNotifier{i_commandDoneNotifier}, m_commandUndoNotifier{i_commandUndoNotifier}, m_commandUndoneNotifier{i_commandUndoneNotifier} {
    }
...
CommandProcessor::CommandProcessor(MapDocumentCommandFacade *document, const std::chrono::milliseconds collationInterval) : m_document{document}, m_collationInterval{collationInterval}, m_lastCommandTimestamp{
...
bool CommandProcessor::isCurrentDocumentStateObservable() const {
    return m_transactionStack.size() < 2 || m_transactionStack[m_transactionStack.size() - 2].scope == TransactionScope::LongRunning;
}
