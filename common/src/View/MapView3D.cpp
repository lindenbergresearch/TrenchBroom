...
namespace View {
MapView3D::MapView3D(std::weak_ptr<MapDocument> document, MapViewToolBox &toolBox, Renderer::MapRenderer &renderer, GLContextManager &contextManager, Logger *logger)
...
void MapView3D::initializeToolChain(MapViewToolBox &toolBox) {
    ...
    addTool(std::make_unique<AssembleBrushToolController3D>(toolBox.createComplexBrushTool()));
...
static vm::vec3f computeCameraTargetPosition(const std::vector<Model::Node *> &nodes) {
    ...
    Model::Node::visitAll(nodes, kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [&](auto &&thisLambda, Model::EntityNode *entity) {
...
static float computeCameraOffset(const Renderer::Camera &camera, const std::vector<Model::Node *> &nodes) {
    ...
    const auto handlePoint = [&](const vm::vec3 &point, const vm::plane3f &plane) {
        ...
        const auto newPlane = vm::plane3f(vm::vec3f(point) + 8.0f * plane.normal, plane.normal);
...
void MapView3D::animateCamera(const vm::vec3f &position, const vm::vec3f &direction, const vm::vec3f &up, const float zoom, const int duration) {
...
vm::vec3 MapView3D::doGetMoveDirection(const vm::direction direction) const {
    ...
    case vm::direction::backward:return -doGetMoveDirection(vm::direction::forward);
    case vm::direction::left:return -doGetMoveDirection(vm::direction::right);
    ...
    case vm::direction::up:return vm::vec3::pos_z();
    case vm::direction::down:return vm::vec3::neg_z();
        switchDefault();
    }
}
