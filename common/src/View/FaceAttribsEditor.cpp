...
namespace View {
FaceAttribsEditor::FaceAttribsEditor(std::weak_ptr<MapDocument> document, GLContextManager &contextManager, QWidget *parent) : QWidget{parent}, m_document{std::move(document)}, m_updateControlsSignalDelayer{new SignalDelayer{this}} {
...
std::tuple<QList<int>, QStringList, QStringList> FaceAttribsEditor::getSurfaceFlags() const {
    auto document = kdl::mem_lock(m_document);
    const auto game = document->game();
...
std::tuple<QList<int>, QStringList, QStringList> FaceAttribsEditor::getContentFlags() const {
    auto document = kdl::mem_lock(m_document);
    const auto game = document->game();
