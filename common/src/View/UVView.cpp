...
class UVView::RenderTexture : public Renderer::DirectRenderable {
  ...
  private:
    std::vector<Vertex> getVertices() const {
        ...
        return {
            Vertex(pos1, normal, m_helper.face()->textureCoords(vm::vec3(pos1))), Vertex(pos2, normal, m_helper.face()->textureCoords(vm::vec3(pos2))), Vertex(pos3, normal, m_helper.face()->textureCoords(vm::vec3(pos3))), Vertex(pos4, normal, m_helper.face()->textureCoords(vm::vec3(pos4)))
        };
    }
...
void UVView::renderTextureAxes(Renderer::RenderContext &, Renderer::RenderBatch &renderBatch) {
    ...
    Renderer::DirectEdgeRenderer edgeRenderer(Renderer::VertexArray::move(std::vector<Vertex>({
                                                                                                  Vertex(center, pref(Preferences::XAxisColor)), Vertex(center + length * xAxis, pref(Preferences::XAxisColor)), Vertex(center, pref(Preferences::YAxisColor)), Vertex(center + length * yAxis, pref(Preferences::YAxisColor)),
