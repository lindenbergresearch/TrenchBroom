...
GamesPreferencePane::GamesPreferencePane(MapDocument *document, QWidget *parent)
    : PreferencePane(parent), m_document{document}, m_gameListBox(nullptr), m_stackedWidget(nullptr), m_defaultPage(nullptr), m_currentGamePage(nullptr) {
    createGui();
    updateControls();
...
GamePreferencePane::GamePreferencePane(const std::string &gameName, QWidget *parent)
    : QWidget(parent), m_gameName(gameName), m_gamePathText(nullptr), m_chooseGamePathButton(nullptr) {
    createGui();
}
...
void GamePreferencePane::createGui() {
    ...
    for (auto &tool : gameConfig.compilationTools) {
        ...
        connect(browseButton, &QPushButton::clicked, this, [=]() {
            const QString pathStr = QFileDialog::getOpenFileName(this, tr("%1 Path").arg(QString::fromStdString(toolName)), fileDialogDefaultDirectory(FileDialogDir::CompileTool));
            if (pathStr.isEmpty()) {
                return;
            }
