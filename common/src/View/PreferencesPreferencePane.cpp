...
PreferencesPreferencePane::PreferencesPreferencePane(QWidget *parent) : PreferencePane(parent), m_table(nullptr), m_model(nullptr), m_proxy(nullptr) {
    ...
    m_table->horizontalHeader()->setSectionResizeMode(PreferenceModel::Columns::Index, QHeaderView::ResizeMode::ResizeToContents);
    m_table->horizontalHeader()->setSectionResizeMode(PreferenceModel::Columns::Context, QHeaderView::ResizeMode::ResizeToContents);
    m_table->horizontalHeader()->setSectionResizeMode(PreferenceModel::Columns::Path, QHeaderView::ResizeMode::ResizeToContents);
    m_table->horizontalHeader()->setSectionResizeMode(PreferenceModel::Columns::Type, QHeaderView::ResizeMode::Stretch);
    m_table->horizontalHeader()->setSectionResizeMode(PreferenceModel::Columns::Default, QHeaderView::ResizeMode::Stretch);
    m_table->horizontalHeader()->setSectionResizeMode(PreferenceModel::Columns::Value, QHeaderView::ResizeMode::Stretch);

    m_table->verticalHeader()->setDefaultSectionSize(m_table->fontMetrics().lineSpacing() + LayoutConstants::MediumHMargin);
    ...
    connect(m_table->selectionModel(), &QItemSelectionModel::selectionChanged, this, [&]() {
        ...
        if (selectionModel->hasSelection()) {
            ...
            restoreDefaultButton->setEnabled(m_table->selectionModel()->hasSelection() && !preference->isDefault());
        } else {
    ...
    connect(restoreDefaultButton, &QPushButton::clicked, this, [&]() {
        ...
        if (selectionModel->hasSelection()) {
            ...
            reply = QMessageBox::question(this, "Advanced Preferences", "Reset selected preference to default value?", QMessageBox::Yes | QMessageBox::No);
...
QHBoxLayout *PreferencesPreferencePane::getControlButtonLayout() {
    ...
    buttonLayout->setContentsMargins(0, LayoutConstants::WideHMargin, 0, LayoutConstants::WideHMargin);
    ...
    setValueButton->setContentsMargins(LayoutConstants::NarrowHMargin, LayoutConstants::NarrowHMargin, LayoutConstants::NarrowHMargin, LayoutConstants::NarrowHMargin);
...
void PreferencesPreferencePane::updateLabel() {
    infoLabel->setText(tr("Number of preferences found: ") + QString{}.sprintf("%d", m_proxy->rowCount(m_proxy->index(0, 0))));
}
} // namespace TrenchBroom::View
