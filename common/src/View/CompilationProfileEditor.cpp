...
namespace View {
CompilationProfileEditor::CompilationProfileEditor(std::weak_ptr<MapDocument> document, QWidget *parent) : QWidget{
    parent
...
Variables are allowed.)"
    ...
    connect(m_taskList, &CompilationTaskListBox::taskContextMenuRequested, this, [&](const QPoint &globalPos, const Model::CompilationTask &task) {
          ...
          moveDownAction->setEnabled(static_cast<size_t>(index + 1) < m_profile->tasks.size());
...
void CompilationProfileEditor::addTask() {
    ...
    if (task) {
        ...
        if (index < 0) {
            ...
            m_profile->tasks.insert(std::next(m_profile->tasks.begin(), index + 1), std::move(*task));
            m_taskList->reloadTasks();
...
void CompilationProfileEditor::refresh() {
    ...
    m_moveTaskDownButton->setEnabled(m_profile && m_taskList->currentRow() >= 0 && m_taskList->currentRow() < static_cast<int>(m_profile->tasks.size()) - 1);
}
} // namespace View
} // namespace TrenchBroom
