...
namespace View {
Lasso::Lasso(const Renderer::Camera &camera, const FloatType distance, const vm::vec3 &point) : m_camera{
    camera
}, m_distance{distance}, m_start{point}, m_cur{point} {
}
...
void Lasso::render(Renderer::RenderContext &renderContext, Renderer::RenderBatch &renderBatch) const {
    ...
    const auto polygon = std::vector<vm::vec3f>{
        vm::vec3f{inverseTransform * vm::vec3{box.min.x(), box.min.y(), 0.0}}, vm::vec3f{inverseTransform * vm::vec3{box.min.x(), box.max.y(), 0.0}},
        vm::vec3f{inverseTransform * vm::vec3{box.max.x(), box.max.y(), 0.0}}, vm::vec3f{inverseTransform * vm::vec3{box.max.x(), box.min.y(), 0.0}},
    };
...
vm::mat4x4 Lasso::getTransform() const {
    return vm::mat4x4{
        vm::coordinate_system_matrix(m_camera.right(), m_camera.up(), -m_camera.direction(), m_camera.defaultPoint(static_cast<float>(m_distance)))
    };
}
