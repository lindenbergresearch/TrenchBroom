...
void Lasso::render(
    ...
    const auto polygon = std::vector<vm::vec3f>{vm::vec3f{inverseTransform * vm::vec3{box.min.x(), box.min.y(), 0.0}}, vm::vec3f{inverseTransform * vm::vec3{box.min.x(), box.max.y(), 0.0}},
...
vm::mat4x4 Lasso::getTransform() const {
    return vm::mat4x4{vm::coordinate_system_matrix(
        m_camera.right(), m_camera.up(), -m_camera.direction(), m_camera.defaultPoint(static_cast<float>(m_distance)))};
}
