...
void Lasso::render(Renderer::RenderContext &renderContext, Renderer::RenderBatch &renderBatch) const {
    ...
    const auto polygon = std::vector<vm::vec3f>{
        vm::vec3f{inverseTransform * vm::vec3{box.min.x(), box.min.y(), 0.0}}, vm::vec3f{inverseTransform * vm::vec3{box.min.x(), box.max.y(), 0.0}}, vm::vec3f{inverseTransform * vm::vec3{box.max.x(), box.max.y(), 0.0}}, vm::vec3f{inverseTransform * vm::vec3{box.max.x(), box.min.y(), 0.0}},
    };
...
vm::plane3 Lasso::getPlane() const {
    return vm::plane3{
        vm::vec3{m_camera.defaultPoint(static_cast<float>(m_distance))}, vm::vec3{m_camera.direction()}};
}
