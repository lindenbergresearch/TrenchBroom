...
TextureCollectionEditor::TextureCollectionEditor(std::weak_ptr<MapDocument> document, QWidget *parent) : QWidget{parent}, m_document{std::move(document)} {
    createGui();
    connectObservers();
...
void TextureCollectionEditor::addSelectedTextureCollections() {
...
void TextureCollectionEditor::removeSelectedTextureCollections() {
    auto enabledCollections = enabledTextureCollections();
...
void TextureCollectionEditor::reloadTextureCollections() {
    auto document = kdl::mem_lock(m_document);
    document->reloadTextureCollections();
}

void TextureCollectionEditor::availableTextureCollectionSelectionChanged() {
    updateButtons();
}

void TextureCollectionEditor::enabledTextureCollectionSelectionChanged() {
    updateButtons();
}

bool TextureCollectionEditor::canAddTextureCollections() const {
    return !m_availableCollectionsList->selectedItems().empty();
}

bool TextureCollectionEditor::canRemoveTextureCollections() const {
    return !m_enabledCollectionsList->selectedItems().empty();
}

bool TextureCollectionEditor::canReloadTextureCollections() const {
...
void TextureCollectionEditor::createGui() {
    ...
    auto *toolBar = createMiniToolBarLayout(m_addCollectionsButton, m_removeCollectionsButton, LayoutConstants::WideHMargin, m_reloadCollectionsButton);
    ...
    connect(m_availableCollectionsList, &QListWidget::itemSelectionChanged, this, &TextureCollectionEditor::availableTextureCollectionSelectionChanged);
    connect(m_enabledCollectionsList, &QListWidget::itemSelectionChanged, this, &TextureCollectionEditor::enabledTextureCollectionSelectionChanged);
    connect(m_availableCollectionsList, &QListWidget::itemDoubleClicked, this, &TextureCollectionEditor::addSelectedTextureCollections);
    connect(m_enabledCollectionsList, &QListWidget::itemDoubleClicked, this, &TextureCollectionEditor::removeSelectedTextureCollections);
    connect(m_addCollectionsButton, &QAbstractButton::clicked, this, &TextureCollectionEditor::addSelectedTextureCollections);
    connect(m_removeCollectionsButton, &QAbstractButton::clicked, this, &TextureCollectionEditor::removeSelectedTextureCollections);
    connect(m_reloadCollectionsButton, &QAbstractButton::clicked, this, &TextureCollectionEditor::reloadTextureCollections);
}

void TextureCollectionEditor::updateButtons() {
...
void TextureCollectionEditor::connectObservers() {
    auto document = kdl::mem_lock(m_document);
    m_notifierConnection += document->documentWasNewedNotifier.connect(this, &TextureCollectionEditor::documentWasNewedOrLoaded);
    m_notifierConnection += document->nodesDidChangeNotifier.connect(this, &TextureCollectionEditor::nodesDidChange);
    m_notifierConnection += document->documentWasLoadedNotifier.connect(this, &TextureCollectionEditor::documentWasNewedOrLoaded);
    m_notifierConnection += document->textureCollectionsDidChangeNotifier.connect(this, &TextureCollectionEditor::textureCollectionsDidChange);
    m_notifierConnection += document->modsDidChangeNotifier.connect(this, &TextureCollectionEditor::modsDidChange);

    auto &prefs = PreferenceManager::instance();
    m_notifierConnection += prefs.preferenceDidChangeNotifier.connect(this, &TextureCollectionEditor::preferenceDidChange);
}
...
void TextureCollectionEditor::nodesDidChange(const std::vector<Model::Node *> &nodes) {
    auto document = kdl::mem_lock(m_document);
...
void TextureCollectionEditor::modsDidChange() {
    updateAllTextureCollections();
    updateButtons();
}

void TextureCollectionEditor::preferenceDidChange(const std::filesystem::path &path) {
    auto document = kdl::mem_lock(m_document);
...
void TextureCollectionEditor::updateAllTextureCollections() {
...
void TextureCollectionEditor::updateListBox(QListWidget *box, const std::vector<std::filesystem::path> &paths) {
...
std::vector<std::filesystem::path> TextureCollectionEditor::availableTextureCollections() const {
    auto document = kdl::mem_lock(m_document);
    return document->disabledTextureCollections();
}

std::vector<std::filesystem::path> TextureCollectionEditor::enabledTextureCollections() const {
    auto document = kdl::mem_lock(m_document);
