...
void TextureBrowserView::addTextureToLayout(Layout &layout, const Assets::Texture *texture, const Renderer::FontDescriptor &font) {
  ...
  layout.addItem(
      texture, textureName, scaledTextureWidth, scaledTextureHeight, maxCellWidth, titleHeight + 4.0f
  );
}
...
std::vector<const Assets::Texture *> TextureBrowserView::filterTextures(std::vector<const Assets::Texture *> textures) const {
  if (m_hideUnused) {
    textures = kdl::vec_erase_if(
        std::move(textures), [](const auto *texture) {
...
std::vector<const Assets::Texture *> TextureBrowserView::sortTextures(std::vector<const Assets::Texture *> textures) const {
  ...
  case TextureSortOrder::Usage:
    return kdl::vec_sort(
        std::move(textures), [&](const auto *lhs, const auto *rhs) {
...
void TextureBrowserView::renderBounds(Layout &layout, const float y, const float height) {
  ...
  auto shader = Renderer::ActiveShader{shaderManager(), Renderer::Shaders::TextureBrowserBorderShader};
      ...
      for (const auto &row : group.rows()) {
        if (row.intersectsY(y, height)) {
          for (const auto &cell : row.cells()) {
            ...
            auto vertexArray = Renderer::VertexArray::move(
                std::vector<TextureVertex>{
                    TextureVertex{{bounds.left(), height - (bounds.top() - y)}, {0, 0}}, TextureVertex{{bounds.left(), height - (bounds.bottom() - y)}, {0, 1}},
...
void TextureBrowserView::doContextMenu(Layout &layout, float x, float y, QContextMenuEvent *event) {
  if (const auto *cell = layout.cellAt(x, y)) {
    ...
    if (! cellData(*cell).overridden()) {
      ...
      menu.addAction(
          tr("Select Faces"), this, [&, texture = &texture]() {
