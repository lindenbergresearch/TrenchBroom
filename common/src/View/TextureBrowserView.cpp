...
namespace View {
TextureBrowserView::TextureBrowserView(QScrollBar *scrollBar, GLContextManager &contextManager, std::weak_ptr<MapDocument> document) : CellView(contextManager,
    scrollBar
), m_document(document), m_group(false), m_hideUnused(false), m_sortOrder(TextureSortOrder::Name), m_selectedTexture(nullptr) {
...
void
TextureBrowserView::addTextureToLayout(Layout &layout, const Assets::Texture *texture, const std::string &groupName, const Renderer::FontDescriptor &font) {
    ...
    auto cellData = TextureCellData{
        texture, textureName, groupName, vm::vec2f((maxCellWidth - textureNameSize.x()) / 2.0f, defaultTextHeight + 3.0f),
        vm::vec2f((maxCellWidth - groupNameSize.x()) / 2.0f, 1.0f), textureFont, groupFont
    };

    layout.addItem(std::move(cellData), scaledTextureWidth, scaledTextureHeight, maxCellWidth, totalSize.y());
}
...
TextureBrowserView::StringMap TextureBrowserView::collectStringVertices(Layout &layout, const float y, const float height) {
    Renderer::FontDescriptor defaultDescriptor(pref(Preferences::RendererFontPath), static_cast<size_t>(pref(Preferences::BrowserFontSize)));
    ...
    for (const auto &group: layout.groups()) {
        if (group.intersectsY(y, height)) {
            ...
            if (!title.empty()) {
                ...
                const auto titleVertices = TextVertex::toList(quads.size() / 2, kdl::skip_iterator(std::begin(quads), std::end(quads), 0, 2),
                    kdl::skip_iterator(std::begin(quads), std::end(quads), 1, 2), kdl::skip_iterator(std::begin(textColor), std::end(textColor), 0, 0));
            ...
            for (const auto &row: group.rows()) {
                if (row.intersectsY(y, height)) {
                    for (const auto &cell: row.cells()) {
                        ...
                        const auto textureNameVertices = TextVertex::toList(textureNameQuads.size() / 2,
                        ...
                        const auto groupNameVertices = TextVertex::toList(groupNameQuads.size() / 2,
