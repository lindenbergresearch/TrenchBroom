...
CompilationTaskEditorBase::CompilationTaskEditorBase(QString title, std::weak_ptr<MapDocument> document, Model::CompilationProfile &profile, Model::CompilationTask &task, QWidget *parent)
    : ControlListBoxItemRenderer{parent}, m_title{std::move(title)}, m_document{std::move(document)}, m_profile{profile}, m_task{task} {
    ...
    auto layout = new QVBoxLayout;
    layout->setContentsMargins(LayoutConstants::NoMargin, LayoutConstants::NarrowVMargin, LayoutConstants::NarrowHMargin, LayoutConstants::NarrowVMargin);
    ...
    titleLabel->setContentsMargins(LayoutConstants::MediumHMargin, LayoutConstants::NarrowVMargin, LayoutConstants::NoMargin, LayoutConstants::NarrowVMargin);
...
void CompilationTaskEditorBase::updateCompleter(QCompleter *completer) {
    ...
    try {
        workDir = EL::interpolate(m_profile.workDirSpec, EL::EvaluationContext{CompilationWorkDirVariables{kdl::mem_lock(m_document)}});
...
CompilationExportMapTaskEditor::CompilationExportMapTaskEditor(std::weak_ptr<MapDocument> document, Model::CompilationProfile &profile, Model::CompilationTask &task, QWidget *parent)
...
CompilationCopyFilesTaskEditor::CompilationCopyFilesTaskEditor(std::weak_ptr<MapDocument> document, Model::CompilationProfile &profile, Model::CompilationTask &task, QWidget *parent)
...
CompilationRenameFileTaskEditor::CompilationRenameFileTaskEditor(std::weak_ptr<MapDocument> document, Model::CompilationProfile &profile, Model::CompilationTask &task, QWidget *parent)
...
Variables are allowed.)");
    ...
    connect(m_sourceEditor, &QLineEdit::textChanged, this, &CompilationRenameFileTaskEditor::sourceSpecChanged);
    connect(m_targetEditor, &QLineEdit::textChanged, this, &CompilationRenameFileTaskEditor::targetSpecChanged);
}
...
CompilationDeleteFilesTaskEditor::CompilationDeleteFilesTaskEditor(std::weak_ptr<MapDocument> document, Model::CompilationProfile &profile, Model::CompilationTask &task, QWidget *parent)
...
Variables are allowed.)");
    ...
    connect(m_targetEditor, &QLineEdit::textChanged, this, &CompilationDeleteFilesTaskEditor::targetSpecChanged);
}
...
CompilationRunToolTaskEditor::CompilationRunToolTaskEditor(std::weak_ptr<MapDocument> document, Model::CompilationProfile &profile, Model::CompilationTask &task, QWidget *parent)
    ...
    m_toolEditor->setToolTip(R"(The absolute path to the executable of the tool that should be run.
...
Variables are allowed.)");
    ...
    m_parametersEditor->setToolTip(R"(The parameters that should be passed to the tool when it is executed.
Variables are allowed.)");
    ...
    connect(m_treatNonZeroResultCodeAsError, &QCheckBox::stateChanged, this, &CompilationRunToolTaskEditor::treatNonZeroResultCodeAsErrorChanged);
}
...
void CompilationRunToolTaskEditor::updateItem() {
    ...
    if (m_treatNonZeroResultCodeAsError->isChecked() != task().treatNonZeroResultCodeAsError) {
        m_treatNonZeroResultCodeAsError->setCheckState(task().treatNonZeroResultCodeAsError ? Qt::CheckState::Checked : Qt::CheckState::Unchecked);
    }
}
...
CompilationTaskListBox::CompilationTaskListBox(std::weak_ptr<MapDocument> document, QWidget *parent)
    : ControlListBox{"Click the '+' button to create a task.", QMargins{}, false, parent}, m_document{std::move(document)} {
}
...
ControlListBoxItemRenderer *CompilationTaskListBox::createItemRenderer(QWidget *parent, const size_t index) {
    ...
    auto *renderer = std::visit(kdl::overload([&](const Model::CompilationExportMap &) -> ControlListBoxItemRenderer * {
    ...
    connect(renderer, &QWidget::customContextMenuRequested, this, [=](const QPoint &pos) {
        emit this->taskContextMenuRequested(renderer->mapToGlobal(pos), m_profile->tasks[index]);
    });
