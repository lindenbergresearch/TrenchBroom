...
GLContextManager::GLContextManager() : m_initialized(false), m_shaderManager(std::make_unique<Renderer::ShaderManager>()),
    m_vboManager(std::make_unique<Renderer::VboManager>(m_shaderManager.get())), m_fontManager(std::make_unique<Renderer::FontManager>()) {
}
...
bool GLContextManager::initialize() {
    ...
    if (!m_initialized) {
        ...
        kdl::fold_results(kdl::vec_transform(std::vector<Renderer::ShaderConfig>{
                Grid2DShader, VaryingPCShader, VaryingPUniformCShader, MiniMapEdgeShader, EntityModelShader, FaceShader, PatchShader, EdgeShader, ColoredTextShader,
                TextBackgroundShader, TextureBrowserShader, TextureBrowserBorderShader, HandleShader, ColoredHandleShader, CompassShader, CompassOutlineShader,
