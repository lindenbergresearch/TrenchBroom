...
namespace View {
BrushVertexCommandBase::BrushVertexCommandBase(const std::string &name, std::vector<std::pair<Model::Node *, Model::NodeContents>> nodes) : SwapNodeContentsCommand{name, std::move(nodes)} {
}
...
BrushVertexCommandResult::BrushVertexCommandResult(const bool success, const bool hasRemainingVertices) : CommandResult{success}, m_hasRemainingVertices{hasRemainingVertices} {
}
...
BrushVertexCommand::BrushVertexCommand(
    ...
    std::vector<vm::vec3> newVertexPositions) : BrushVertexCommandBase{
                                                    name, std::move(nodes)},
                                                m_oldVertexPositions{std::move(oldVertexPositions)}, m_newVertexPositions{std::move(newVertexPositions)} {
}
...
BrushEdgeCommand::BrushEdgeCommand(
    ...
    std::vector<vm::segment3> newEdgePositions) : BrushVertexCommandBase{
                                                      name, std::move(nodes)},
                                                  m_oldEdgePositions{std::move(oldEdgePositions)}, m_newEdgePositions{std::move(newEdgePositions)} {
}
...
BrushFaceCommand::BrushFaceCommand(
    ...
    std::vector<vm::polygon3> newFacePositions) : BrushVertexCommandBase{
                                                      name, std::move(nodes)},
                                                  m_oldFacePositions{std::move(oldFacePositions)}, m_newFacePositions{std::move(newFacePositions)} {
}
