...
void CellView::onScrollBarActionTriggered(int action) {
    ...
    switch (action) {
    case QAbstractSlider::SliderSingleStepAdd:m_scrollBar->setSliderPosition(int(m_layout.rowPosition(top, 1))); // line down
        break;
    case QAbstractSlider::SliderSingleStepSub:m_scrollBar->setSliderPosition(int(m_layout.rowPosition(top, -1))); // line up
        break;
    case QAbstractSlider::SliderPageStepAdd: m_scrollBar->setSliderPosition(int(m_layout.rowPosition(top + height, 0))); // page down
        break;
    case QAbstractSlider::SliderPageStepSub:m_scrollBar->setSliderPosition(int(m_layout.rowPosition(top - height, 0))); // page up
        break;
    default:break;
    }
}
    ...
    for (const auto &group : layout.groups()) {
        if (group.intersectsY(y, height)) {
            ...
            if (!groupTitle.empty()) {
                ...
                const auto titleVertices = TextVertex::toList(quads.size() / 2, kdl::skip_iterator{std::begin(quads), std::end(quads), 0, 2}, kdl::skip_iterator{std::begin(quads), std::end(quads), 1, 2}, kdl::skip_iterator{std::begin(textColor), std::end(textColor), 0, 0});
            ...
            for (const auto &row : group.rows()) {
                if (row.intersectsY(y, height)) {
                    for (const auto &cell : row.cells()) {
                        ...
                        const auto textureNameFont = fontManager.selectFontSize(defaultFont, cellTitle, textureNameBounds.width, 6);
                        ...
                        const auto textureNameVertices = TextVertex::toList(cellTitleQuads.size() / 2, kdl::skip_iterator{
