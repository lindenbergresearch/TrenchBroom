...
SelectionCommand::SelectionCommand(const Action action, std::vector<Model::Node *> nodes, std::vector<Model::BrushFaceHandle> faces) : UndoableCommand{makeName(action, nodes.size(), faces.size()), false}, m_action{action}, m_nodes{
                                                                                                                                                                                                                                   std::move(nodes)},
    ...
    switch (m_action) {
        ...
        case Action::SelectFaces:
            return std::make_unique<CommandResult>(
                Model::resolveAllRefs(m_faceRefs).transform([&](const auto &faceHandles) { document->performSelect(faceHandles); }).transform_error([&](const auto &e) { document->error() << e.msg; }).is_success());
        case Action::SelectAllNodes:
        ...
        case Action::DeselectFaces:
            return std::make_unique<CommandResult>(
                Model::resolveAllRefs(m_faceRefs).transform([&](const auto &faceHandles) { document->performDeselect(faceHandles); }).transform_error([&](const auto &e) { document->error() << e.msg; }).is_success());
        case Action::DeselectAll:
...
std::unique_ptr<CommandResult> SelectionCommand::doPerformUndo(MapDocumentCommandFacade *document) {
    ...
    if (!m_previouslySelectedFaceRefs.empty()) {
        return std::make_unique<CommandResult>(
            Model::resolveAllRefs(m_previouslySelectedFaceRefs).transform([&](const auto &faceHandles) { document->performSelect(faceHandles); }).transform_error([&](const auto &e) { document->error() << e.msg; }).is_success());
    }
    return std::make_unique<CommandResult>(true);
}
