...
class RotateObjectsDragDelegate : public HandleDragTrackerDelegate {
  ...
  public:
    ...
    HandlePositionProposer start(const InputState &inputState, const vm::vec3 & /* initialHandlePosition */, const vm::vec3 &handleOffset) override {
        ...
        return makeHandlePositionProposer(makeCircleHandlePicker(center, axis, radius, handleOffset), makeCircleHandleSnapper(m_tool.grid(), m_tool.angle(), center, axis, radius));
    }
...
class RotateObjectsBase : public ToolController {
  ...
  private:
    ...
    std::unique_ptr<DragTracker> acceptMouseDrag(const InputState &inputState) override {
        ...
        return createHandleDragTracker(RotateObjectsDragDelegate{m_tool, area, std::move(renderHighlight)}, inputState, initialHandlePosition, initialHandlePosition);
    }
...
class MoveCenterBase : public ToolController {
  ...
  protected:
    ...
    std::unique_ptr<DragTracker> acceptMouseDrag(const InputState &inputState) override {
        ...
        if (!inputState.mouseButtonsPressed(MouseButtons::MBLeft) || !inputState.checkModifierKeys(ModifierKeyPressed::MK_No, ModifierKeyPressed::MK_DontCare, ModifierKeyPressed::MK_No)) {
            return nullptr;
        }
        ...
        return createMoveHandleDragTracker(MoveRotationCenterDragDelegate{m_tool, std::move(renderHighlight)}, inputState, m_tool.rotationCenter(), hit.hitPoint());
    }
