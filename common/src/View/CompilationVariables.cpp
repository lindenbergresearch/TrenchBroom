...
CommonVariables::CommonVariables(std::shared_ptr<MapDocument> document) {
    ...
    for (const auto &tool: document->game()->compilationTools()) {
        const auto toolPath = factory.compilationToolPath(document->game()->gameName(), tool.name);
...
CommonCompilationVariables::CommonCompilationVariables(std::shared_ptr<MapDocument> document) : CommonVariables{document} {
    const auto filename = document->path().filename();
...
CompilationVariables::CompilationVariables(std::shared_ptr<MapDocument> document, const std::string &workDir) : CommonCompilationVariables{std::move(document)} {
...
LaunchGameEngineVariables::LaunchGameEngineVariables(std::shared_ptr<MapDocument> document) : CommonVariables{std::move(document)} {
}
}// namespace View
}// namespace TrenchBroom
