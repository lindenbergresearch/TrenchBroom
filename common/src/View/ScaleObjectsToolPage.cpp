...
ScaleObjectsToolPage::ScaleObjectsToolPage(std::weak_ptr<MapDocument> document, QWidget *parent) :
    QWidget(parent), m_document(std::move(document)), m_book(nullptr), m_sizeTextBox(nullptr), m_factorsTextBox(nullptr), m_scaleFactorsOrSize(nullptr),
    m_button(nullptr) {
  createGui();
  connectObservers();
  updateGui();
...
std::optional<vm::vec3> ScaleObjectsToolPage::getScaleFactors() const {
  ...
  default:return vm::parse<FloatType, 3>(m_factorsTextBox->text().toStdString());
  }
}
