...
SetBrushFaceAttributesTool::SetBrushFaceAttributesTool(std::weak_ptr<MapDocument> document) : ToolController{}, Tool{true}, m_document{std::move(document)} {
}
 ...
 */
std::optional<Model::BrushFaceHandle> selectTargetFaceHandleForLinkedGroups(Model::GroupNode &containingSourceGroupNode, const Model::BrushFaceHandle &sourceFaceHandle, const Model::BrushFaceHandle &oldTargetFaceHandle) {
    ...
    const auto linkedTargetBrushNodesInSourceGroup = Model::collectLinkedNodes({&containingSourceGroupNode}, oldTargetBrushNode);

    if (linkedTargetBrushNodesInSourceGroup.empty()) {
        return oldTargetFaceHandle;
    }

    auto *newTargetBrushNode = dynamic_cast<Model::BrushNode *>(linkedTargetBrushNodesInSourceGroup.front());
    ...
    ensure(oldTargetContainingGroupNode->linkId() == newTargetContainingGroupNode->linkId(), "containing groups are linked");

    const auto [invertible, oldTargetTransformation] = vm::invert(oldTargetContainingGroupNode->group().transformation());
    assert(invertible);
    unused(invertible);

    const auto newTargetTransformation = newTargetContainingGroupNode->group().transformation();
    const auto oldToNewTargetTransformation = newTargetTransformation * oldTargetTransformation;
    ...
    const auto newTargetNormal = vm::strip_translation(oldToNewTargetTransformation) * oldTargetNormal;

    const auto newTargetFaceIndex = findClosestFace(newTargetBrushNode->brush(), newTargetNormal);

    // Can't apply to the same face
    if (newTargetBrushNode != &sourceBrushNode || sourceFaceHandle.faceIndex() != newTargetFaceIndex) {
...
auto selectTargetFaceHandlesForLinkedGroups(const Model::BrushFaceHandle &sourceFaceHandle, const std::vector<Model::BrushFaceHandle> &targetFaceHandles) {
    ...
    for (const auto &targetFaceHandle : targetFaceHandles) {
        if (auto newTargetFaceHandle = selectTargetFaceHandleForLinkedGroups(*containingGroupNode, sourceFaceHandle, targetFaceHandle)) {
...
void transferFaceAttributes(MapDocument &document, const InputState &inputState, const Model::BrushFaceHandle &sourceFaceHandle, const std::vector<Model::BrushFaceHandle> &targetFaceHandles, const Model::BrushFaceHandle &faceToSelectAfter) {
    const auto targetFaceHandlesForLinkedGroups = selectTargetFaceHandlesForLinkedGroups(sourceFaceHandle, targetFaceHandles);
