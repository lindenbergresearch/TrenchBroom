...
EntityBrowserView::EntityBrowserView(QScrollBar *scrollBar, GLContextManager &contextManager, Assets::EntityDefinitionManager &entityDefinitionManager, Assets::EntityModelManager &entityModelManager, Logger &logger)
    : CellView{contextManager, scrollBar}, m_entityDefinitionManager{entityDefinitionManager}, m_entityModelManager{entityModelManager}, m_logger{logger}, m_sortOrder{Assets::EntityDefinitionSortOrder::Name} {
...
namespace {
bool matchesFilterText(const Assets::PointEntityDefinition &definition, const std::string &filterText) {
    return filterText.empty() || kdl::all_of(kdl::str_split(filterText, " "), [&](const auto &pattern) {
...
void EntityBrowserView::addEntityToLayout(Layout &layout, const Assets::PointEntityDefinition *definition, const Renderer::FontDescriptor &font) {
    if ((!m_hideUnused || definition->usageCount() > 0) && matchesFilterText(*definition, m_filterText)) {
        ...
        }, definition->name(), boundsSize.y(), boundsSize.z(), actualSize.x(), static_cast<float>(font.size()) + 2.0f);
    }
}
                ...
                if (row.intersectsY(y, height)) {
                    for (const auto &cell : row.cells()) {
                        ...
                        if (modelRenderer == nullptr) {
                            ...
                            vm::bbox3f{definition->bounds()}.for_each_edge([&](const vm::vec3f &v1, const vm::vec3f &v2) {
...
vm::mat4x4f EntityBrowserView::itemTransformation(const Cell &cell, const float y, const float height, const bool applyModelScale) const {
    ...
    return vm::translation_matrix(offset) * vm::scaling_matrix(vm::vec3f::fill(scaling)) * vm::translation_matrix(rotationOffset) * vm::translation_matrix(scaledBoundsCenter) * vm::rotation_matrix(m_rotation) * scalingMatrix * vm::translation_matrix(-boundsCenter);
}
