...
void EntityDefinitionFileChooser::bindEvents() {
    ...
    connect(m_browseExternal, &QAbstractButton::clicked, this, &EntityDefinitionFileChooser::chooseExternalClicked);
...
void EntityDefinitionFileChooser::chooseExternalClicked() {
    const auto fileName = QFileDialog::getOpenFileName(nullptr, tr("Load Entity Definition File"), fileDialogDefaultDirectory(FileDialogDir::EntityDefinition), "All supported entity definition files (*.fgd *.def *.ent);;"
