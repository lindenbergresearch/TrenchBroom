...
QWidget *FaceInspector::createTextureBrowserInfo() {
    auto *label = new QLabel{
        tr(R"(To manage wad files, select the "wad" property of the worldspawn entity to reveal a wad file manager below the entity property table.)")
    };
    ...
    labelLayout->setContentsMargins(LayoutConstants::WideHMargin, LayoutConstants::WideVMargin, LayoutConstants::WideHMargin, LayoutConstants::WideVMargin);
...
void FaceInspector::connectObservers() {
    auto document = kdl::mem_lock(m_document);
    m_notifierConnection += document->documentWasNewedNotifier.connect(this, &FaceInspector::documentWasNewedOrOpened);
    m_notifierConnection += document->documentWasLoadedNotifier.connect(this, &FaceInspector::documentWasNewedOrOpened);
}
