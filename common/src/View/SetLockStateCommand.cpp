...
static bool shouldUpdateModificationCount(const std::vector<Model::Node *> &nodes) {
    for (const auto *node : nodes) {
        const auto modifiesLayer = node->accept(kdl::overload([](const Model::WorldNode *) { return false; }, [](const Model::LayerNode *) { return true; }, [](const Model::GroupNode *) { return false; }, [](const Model::EntityNode *) { return false; }, [](const Model::BrushNode *) { return false; }, [](const Model::PatchNode *) { return false; }));
        if (modifiesLayer) {
            return true;
        }
    }
...
SetLockStateCommand::SetLockStateCommand(const std::vector<Model::Node *> &nodes, const Model::LockState lockState)
    : UndoableCommand(makeName(lockState), shouldUpdateModificationCount(nodes)), m_nodes(nodes), m_lockState(lockState) {
}

std::string SetLockStateCommand::makeName(const Model::LockState state) {
    switch (state) {
    case Model::LockState::Inherited:return "Reset Locking";
    case Model::LockState::Locked:return "Lock Objects";
    case Model::LockState::Unlocked:return "Unlock Objects";
        switchDefault();
    }
}
