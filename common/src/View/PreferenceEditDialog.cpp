...
PreferenceEditDialog::PreferenceEditDialog(PreferenceBase *preference, QWidget *parent, const Qt::WindowFlags &f) : QDialog(parent, f), m_preference(preference) {
    createGui();
}
        ...
        connect(
            browseButton, &QPushButton::clicked, this, [=]() {
                const QString pathStr = QFileDialog::getOpenFileName(
                    this, tr("%1 Path").arg(QString::fromStdString(preference->defaultValue().string().c_str())), fileDialogDefaultDirectory(FileDialogDir::Resources));
