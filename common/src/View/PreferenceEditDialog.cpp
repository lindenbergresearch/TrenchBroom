...
void PreferenceEditDialog::createGui() {
    ...
    setWindowTitle("Advanced Preference Editor" + QString::fromStdString(m_preference->path().root_name()));
...
void PreferenceEditDialog::loadPreference(QVBoxLayout *layout) {
    ...
    if (IS_TYPE(std::filesystem::path, preference, m_preference)) {
        ...
        connect(browseButton, &QPushButton::clicked, this, [=]() {
            const QString pathStr = QFileDialog::getOpenFileName(this, tr("%1 Path").arg(QString::fromStdString(preference->defaultValue().string().c_str())), fileDialogDefaultDirectory(FileDialogDir::Resources));
