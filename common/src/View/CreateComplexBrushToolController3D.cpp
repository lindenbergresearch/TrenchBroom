...
class DrawFaceDragDelegate : public HandleDragTrackerDelegate {
  ...
  public:
    ...
    HandlePositionProposer start(const InputState &, const vm::vec3 &initialHandlePosition, const vm::vec3 &handleOffset) {
        ...
        return makeHandlePositionProposer(makeSurfaceHandlePicker(type(Model::BrushNode::BrushHitType), handleOffset), makeIdentityHandleSnapper());
    }
  ...
  private:
    void updatePolyhedron(const vm::vec3 &initialHandlePosition, const vm::vec3 &currentHandlePosition) {
        ...
        const auto newVertices = std::vector<vm::vec3>{
            vm::unswizzle(vm::vec3{topLeft2, swizzledPlane.zAt(topLeft2)}, axis), vm::unswizzle(vm::vec3{topRight2, swizzledPlane.zAt(topRight2)}, axis), vm::unswizzle(vm::vec3{bottomLeft2, swizzledPlane.zAt(bottomLeft2)}, axis), vm::unswizzle(vm::vec3{bottomRight2, swizzledPlane.zAt(bottomRight2)}, axis)
        };
...
class DrawFacePart : public Part, public ToolController {
  ...
  private:
    ...
    std::unique_ptr<DragTracker> acceptMouseDrag(const InputState &inputState) override {
        ...
        return createHandleDragTracker(DrawFaceDragDelegate{m_tool, plane}, inputState, initialHandlePosition, initialHandlePosition);
    }
...
class DuplicateFaceDragDelegate : public HandleDragTrackerDelegate {
  ...
  public:
    ...
    HandlePositionProposer start(const InputState &, const vm::vec3 &initialHandlePosition, const vm::vec3 &handleOffset) {
        ...
        return makeHandlePositionProposer(makeLineHandlePicker(line, handleOffset), makeRelativeLineHandleSnapper(m_tool.grid(), line));
    }
...
class DuplicateFacePart : public Part, public ToolController {
  ...
  private:
    ...
    std::unique_ptr<DragTracker> acceptMouseDrag(const InputState &inputState) override {
        ...
        return createHandleDragTracker(DuplicateFaceDragDelegate{m_tool, normal}, inputState, initialHandlePosition, initialHandlePosition);
    }
...
bool CreateComplexBrushToolController3D::mouseClick(const InputState &inputState) {
    ...
    if (const auto faceHandle = Model::hitToFaceHandle(hit)) {
        ...
        m_tool.update(Model::Polyhedron3{
            kdl::vec_concat(std::vector<vm::vec3>({snapped}), m_tool.polyhedron().vertexPositions())
        });
