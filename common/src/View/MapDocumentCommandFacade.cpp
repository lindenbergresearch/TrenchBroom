...
void MapDocumentCommandFacade::performSelect(const std::vector<Model::BrushFaceHandle> &faces) {
    ...
    nodeLockingDidChangeNotifier(kdl::vec_static_cast<Model::Node *>(constrained.groupsToLock));
...
void MapDocumentCommandFacade::performDeselect(const std::vector<Model::BrushFaceHandle> &faces) {
    const auto implicitlyLockedGroups = kdl::vector_set{
        kdl::vec_filter(Model::collectGroups({m_world.get()}), [](const auto *groupNode) { return groupNode->lockedByOtherSelection(); })
    };
    ...
    nodeLockingDidChangeNotifier(kdl::vec_static_cast<Model::Node *>(groupsToLock.get_data()));
