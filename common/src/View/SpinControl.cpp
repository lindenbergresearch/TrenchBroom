...
namespace View {
SpinControl::SpinControl(QWidget *parent) : QDoubleSpinBox(parent), m_regularIncrement(1.0), m_shiftIncrement(2.0), m_ctrlIncrement(4.0), m_minDigits(0), m_maxDigits(6) {
...
void SpinControl::updateTooltip() {
    setToolTip(
        tr("Increment: %1 (%2: %3, %4: %5)").arg(QString::fromStdString(kdl::str_to_string(m_regularIncrement))).arg(nativeModifierLabel(Qt::SHIFT)).arg(QString::fromStdString(kdl::str_to_string(m_shiftIncrement))).arg(nativeModifierLabel(Qt::CTRL)).arg(QString::fromStdString(kdl::str_to_string(m_ctrlIncrement))));
}
}// namespace View
}// namespace TrenchBroom
