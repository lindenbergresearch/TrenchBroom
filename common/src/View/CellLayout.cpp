...
LayoutCell::LayoutCell(std::any item, std::string title, const float x, const float y, const float itemWidth, const float itemHeight, const float titleWidth, const float titleHeight, const float titleMargin, const float maxUpScale, const float minWidth, const float maxWidth, const float minHeight, const float maxHeight)
    : m_item{std::move(item)}, m_title{std::move(title)}, m_x{x}, m_y{y}, m_itemWidth{itemWidth}, m_itemHeight{itemHeight}, m_titleWidth{titleWidth}, m_titleHeight{titleHeight}, m_titleMargin{titleMargin} {
...
void LayoutCell::doLayout(const float maxUpScale, const float minWidth, const float maxWidth, const float minHeight, const float maxHeight) {
    ...
    const auto cellHeight = std::max(minHeight, std::max(minHeight, scaledItemHeight) + m_titleHeight + m_titleMargin);
...
LayoutRow::LayoutRow(const float x, const float y, const float cellMargin, const float titleMargin, const float maxWidth, const size_t maxCells, const float maxUpScale, const float minCellWidth, const float maxCellWidth, const float minCellHeight, const float maxCellHeight)
    : m_cellMargin{cellMargin}, m_titleMargin{titleMargin}, m_maxWidth{maxWidth}, m_maxCells{maxCells}, m_maxUpScale{maxUpScale}, m_minCellWidth{minCellWidth}, m_maxCellWidth{maxCellWidth}, m_minCellHeight{minCellHeight}, m_maxCellHeight{maxCellHeight}, m_bounds{x, y, 0.0f, 0.0f} {
}
...
bool LayoutRow::canAddItem(const float itemWidth, const float itemHeight, const float titleWidth, const float titleHeight) const {
    ...
    auto cell = LayoutCell{
        std::any{}, "", x, m_bounds.top(), itemWidth, itemHeight, titleWidth, titleHeight, m_titleMargin, m_maxUpScale, m_minCellWidth, m_maxCellWidth, m_minCellHeight, m_maxCellHeight
    };
...
void LayoutRow::addItem(std::any item, std::string title, const float itemWidth, const float itemHeight, const float titleWidth, const float titleHeight) {
    ...
    auto cell = LayoutCell{
        std::move(item), std::move(title), x, m_bounds.top(), itemWidth, itemHeight, titleWidth, titleHeight, m_titleMargin, m_maxUpScale, m_minCellWidth, m_maxCellWidth, m_minCellHeight, m_maxCellHeight
    };
    width += cell.cellBounds().width;
...
LayoutGroup::LayoutGroup(std::string title, const float x, const float y, const float cellMargin, const float titleMargin, const float rowMargin, const float titleHeight, const float width, const size_t maxCellsPerRow, const float maxUpScale, const float minCellWidth, const float maxCellWidth, const float minCellHeight, const float maxCellHeight)
    : m_title{std::move(title)}, m_cellMargin{cellMargin}, m_titleMargin{titleMargin}, m_rowMargin{rowMargin}, m_maxCellsPerRow{maxCellsPerRow}, m_maxUpScale{maxUpScale}, m_minCellWidth{minCellWidth}, m_maxCellWidth{maxCellWidth}, m_minCellHeight{minCellHeight}, m_maxCellHeight{maxCellHeight}, m_titleBounds{0.0f, y, width + 2.0f * x, titleHeight}, m_contentBounds{x, y + titleHeight + m_rowMargin, width, 0.0f} {
}

LayoutGroup::LayoutGroup(const float x, const float y, const float cellMargin, const float titleMargin, const float rowMargin, const float width, const size_t maxCellsPerRow, const float maxUpScale, const float minCellWidth, const float maxCellWidth, const float minCellHeight, const float maxCellHeight)
    : m_cellMargin{cellMargin}, m_titleMargin{titleMargin}, m_rowMargin{rowMargin}, m_maxCellsPerRow{maxCellsPerRow}, m_maxUpScale{maxUpScale}, m_minCellWidth{minCellWidth}, m_maxCellWidth{maxCellWidth}, m_minCellHeight{minCellHeight}, m_maxCellHeight{maxCellHeight}, m_titleBounds{x, y, width, 0.0f}, m_contentBounds{x, y, width, 0.0f} {
}
...
void LayoutGroup::addItem(std::any item, std::string title, const float itemWidth, const float itemHeight, const float titleWidth, const float titleHeight) {
    ...
    m_rows.back().addItem(std::move(item), std::move(title), itemWidth, itemHeight, titleWidth, titleHeight);
...
void CellLayout::addGroup(std::string title, const float titleHeight) {
    ...
    m_groups.emplace_back(std::move(title), m_outerMargin, y, m_cellMargin, m_titleMargin, m_rowMargin, titleHeight, m_width - 2.0f * m_outerMargin, m_maxCellsPerRow, m_maxUpScale, m_minCellWidth, m_maxCellWidth, m_minCellHeight, m_maxCellHeight);
...
void CellLayout::addItem(std::any item, std::string title, const float itemWidth, const float itemHeight, const float titleWidth, const float titleHeight) {
    ...
    m_groups.back().addItem(std::move(item), std::move(title), itemWidth, itemHeight, titleWidth, titleHeight);
        ...
        for (auto &group : copy) {
            ...
            for (const auto &row : group.rows()) {
                for (const auto &cell : row.cells()) {
                    ...
                    addItem(cell.item(), cell.title(), itemWidth, itemHeight, titleBounds.width, titleBounds.height);
                }
            }
        }
    }
