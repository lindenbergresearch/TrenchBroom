...
UVRotateTool::UVRotateTool(std::weak_ptr<MapDocument> document, UVViewHelper &helper) : ToolController{}, Tool{true}, m_document{document}, m_helper{helper} {
}
...
public:
    Render(const UVViewHelper &helper, const float centerRadius, const float outerRadius, const bool highlight) : m_helper{helper}, m_highlight{highlight}, m_center{makeCircle(helper, centerRadius, 10, true)}, m_outer{
...
public:
    UVRotateDragTracker(MapDocument &document, const UVViewHelper &helper, const float initialAngle) : m_document{document}, m_helper{helper}, m_initialAngle{initialAngle} {
