...
namespace View {
GameListBox::GameListBox(QWidget *parent) : ImageListBox("No Games Found", false, parent) {
    reloadGameInfos();
}
...
GameListBox::Info GameListBox::makeGameInfo(const std::string &gameName) const {
    ...
    return Info{
        gameName, IO::loadPixmapResource(iconPath), QString::fromStdString(gameName + (experimental ? " (experimental)" : "")), QString::fromStdString(gamePath.empty() ? std::string("Game not found") : gamePath.string())
    };
}
