...
MapViewBase::MapViewBase(
    Logger *logger, std::weak_ptr<MapDocument> document, MapViewToolBox &toolBox, Renderer::MapRenderer &renderer, GLContextManager &contextManager) : RenderView{contextManager}, m_logger{logger}, m_document{
                                                                                                                                                                                                         std::move(document)},
                                                                                                                                                       m_toolBox{toolBox}, m_animationManager{std::make_unique<AnimationManager>(this)}, m_renderer{renderer}, m_compass{nullptr}, m_portalFileRenderer{nullptr}, m_isCurrent{false}, m_updateActionStatesSignalDelayer{new SignalDelayer{this}} {
    setToolBox(toolBox);
    bindEvents();
    connectObservers();
...
ActionContext::Type MapViewBase::actionContext() const {
    ...
    const auto toolContext =
        m_toolBox.createComplexBrushToolActive() ? ActionContext::CreateComplexBrushTool : m_toolBox.clipToolActive()        ? ActionContext::ClipTool
    ...
    const auto selectionContext = document->hasSelectedNodes() ? ActionContext::NodeSelection : document->hasSelectedBrushFaces() ? ActionContext::FaceSelection
...
void MapViewBase::initializeGL() {
    if (doInitializeGL()) {
        m_logger
                ->info()
            << "Renderer info: " << GLContextManager::GLRenderer << " version " << GLContextManager::GLVersion << " from " << GLContextManager::GLVendor;
    ...
    if (portalFile != nullptr) {
        for (const auto &poly: portalFile->portals()) {
            m_portalFileRenderer->renderFilledPolygon(
                pref(Preferences::PortalFileFillColor), Renderer::PrimitiveRendererOcclusionPolicy::Hide, Renderer::PrimitiveRendererCullingPolicy::ShowBackfaces,
                poly.vertices());
...
void MapViewBase::showPopupMenuLater() {
    ...
    if (faceHandle) {
        ...
        menu.addAction(
            tr("Reveal %1 in Texture Browser").arg(QString::fromStdString(faceHandle->face().attributes().textureName())), mapFrame,
...
Model::GroupNode *MapViewBase::findGroupToMergeGroupsInto(const Model::NodeCollection &selectedNodes) const {
    ...
    if (!hits.empty()) {
        if (auto *mergeTarget = findOutermostClosedGroup(Model::hitToNode(hits.front()))) {
            if (kdl::all_of(
                    selectedNodes.nodes(), [&](const auto *node) {
