...
namespace View {
template<typename T> std::tuple<vm::vec<T, 3>, vm::vec<T, 3>, vm::vec<T, 3>> computeAxes(const vm::vec3 &handlePos, const vm::vec<T, 3> &cameraPos) {
...
RotateObjectsHandle::Handle::Handle(const vm::vec3 &position) : m_position(position) {
}
...
vm::mat4x4 RotateObjectsHandle::Handle::handleTransform(const Renderer::Camera &camera, const HitArea area) const {
    ...
    switch (area) {
    case HitArea::XAxis:return vm::mat4x4::rot_90_y_ccw() * scalingMatrix;
    case HitArea::YAxis:return vm::mat4x4::rot_90_x_cw() * scalingMatrix;
    case HitArea::ZAxis:
    case HitArea::Center:
    case HitArea::None:return vm::mat4x4::identity() * scalingMatrix;
        switchDefault();
    }
}
...
Model::Hit RotateObjectsHandle::Handle2D::pick(const vm::ray3 &pickRay, const Renderer::Camera &camera) const {
    ...
    case vm::axis::x: return Model::selectClosest(pickCenterHandle(pickRay, camera), pickRotateHandle(pickRay, camera, HitArea::XAxis));
    case vm::axis::y: return Model::selectClosest(pickCenterHandle(pickRay, camera), pickRotateHandle(pickRay, camera, HitArea::YAxis));
    default: return Model::selectClosest(pickCenterHandle(pickRay, camera), pickRotateHandle(pickRay, camera, HitArea::ZAxis));
    }
}
...
Model::Hit RotateObjectsHandle::Handle2D::pickRotateHandle(const vm::ray3 &pickRay, const Renderer::Camera &camera, const HitArea area) const {
    ...
    auto ray = pickRay;
    switch (area) {
    case HitArea::XAxis:ray.origin[0] = m_position[0];
        break;

    case HitArea::YAxis:ray.origin[1] = m_position[1];
        break;

    case HitArea::ZAxis:ray.origin[2] = m_position[2];
        break;

    case HitArea::None:
    case HitArea::Center:break;
...
void RotateObjectsHandle::Handle2D::renderHighlight(Renderer::RenderContext &renderContext, Renderer::RenderBatch &renderBatch, RotateObjectsHandle::HitArea area) const {
    ...
    switch (area) {
    case HitArea::Center:renderService.setForegroundColor(pref(Preferences::SelectedHandleColor));
        renderService.renderHandleHighlight(vm::vec3f(m_position));
        break;
    case HitArea::XAxis:
    case HitArea::YAxis:
    case HitArea::ZAxis:renderService.setLineWidth(3.0f);
        renderService.setForegroundColor(pref(Preferences::axisColor(vm::find_abs_max_component(camera.direction()))));
        renderService.renderCircle(vm::vec3f(m_position), vm::find_abs_max_component(camera.direction()), 64, radius);
        break;
    case HitArea::None:break;
        switchDefault();
    }
}

Model::Hit RotateObjectsHandle::Handle3D::pick(const vm::ray3 &pickRay, const Renderer::Camera &camera) const {
    return Model::selectClosest(pickCenterHandle(pickRay, camera), pickRotateHandle(pickRay, camera, HitArea::XAxis), pickRotateHandle(pickRay, camera, HitArea::YAxis), pickRotateHandle(pickRay, camera, HitArea::ZAxis));
}
...
void RotateObjectsHandle::Handle3D::renderHighlight(Renderer::RenderContext &renderContext, Renderer::RenderBatch &renderBatch, RotateObjectsHandle::HitArea area) const {
    ...
    case HitArea::Center:renderService.setForegroundColor(pref(Preferences::SelectedHandleColor));
        ...
        break;
    case HitArea::XAxis:renderService.setForegroundColor(pref(Preferences::XAxisColor));
        renderService.setLineWidth(3.0f);
        renderService.renderCircle(vm::vec3f(m_position), vm::axis::x, 64, radius, zAxis, yAxis);
        break;
    case HitArea::YAxis:renderService.setForegroundColor(pref(Preferences::YAxisColor));
        renderService.setLineWidth(3.0f);
        renderService.renderCircle(vm::vec3f(m_position), vm::axis::y, 64, radius, xAxis, zAxis);
        break;
    case HitArea::ZAxis:renderService.setForegroundColor(pref(Preferences::ZAxisColor));
        renderService.setLineWidth(3.0f);
        renderService.renderCircle(vm::vec3f(m_position), vm::axis::z, 64, radius, xAxis, yAxis);
        break;
    case HitArea::None:break;
...
RotateObjectsHandle::RotateObjectsHandle() : m_handle2D(m_position), m_handle3D(m_position) {
}
...
vm::vec3 RotateObjectsHandle::rotationAxis(const HitArea area) const {
    switch (area) {
    case HitArea::XAxis:return vm::vec3::pos_x();
    case HitArea::YAxis:return vm::vec3::pos_y();
    case HitArea::ZAxis:return vm::vec3::pos_z();
    case HitArea::None:
    case HitArea::Center:return vm::vec3::pos_z();
        switchDefault();
    }
}
