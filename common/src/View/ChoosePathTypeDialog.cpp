...
std::filesystem::path convertToPathType(const PathType pathType, const std::filesystem::path &absPath, const std::filesystem::path &docPath, const std::filesystem::path &gamePath) {
    switch (pathType) {
    ...
    case PathType::GameRelative:return absPath.lexically_relative(gamePath);
    case PathType::AppRelative:return absPath.lexically_relative(IO::SystemPaths::appDirectory());
        switchDefault();
    }
}

ChoosePathTypeDialog::ChoosePathTypeDialog(QWidget *parent, const std::filesystem::path &absPath, const std::filesystem::path &docPath, const std::filesystem::path &gamePath)
    : QDialog{parent} {
...
void ChoosePathTypeDialog::createGui(const std::filesystem::path &absPath, const std::filesystem::path &docPath, const std::filesystem::path &gamePath) {
    ...
    auto *mapRelativePathText = makeInfo(new QLabel{
        docRelativePath.empty() ? tr("Could not build a path.") : IO::pathAsQString(docRelativePath)
    });
    ...
    auto *appRelativePathText = makeInfo(new QLabel{
        appRelativePath.empty() ? tr("Could not build a path.") : IO::pathAsQString(appRelativePath)
    });
    ...
    auto *gameRelativePathText = makeInfo(new QLabel{
        gameRelativePath.empty() ? tr("Could not build a path.") : IO::pathAsQString(gameRelativePath)
    });

    auto *okCancelButtons = new QDialogButtonBox{QDialogButtonBox::Ok | QDialogButtonBox::Cancel};

    auto *innerLayout = new QVBoxLayout{};
    innerLayout->setContentsMargins(LayoutConstants::DialogOuterMargin, LayoutConstants::DialogOuterMargin, LayoutConstants::DialogOuterMargin, LayoutConstants::DialogOuterMargin);
