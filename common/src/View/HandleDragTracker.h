...
struct HandleDragTrackerDelegate {
   ...
   */
  virtual HandlePositionProposer start(const InputState &inputState, const vm::vec3 &initialHandlePosition, const vm::vec3 &initialHitPoint) = 0;
...
public:
     ...
     */
    HandleDragTracker(Delegate delegate, const InputState &inputState, const vm::vec3 &initialHandlePosition, const vm::vec3 &initialHitPoint) : m_delegate{
        std::move(delegate)
    }, m_dragState{
        initialHandlePosition, initialHandlePosition, initialHandlePosition - initialHitPoint
    }, m_proposeHandlePosition{
        m_delegate.start(inputState, m_dragState.initialHandlePosition, m_dragState.handleOffset)
    } {
    }
 ...
 */
template<typename Delegate>
std::unique_ptr<HandleDragTracker<Delegate>>
createHandleDragTracker(Delegate delegate, const InputState &inputState, const vm::vec3 &initialHandlePosition, const vm::vec3 &initialHitPoint) {
    return std::make_unique<HandleDragTracker<Delegate>>(std::move(delegate), inputState, initialHandlePosition, initialHitPoint);
}
