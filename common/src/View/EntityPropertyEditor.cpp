...
namespace View {
EntityPropertyEditor::EntityPropertyEditor(std::weak_ptr<MapDocument> document, QWidget *parent) : QWidget{parent}, m_document{std::move(document)}, m_splitter{nullptr}, m_propertyGrid{nullptr}, m_smartEditorManager{
                                                                                                                                                                                                       nullptr},
...
void EntityPropertyEditor::updateIfSelectedEntityDefinitionChanged() {
    auto document = kdl::mem_lock(m_document);
    const auto *entityDefinition = Model::selectEntityDefinition(document->allSelectedEntityNodes());
...
void EntityPropertyEditor::updateDocumentation(const std::string &propertyKey) {
    ...
    auto document = kdl::mem_lock(m_document);
    if (const auto *entityDefinition = Model::selectEntityDefinition(document->allSelectedEntityNodes())) {
