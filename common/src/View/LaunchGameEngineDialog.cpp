...
void LaunchGameEngineDialog::createGui()
{
  ...
  m_gameEngineList->setContentsMargins(
  ...
  parameterLabel->setContentsMargins(
    LayoutConstants::NarrowVMargin,
    LayoutConstants::NarrowHMargin,
    LayoutConstants::NarrowHMargin,
    LayoutConstants::NarrowHMargin);
  ...
  midRightLayout->setContentsMargins(
    LayoutConstants::NarrowVMargin,
    LayoutConstants::NarrowHMargin,
    LayoutConstants::NarrowHMargin,
    LayoutConstants::NarrowHMargin);
  ...
  connect(
    m_testButton, &QPushButton::clicked, this, &LaunchGameEngineDialog::testCommandLine);
...
void LaunchGameEngineDialog::launchEngine()
{
  try
  {
    ...
    const auto arguments = QStringList{
      IO::pathAsQString(profile->path), "--args", QString::fromStdString(parameters)};
...
void LaunchGameEngineDialog::testCommandLine()
{
  ...
  {
    const auto message =
      kdl::str_to_string("Error while testing command line: ", e.what());
    auto document = kdl::mem_lock(m_document);
    document->logger().error(message);
    QMessageBox::critical(
      this, "TrenchBroom", QString::fromStdString(message), QMessageBox::Ok);
