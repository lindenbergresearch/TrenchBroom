...
CompilationProfileListBox::CompilationProfileListBox(Model::CompilationConfig &config, QWidget *parent) : ControlListBox{
    "Click the '+' button to create a compilation profile.", true, parent
}, m_config{config} {
    reload();
}
...
ControlListBoxItemRenderer *CompilationProfileListBox::createItemRenderer(QWidget *parent, const size_t index) {
    ...
    connect(renderer, &QWidget::customContextMenuRequested, this, [=](const QPoint &pos) {
          emit this->profileContextMenuRequested(renderer->mapToGlobal(pos), m_config.profiles[index]);
        }
    );
    return renderer;
}
