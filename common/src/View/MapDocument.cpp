...
template<typename T> auto collectContainingGroups(const std::vector<T *> &nodes) {
    auto result = std::vector<Model::GroupNode *>{};
    Model::Node::visitAll(nodes, kdl::overload([](const Model::WorldNode *) {}, [](const Model::LayerNode *) {}, [&](Model::GroupNode *groupNode) {
...
std::vector<Model::GroupNode *> collectGroupsOrContainers(const std::vector<Model::Node *> &nodes) {
    auto result = std::vector<Model::GroupNode *>{};
    Model::Node::visitAll(nodes, kdl::overload([](const Model::WorldNode *) {}, [](const Model::LayerNode *) {}, [&](Model::GroupNode *groupNode) { result.push_back(groupNode); }, [&](Model::EntityNode *entityNode) {
 ...
 */
template<typename N, typename L> std::optional<std::vector<std::pair<Model::Node *, Model::NodeContents>>> applyToNodeContents(const std::vector<N *> &nodes, L lambda) {
    ...
    std::transform(std::begin(nodes), std::end(nodes), std::back_inserter(newNodes), [&](auto *node) {
        NodeContentType nodeContents = node->accept(kdl::overload([](const Model::WorldNode *worldNode) -> NodeContentType {
 ...
 */
template<typename N, typename L> bool applyAndSwap(MapDocument &document, const std::string &commandName, const std::vector<N *> &nodes, std::vector<Model::GroupNode *> changedLinkedGroups, L lambda) {
    if (nodes.empty()) {
        return true;
    }

    if (auto newNodes = applyToNodeContents(nodes, std::move(lambda))) {
        return document.swapNodeContents(commandName, std::move(*newNodes), std::move(changedLinkedGroups));
    }
 ...
 */
template<typename L> bool applyAndSwap(MapDocument &document, const std::string &commandName, const std::vector<Model::BrushFaceHandle> &faces, L lambda) {
    if (faces.empty()) {
        return true;
    }
    ...
    if (success) {
        ...
        auto changedLinkedGroups = collectContainingGroups(kdl::vec_transform(newNodes, [](const auto &p) { return p.first; }));
...
MapDocument::MapDocument()
    : m_worldBounds(DefaultWorldBounds), m_world(nullptr), m_entityDefinitionManager(std::make_unique<Assets::EntityDefinitionManager>()), m_entityModelManager(std::make_unique<Assets::EntityModelManager>(pref(Preferences::TextureMagFilter), pref(Preferences::TextureMinFilter), logger())), m_textureManager(std::make_unique<Assets::TextureManager>(pref(Preferences::TextureMagFilter), pref(Preferences::TextureMinFilter), logger())), m_tagManager(std::make_unique<Model::TagManager>()), m_editorContext(std::make_unique<Model::EditorContext>()), m_grid(std::make_unique<Grid>(4)), m_path(DefaultDocumentName), m_lastSaveModificationCount(0), m_modificationCount(0), m_currentLayer(nullptr), m_currentTextureName(Model::BrushFaceAttributes::NoTextureName), m_lastSelectionBounds(0.0, 32.0), m_selectionBoundsValid(true), m_viewEffectsService(nullptr), m_repeatStack(std::make_unique<RepeatStack>()) {
    connectObservers();
}
...
Result<void> MapDocument::newDocument(const Model::MapFormat mapFormat, const vm::bbox3 &worldBounds, std::shared_ptr<Model::Game> game) {
...
auto extractNodesToPaste(const std::vector<Model::Node *> &nodes, Model::Node *parent) {
    ...
    for (auto *node : nodes) {
        node->accept(kdl::overload([&](auto &&thisLambda, Model::WorldNode *world) {
...
std::vector<Model::IdType> allPersistentGroupIds(const Model::Node &root) {
    ...
    root.accept(kdl::overload([](auto &&thisLambda, const Model::WorldNode *worldNode) {
    ...
    }, [](const Model::EntityNode *) {}, [](const Model::BrushNode *) {}, [](const Model::PatchNode *) {}));
    return result;
}

void fixRedundantPersistentIds(const std::map<Model::Node *, std::vector<Model::Node *>> &nodesToAdd, const std::vector<Model::IdType> &existingPersistentGroupIds) {
    ...
    for (auto &[newParent, nodesToAddToParent] : nodesToAdd) {
        for (auto *node : nodesToAddToParent) {
            node->accept(kdl::overload([&](auto &&thisLambda, Model::WorldNode *worldNode) {
                worldNode->visitChildren(thisLambda);
...
void fixRecursiveLinkedGroups(const std::map<Model::Node *, std::vector<Model::Node *>> &nodesToAdd, Logger &logger) {
    for (auto &[newParent, nodesToAddToParent] : nodesToAdd) {
        const auto linkedGroupIds = kdl::vec_sort(Model::collectParentLinkedGroupIds(*newParent));
        for (auto *node : nodesToAddToParent) {
            node->accept(kdl::overload([&](auto &&thisLambda, Model::WorldNode *worldNode) {
                worldNode->visitChildren(thisLambda);
            }, [&](auto &&thisLambda, Model::LayerNode *layerNode) {
...
void copyAndSetLinkIds(const std::map<Model::Node *, std::vector<Model::Node *>> &nodesToAdd, Model::WorldNode &worldNode, Logger &logger) {
    const auto groupsToAdd = kdl::vec_sort(Model::collectGroups(kdl::vec_flatten(kdl::map_values(nodesToAdd))), Model::compareGroupNodesByLinkId);

    const auto groupsByLinkId = kdl::make_grouped_range(groupsToAdd, [](const auto *lhs, const auto *rhs) { return lhs->linkId() == rhs->linkId(); });
    ...
    for (const auto &linkedGroupsToAdd : groupsByLinkId) {
        ...
        if (!existingLinkedNodes.empty()) {
            if (auto *existingLinkedGroup = dynamic_cast<Model::GroupNode *>(existingLinkedNodes.front())) {
                const auto errors = Model::copyAndSetLinkIds(*existingLinkedGroup, std::vector(linkedGroupsToAdd.begin(), linkedGroupsToAdd.end()));
...
void MapDocument::loadPointFile(std::filesystem::path path) {
    static_assert(!std::is_reference<decltype(path)>::value, "path must be passed by value because reloadPointFile() passes m_pointFilePath");
...
void MapDocument::loadPortalFile(std::filesystem::path path) {
    static_assert(!std::is_reference<decltype(path)>::value, "path must be passed by value because reloadPortalFile() passes m_portalFilePath");
...
std::vector<Model::EntityNodeBase *> MapDocument::allSelectedEntityNodes() const {
    ...
    for (auto *node : m_selectedNodes) {
        node->accept(kdl::overload([&](auto &&thisLambda, Model::WorldNode *world) {
            ...
            world->visitChildren(thisLambda);
        }, [&](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [&](Model::EntityNode *entity) { result.push_back(entity); }, [&](Model::BrushNode *brush) { result.push_back(brush->entity()); }, [&](Model::PatchNode *patch) { result.push_back(patch->entity()); }));
    }

    result = kdl::vec_sort_and_remove_duplicates(std::move(result));

    // Don't select worldspawn together with any other entities
    return result.size() == 1 ? result : kdl::vec_filter(std::move(result), [](const auto *entityNode) {
        return entityNode->entity().classname() != Model::EntityPropertyValues::WorldspawnClassname;
    });
}
...
std::vector<Model::BrushNode *> MapDocument::allSelectedBrushNodes() const {
    ...
    for (auto *node : m_selectedNodes.nodes()) {
        node->accept(kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::EntityNode *entity) {
            entity->visitChildren(thisLambda);
        }, [&](Model::BrushNode *brush) { brushes.push_back(brush); }, [&](Model::PatchNode *) {}));
    }
    return brushes;
}
...
bool MapDocument::hasAnySelectedBrushNodes() const {
    ...
    for (const auto *node : m_selectedNodes.nodes()) {
        const auto hasBrush = node->accept(kdl::overload([&](auto &&thisLambda, const Model::WorldNode *world) -> bool {
        ...
        }, [](const Model::BrushNode *) -> bool { return true; }, [](const Model::PatchNode *) -> bool { return false; }));
        if (hasBrush) {
            return true;
        }
    }
...
void MapDocument::selectTouching(const bool del) {
    const auto nodes = kdl::vec_filter(Model::collectTouchingNodes(std::vector<Model::Node *>{m_world.get()}, m_selectedNodes.brushes()), [&](Model::Node *node) { return m_editorContext->selectable(node); });
...
void MapDocument::selectInside(const bool del) {
    const auto nodes = kdl::vec_filter(Model::collectContainedNodes(std::vector<Model::Node *>{m_world.get()}, m_selectedNodes.brushes()), [&](Model::Node *node) { return m_editorContext->selectable(node); });
...
void MapDocument::selectInverse() {
    ...
    currentGroupOrWorld()->accept(kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, Model::GroupNode *group) {
        collectNode(group);
        group->visitChildren(thisLambda);
    }, [&](auto &&thisLambda, Model::EntityNode *entity) {
        collectNode(entity);
        entity->visitChildren(thisLambda);
    }, [&](Model::BrushNode *brush) { collectNode(brush); }, [&](Model::PatchNode *patch) { collectNode(patch); }));
...
void MapDocument::selectNodesWithFilePosition(const std::vector<size_t> &positions) {
    ...
    m_world->accept(kdl::overload([&](auto &&thisLambda, Model::WorldNode *worldNode) {
        worldNode->visitChildren(thisLambda);
    }, [&](auto &&thisLambda, Model::LayerNode *layerNode) {
        layerNode->visitChildren(thisLambda);
    }, [&](auto &&thisLambda, Model::GroupNode *groupNode) {
        ...
        if (hasFilePosition(entityNode)) {
            if (m_editorContext->selectable(entityNode)) {
                ...
                if (previousCount == nodesToSelect.size()) {
                    ...
                    nodesToSelect = kdl::vec_concat(std::move(nodesToSelect), Model::collectSelectableNodes(entityNode->children(), *m_editorContext));
                }
            }
        }
    }, [&](Model::BrushNode *brushNode) {
...
void MapDocument::selectFacesWithTexture(const Assets::Texture *texture) {
    const auto faces = kdl::vec_filter(Model::collectSelectableBrushFaces(std::vector<Model::Node *>{m_world.get()}, *m_editorContext), [&](const auto &faceHandle) { return faceHandle.face().texture() == texture; });
...
void MapDocument::selectTall(const vm::axis::type cameraAxis) {
    ...
    kdl::fold_results(kdl::vec_transform(selectionBrushNodes, [&](const auto *selectionBrushNode) {
        ...
        return brushBuilder.createBrush(tallVertices, Model::BrushFaceAttributes::NoTextureName).transform([](auto brush) {
        ...
        const auto nodesToSelect = kdl::vec_filter(Model::collectContainedNodes({world()}, kdl::vec_transform(tallBrushes, [](const auto &b) { return b.get(); })), [&](const auto *node) { return editorContext().selectable(node); });
        selectNodes(nodesToSelect);

        transaction.commit();
    }).transform_error([&](auto e) { logger().error() << "Could not create selection brush: " << e.msg; });
}
...
namespace {
auto setLinkIdsForReparentingNodes(const std::map<Model::Node *, std::vector<Model::Node *>> &nodesToReparent) {
    auto result = std::vector<std::tuple<Model::Node *, std::string>>{};
    for (const auto &[newParent, nodes] : nodesToReparent) {
        Model::Node::visitAll(nodes, kdl::overload([](const Model::WorldNode *) {}, [](const Model::LayerNode *) {}, [](const Model::GroupNode *) {
...
bool MapDocument::reparentNodes(const std::map<Model::Node *, std::vector<Model::Node *>> &nodesToAdd) {
    ...
    const auto nodesToRemove = parentChildrenMap(kdl::vec_flatten(kdl::map_values(nodesToAdd)));

    const auto changedLinkedGroups = collectGroupsOrContainers(kdl::vec_concat(kdl::map_keys(nodesToAdd), kdl::map_keys(nodesToRemove)));
    ...
    for (auto &[newParent, nodes] : nodesToAdd) {
        ...
        const auto nodesToDowngrade = Model::collectNodesAndDescendants(nodes, [&](Model::Object *node) { return node->containingLayer() != newParentLayer; });
    ...
    executeAndStore(std::make_unique<SetLinkIdsCommand>("Set Link ID", setLinkIdsForReparentingNodes(nodesToAdd)));

    const auto result = executeAndStore(ReparentNodesCommand::reparent(nodesToAdd, nodesToRemove));
    if (!result->success()) {
        transaction.cancel();
        return false;
    }
    ...
    while (!removableNodes.empty()) {
        setHasPendingChanges(collectContainingGroups(kdl::vec_flatten(kdl::map_values(removableNodes))), true);
...
static bool shouldCloneParentWhenCloningNode(const Model::Node *node) {
    return node->parent()->accept(kdl::overload([](const Model::WorldNode *) { return false; }, [](const Model::LayerNode *) { return false; }, [](const Model::GroupNode *) { return false; }, [&](const Model::EntityNode *) { return true; }, [](const Model::BrushNode *) { return false; }, [](const Model::PatchNode *) { return false; }));
}
...
void MapDocument::duplicateObjects() {
    ...
    for (auto *original : selectedNodes().nodes()) {
        ...
        const auto isLinkedGroup = dynamic_cast<const Model::GroupNode *>(original) != nullptr && Model::collectLinkedNodes({m_world.get()}, *original).size() > 1;
        const auto setLinkIds = isLinkedGroup ? Model::SetLinkId::keep : Model::SetLinkId::generate;
...
Model::EntityNode *MapDocument::createPointEntity(const Assets::PointEntityDefinition *definition, const vm::vec3 &delta) {
    ...
    if (m_world->entityPropertyConfig().setDefaultProperties) {
        Model::setDefaultProperties(m_world->entityPropertyConfig(), *definition, entity, Model::SetDefaultPropertyMode::SetAll);
    }
...
Model::EntityNode *MapDocument::createBrushEntity(const Assets::BrushEntityDefinition *definition) {
    ...
    // properties
    auto entity = (brushes.front()->entity() != m_world.get() && std::all_of(std::next(brushes.begin()), brushes.end(), [&](const auto *brush) { return brush->entity() == brushes.front()->entity(); })) ? brushes.front()->entity()->entity() : Model::Entity{};
...
static std::vector<Model::Node *> collectGroupableNodes(const std::vector<Model::Node *> &selectedNodes, const Model::EntityNodeBase *world) {
    ...
    Model::Node::visitAll(selectedNodes, kdl::overload([](Model::WorldNode *) {}, [](Model::LayerNode *) {}, [&](Model::GroupNode *group) { result.push_back(group); }, [&](Model::EntityNode *entity) { result.push_back(entity); }, [&](auto &&thisLambda, Model::BrushNode *brush) { addNode(thisLambda, brush); }, [&](auto &&thisLambda, Model::PatchNode *patch) { addNode(thisLambda, patch); }));
    return kdl::vec_sort_and_remove_duplicates(std::move(result));
}
...
void MapDocument::ungroupSelection() {
    ...
    auto success = true;
    Model::Node::visitAll(selectedNodes, kdl::overload([](Model::WorldNode *) {}, [](Model::LayerNode *) {}, [&](Model::GroupNode *group) {
    ...
    }, [&](Model::EntityNode *entity) { nodesToReselect.push_back(entity); }, [&](Model::BrushNode *brush) { nodesToReselect.push_back(brush); }, [&](Model::PatchNode *patch) { nodesToReselect.push_back(patch); }));
...
void MapDocument::renameGroups(const std::string &name) {
    if (hasSelectedNodes() && m_selectedNodes.hasOnlyGroups()) {
        ...
        applyAndSwap(*this, commandName, m_selectedNodes.groups(), {}, kdl::overload([](Model::Layer &) { return true; }, [&](Model::Group &group) {
            group.setName(name);
            return true;
        }, [](Model::Entity &) { return true; }, [](Model::Brush &) { return true; }, [](Model::BezierPatch &) { return true; }));
    }
}
...
void MapDocument::selectLinkedGroups() {
    ...
    const auto linkIdsToSelect = kdl::vec_sort_and_remove_duplicates(kdl::vec_transform(m_selectedNodes.groups(), [](const auto *groupNode) { return groupNode->linkId(); }));
    const auto groupNodesToSelect = kdl::vec_flatten(kdl::vec_transform(linkIdsToSelect, [&](const auto &linkId) {
...
bool MapDocument::canSelectLinkedGroups() const {
    ...
    const auto allLinkIds = kdl::vec_sort(kdl::vec_transform(Model::collectGroups({m_world.get()}), [](const auto &groupNode) { return groupNode->linkId(); }));

    return kdl::all_of(m_selectedNodes.groups(), [&](const auto *groupNode) {
        const auto [iBegin, iEnd] = std::equal_range(allLinkIds.begin(), allLinkIds.end(), groupNode->linkId());
...
void MapDocument::linkGroups(const std::vector<Model::GroupNode *> &groupNodes) {
    if (groupNodes.size() > 1) {
        ...
        const auto targetGroupNodes = kdl::vec_slice_suffix(groupNodes, groupNodes.size() - 1);
        Model::copyAndReturnLinkIds(sourceGroupNode, targetGroupNodes).transform([&](auto linkIds) {
            auto linkIdVector = kdl::vec_transform(std::move(linkIds), [](auto pair) -> std::tuple<Model::Node *, std::string> {
                return {std::move(pair)};
            });

            executeAndStore(std::make_unique<SetLinkIdsCommand>("Set Link ID", std::move(linkIdVector)));
...
void MapDocument::unlinkGroups(const std::vector<Model::GroupNode *> &groupNodes) {
    ...
    auto linkIds = kdl::vec_transform(nodesToUnlink, [](auto *node) -> std::tuple<Model::Node *, std::string> {
        return {node, generateUuid()};
    });

    executeAndStore(std::make_unique<SetLinkIdsCommand>("Reset Link ID", std::move(linkIds)));
}
...
bool MapDocument::canSeparateLinkedGroups() const {
    return kdl::any_of(m_selectedNodes.groups(), [&](const auto *groupNode) {
        const auto linkedGroups = Model::collectNodesWithLinkId({m_world.get()}, groupNode->linkId());
        return linkedGroups.size() > 1u && kdl::any_of(linkedGroups, [](const auto *linkedGroupNode) {
...
void MapDocument::setHasPendingChanges(const std::vector<Model::GroupNode *> &groupNodes, const bool hasPendingChanges) {
...
static std::vector<Model::GroupNode *> collectGroupsWithPendingChanges(Model::Node &node) {
    ...
    node.accept(kdl::overload([](auto &&thisLambda, const Model::WorldNode *worldNode) {
        worldNode->visitChildren(thisLambda);
    }, [](auto &&thisLambda, const Model::LayerNode *layerNode) {
        layerNode->visitChildren(thisLambda);
    }, [&](auto &&thisLambda, Model::GroupNode *groupNode) {
...
void MapDocument::separateSelectedLinkedGroups(const bool relinkGroups) {
    const auto selectedLinkIds = kdl::vec_sort_and_remove_duplicates(kdl::vec_transform(m_selectedNodes.groups(), [](const auto *groupNode) { return groupNode->linkId(); }));
    ...
    for (const auto &linkedGroupId : selectedLinkIds) {
        ...
        const auto it = std::partition(std::begin(linkedGroups), std::end(linkedGroups), [](const auto *linkedGroupNode) {
            return linkedGroupNode->selected();
        });
...
void MapDocument::renameLayer(Model::LayerNode *layerNode, const std::string &name) {
    applyAndSwap(*this, "Rename Layer", std::vector<Model::Node *>{layerNode}, {}, kdl::overload([&](Model::Layer &layer) {
        layer.setName(name);
        return true;
    }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [](Model::Brush &) { return true; }, [](Model::BezierPatch &) { return true; }));
}
...
bool MapDocument::moveLayerByOne(Model::LayerNode *layerNode, MoveDirection direction) {
    ...
    swapNodeContents("Swap Layer Positions", {{layerNode, Model::NodeContents(std::move(layer))}, {neighbourNode, Model::NodeContents(std::move(neighbourLayer))}}, {});
...
void MapDocument::moveSelectionToLayer(Model::LayerNode *layer) {
    ...
    for (auto *node : selectedNodes) {
        ...
        }, [&](Model::BrushNode *brush) { addBrushOrPatchNode(brush); }, [&](Model::PatchNode *patch) { addBrushOrPatchNode(patch); }));
    }
...
bool MapDocument::canMoveSelectionToLayer(Model::LayerNode *layer) const {
    ...
    const bool isAnyNodeInOtherLayer = std::any_of(std::begin(nodes), std::end(nodes), [&](auto *node) {
...
bool MapDocument::canHideLayers(const std::vector<Model::LayerNode *> &layers) const {
    return std::any_of(layers.begin(), layers.end(), [](const auto *layer) { return layer->visible(); });
}
...
void MapDocument::isolate() {
    ...
    m_world->accept(kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, Model::GroupNode *group) {
        collectNode(group);
        group->visitChildren(thisLambda);
    }, [&](auto &&thisLambda, Model::EntityNode *entity) {
        collectNode(entity);
...
void MapDocument::selectAllInLayers(const std::vector<Model::LayerNode *> &layers) {
    const auto nodes = Model::collectSelectableNodes(kdl::vec_static_cast<Model::Node *>(layers), editorContext());
...
bool MapDocument::swapNodeContents(const std::string &commandName, std::vector<std::pair<Model::Node *, Model::NodeContents>> nodesToSwap, std::vector<Model::GroupNode *> changedLinkedGroups) {
    ...
    auto transaction = Transaction{*this};
    const auto result = executeAndStore(std::make_unique<SwapNodeContentsCommand>(commandName, std::move(nodesToSwap)));
...
bool MapDocument::swapNodeContents(const std::string &commandName, std::vector<std::pair<Model::Node *, Model::NodeContents>> nodesToSwap) {
    auto changedLinkedGroups = collectContainingGroups(kdl::vec_transform(nodesToSwap, [](const auto &p) { return p.first; }));
...
bool MapDocument::transformObjects(const std::string &commandName, const vm::mat4x4 &transformation) {
    ...
    for (const auto &[entityNode, transformedChildCount] : entitiesToTransform) {
        if (transformedChildCount == entityNode->childCount() && !Model::isWorldspawn(entityNode->entity().classname())) {
            nodesToTransform.push_back(entityNode);
        }
    }
    ...
    auto transformResults = kdl::vec_parallel_transform(nodesToTransform, [&](Model::Node *node) -> TransformResult {
        return node->accept(kdl::overload([&](Model::WorldNode *) -> TransformResult {
            ...
            const bool lockTextures = lockTexturesPref || Model::collectLinkedNodes({m_world.get()}, *brushNode).size() > 1;

            auto brush = brushNode->brush();
            return brush.transform(m_worldBounds, transformation, lockTextures).and_then([&]() -> TransformResult {
    ...
    return kdl::fold_results(std::move(transformResults)).and_then([&](auto nodesToUpdate) -> Result<bool> {
        const auto success = swapNodeContents(commandName, std::move(nodesToUpdate), collectContainingGroups(m_selectedNodes.nodes()));
...
bool MapDocument::createBrush(const std::vector<vm::vec3> &points) {
    ...
    return builder.createBrush(points, currentTextureName()).and_then([&](auto b) -> Result<void> {
    ...
    }).if_error([&](auto e) { error() << "Could not create brush: " << e.msg; }).is_success();
}
...
bool MapDocument::csgConvexMerge() {
    ...
    return builder.createBrush(polyhedron, currentTextureName()).transform([&](auto b) {
...
bool MapDocument::csgSubtract() {
    ...
    return kdl::fold_results(kdl::vec_transform(minuendNodes, [&](auto *minuendNode) {
        ...
        return kdl::fold_results(kdl::vec_filter(std::move(currentSubtractionResults), [](const auto r) { return r.is_success(); })).transform([&](auto currentBrushes) {
...
bool MapDocument::csgIntersect() {
    ...
    for (auto it = std::next(std::begin(brushes)), end = std::end(brushes); it != end && valid; ++it) {
        ...
        valid = intersection.intersect(m_worldBounds, brush).if_error([&](auto e) { error() << "Could not intersect brushes: " << e.msg; }).is_success();
    }
...
bool MapDocument::csgHollow() {
    ...
    for (auto *brushNode : brushNodes) {
        ...
        shrunkenBrush.expand(m_worldBounds, -FloatType(m_grid->actualSize()), true).and_then([&]() {
            didHollowAnything = true;

            return kdl::fold_results(originalBrush.subtract(m_world->mapFormat(), m_worldBounds, currentTextureName(), shrunkenBrush)).transform([&](auto fragments) {
        ...
        }).transform_error([&](const auto &e) { error() << "Could not hollow brush: " << e; });
    }
...
bool MapDocument::clipBrushes(const vm::vec3 &p1, const vm::vec3 &p2, const vm::vec3 &p3) {
    return kdl::fold_results(kdl::vec_transform(m_selectedNodes.brushes(), [&](const Model::BrushNode *originalBrush) {
        auto clippedBrush = originalBrush->brush();
        return Model::BrushFace::create(p1, p2, p3, Model::BrushFaceAttributes{currentTextureName()}, m_world->mapFormat()).and_then([&](Model::BrushFace &&clipFace) {
        ...
        return kdl::void_success;
    }).if_error([&](const auto &e) { error() << "Could not clip brushes: " << e; }).is_success();
}
...
bool MapDocument::setProperty(const std::string &key, const std::string &value, const bool defaultToProtected) {
    ...
    return applyAndSwap(*this, "Set Property", entityNodes, collectContainingGroups(entityNodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [&](Model::Entity &entity) {
        entity.addOrUpdateProperty(m_world->entityPropertyConfig(), key, value, defaultToProtected);
        return true;
    }, [](Model::Brush &) { return true; }, [](Model::BezierPatch &) { return true; }));
}

bool MapDocument::renameProperty(const std::string &oldKey, const std::string &newKey) {
    const auto entityNodes = allSelectedEntityNodes();
    return applyAndSwap(*this, "Rename Property", entityNodes, collectContainingGroups(entityNodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [&](Model::Entity &entity) {
...
bool MapDocument::removeProperty(const std::string &key) {
    const auto entityNodes = allSelectedEntityNodes();
    return applyAndSwap(*this, "Remove Property", entityNodes, collectContainingGroups(entityNodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [&](Model::Entity &entity) {
...
bool MapDocument::convertEntityColorRange(const std::string &key, Assets::ColorRange::Type range) {
    const auto entityNodes = allSelectedEntityNodes();
    return applyAndSwap(*this, "Convert Color", entityNodes, collectContainingGroups(entityNodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [&](Model::Entity &entity) {
...
bool MapDocument::updateSpawnflag(const std::string &key, const size_t flagIndex, const bool setFlag) {
    const auto entityNodes = allSelectedEntityNodes();
    return applyAndSwap(*this, setFlag ? "Set Spawnflag" : "Unset Spawnflag", entityNodes, collectContainingGroups(entityNodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [&](Model::Entity &entity) {
 ...
 */
std::optional<std::string> findUnprotectedPropertyValue(const std::string &key, const std::vector<Model::EntityNodeBase *> &linkedEntities) {
 ...
 */
std::optional<std::string> findUnprotectedPropertyValue(const std::string &key, const Model::EntityNodeBase &entityNode, Model::WorldNode &worldNode) {
...
bool MapDocument::setProtectedProperty(const std::string &key, const bool value) {
    ...
    return swapNodeContents("Set Protected Property", nodesToUpdate, collectContainingGroups(entityNodes));
}
...
bool MapDocument::clearProtectedProperties() {
    ...
    return swapNodeContents("Clear Protected Properties", nodesToUpdate, collectContainingGroups(entityNodes));
}
...
void MapDocument::setDefaultProperties(const Model::SetDefaultPropertyMode mode) {
    const auto entityNodes = allSelectedEntityNodes();
    applyAndSwap(*this, "Reset Default Properties", entityNodes, collectContainingGroups(entityNodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [&](Model::Entity &entity) {
        if (const auto *definition = entity.definition()) {
            Model::setDefaultProperties(m_world->entityPropertyConfig(), *definition, entity, mode);
        }
        return true;
    }, [](Model::Brush &) { return true; }, [](Model::BezierPatch &) { return true; }));
}
...
bool MapDocument::extrudeBrushes(const std::vector<vm::polygon3> &faces, const vm::vec3 &delta) {
    const auto nodes = m_selectedNodes.nodes();
    return applyAndSwap(*this, "Resize Brushes", nodes, collectContainingGroups(nodes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &brush) {
        ...
        return brush.moveBoundary(m_worldBounds, *faceIndex, delta, pref(Preferences::TextureLock)).transform([&]() { return m_worldBounds.contains(brush.bounds()); }).transform_error([&](auto e) {
...
bool MapDocument::snapVertices(const FloatType snapTo) {
    ...
    const bool applyAndSwapSuccess = applyAndSwap(*this, "Snap Brush Vertices", allSelectedBrushes, collectContainingGroups(allSelectedBrushes), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &originalBrush) {
        if (originalBrush.canSnapVertices(m_worldBounds, snapTo)) {
            originalBrush.snapVertices(m_worldBounds, snapTo, pref(Preferences::UVLock)).transform([&]() { succeededBrushCount += 1; }).transform_error([&](auto e) {
...
MapDocument::MoveVerticesResult MapDocument::moveVertices(std::vector<vm::vec3> vertexPositions, const vm::vec3 &delta) {
    ...
    auto newNodes = applyToNodeContents(m_selectedNodes.nodes(), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &brush) {
        ...
        return brush.moveVertices(m_worldBounds, verticesToMove, delta, pref(Preferences::UVLock)).transform([&]() {
        ...
        }).if_error([&](auto e) { error() << "Could not move brush vertices: " << e.msg; }).is_success();
    }, [](Model::BezierPatch &) { return true; }));
    ...
    if (newNodes) {
        ...
        const auto changedLinkedGroups = collectContainingGroups(kdl::vec_transform(*newNodes, [](const auto &p) { return p.first; }));
...
bool MapDocument::moveEdges(std::vector<vm::segment3> edgePositions, const vm::vec3 &delta) {
    ...
    auto newNodes = applyToNodeContents(m_selectedNodes.nodes(), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &brush) {
        ...
        return brush.moveEdges(m_worldBounds, edgesToMove, delta, pref(Preferences::UVLock)).transform([&]() {
            auto newPositions = brush.findClosestEdgePositions(kdl::vec_transform(edgesToMove, [&](const auto &edge) { return edge.translate(delta); }));
            newEdgePositions = kdl::vec_concat(std::move(newEdgePositions), std::move(newPositions));
        }).if_error([&](auto e) { error() << "Could not move brush edges: " << e.msg; }).is_success();
    }, [](Model::BezierPatch &) { return true; }));
    ...
    if (newNodes) {
        ...
        const auto result = executeAndStore(std::make_unique<BrushEdgeCommand>(commandName, std::move(*newNodes), std::move(edgePositions), std::move(newEdgePositions)));
...
bool MapDocument::moveFaces(std::vector<vm::polygon3> facePositions, const vm::vec3 &delta) {
    ...
    auto newNodes = applyToNodeContents(m_selectedNodes.nodes(), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &brush) {
        ...
        return brush.moveFaces(m_worldBounds, facesToMove, delta, pref(Preferences::UVLock)).transform([&]() {
            auto newPositions = brush.findClosestFacePositions(kdl::vec_transform(facesToMove, [&](const auto &face) { return face.translate(delta); }));
            newFacePositions = kdl::vec_concat(std::move(newFacePositions), std::move(newPositions));
        }).if_error([&](auto e) { error() << "Could not move brush faces: " << e.msg; }).is_success();
    }, [](Model::BezierPatch &) { return true; }));
    ...
    if (newNodes) {
        ...
        const auto result = executeAndStore(std::make_unique<BrushFaceCommand>(commandName, std::move(*newNodes), std::move(facePositions), std::move(newFacePositions)));
...
bool MapDocument::addVertex(const vm::vec3 &vertexPosition) {
    auto newNodes = applyToNodeContents(m_selectedNodes.nodes(), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &brush) {
        ...
        return brush.addVertex(m_worldBounds, vertexPosition).if_error([&](auto e) { error() << "Could not add brush vertex: " << e.msg; }).is_success();
    }, [](Model::BezierPatch &) { return true; }));
    ...
    if (newNodes) {
        ...
        const auto result = executeAndStore(std::make_unique<BrushVertexCommand>(commandName, std::move(*newNodes), std::vector<vm::vec3>{}, std::vector<vm::vec3>{vertexPosition}));
...
bool MapDocument::removeVertices(const std::string &commandName, std::vector<vm::vec3> vertexPositions) {
    auto newNodes = applyToNodeContents(m_selectedNodes.nodes(), kdl::overload([](Model::Layer &) { return true; }, [](Model::Group &) { return true; }, [](Model::Entity &) { return true; }, [&](Model::Brush &brush) {
        ...
        return brush.removeVertices(m_worldBounds, verticesToRemove).if_error([&](auto e) { error() << "Could not remove brush vertices: " << e.msg; }).is_success();
    }, [](Model::BezierPatch &) { return true; }));
    ...
    if (newNodes) {
        ...
        const auto result = executeAndStore(std::make_unique<BrushVertexCommand>(commandName, std::move(*newNodes), std::move(vertexPositions), std::vector<vm::vec3>{}));
...
class ThrowExceptionCommand : public UndoableCommand {
  ...
  public:
    ThrowExceptionCommand() : UndoableCommand("Throw Exception", false) {
    }
...
Result<void> MapDocument::createWorld(const Model::MapFormat mapFormat, const vm::bbox3 &worldBounds, std::shared_ptr<Model::Game> game) {
...
Result<void> MapDocument::loadWorld(const Model::MapFormat mapFormat, const vm::bbox3 &worldBounds, std::shared_ptr<Model::Game> game, const std::filesystem::path &path) {
    return game->loadMap(mapFormat, m_worldBounds, path, logger()).transform([&](auto world) {
        m_worldBounds = worldBounds;
        m_game = game;
        m_world = std::move(world);
        performSetCurrentLayer(m_world->defaultLayer());
...
void MapDocument::setEntityDefinitions(std::vector<std::unique_ptr<Assets::EntityDefinition>> definitions) {
...
std::vector<std::filesystem::path> MapDocument::enabledTextureCollections() const {
    if (m_world) {
        if (const auto *textureCollectionStr = m_world->entity().property(Model::EntityPropertyKeys::EnabledTextureCollections)) {
            return kdl::vec_sort_and_remove_duplicates(kdl::vec_transform(kdl::str_split(*textureCollectionStr, ";"), [](const auto &str) { return std::filesystem::path{str}; }));
        }

        // If the map uses wad files, always enable all of them by default
        if (m_world->entity().property(Model::EntityPropertyKeys::Wad)) {
            return kdl::vec_sort_and_remove_duplicates(kdl::vec_transform(m_textureManager->collections(), [](const auto &collection) { return collection.path(); }));
        }
...
std::vector<std::filesystem::path> MapDocument::disabledTextureCollections() const {
    if (m_world) {
        auto textureCollections = kdl::vec_sort_and_remove_duplicates(kdl::vec_transform(m_textureManager->collections(), [](const auto &collection) { return collection.path(); }));
...
void MapDocument::setEnabledTextureCollections(const std::vector<std::filesystem::path> &enabledTextureCollections) {
    const auto enabledTextureCollectionStr = kdl::str_join(kdl::vec_transform(kdl::vec_sort_and_remove_duplicates(enabledTextureCollections), [](const auto &path) { return path.string(); }), ";");

    auto transaction = Transaction{*this, "Set enabled texture collections"};
    const auto success = setProperty(Model::EntityPropertyKeys::EnabledTextureCollections, enabledTextureCollectionStr);
...
void MapDocument::reloadTextures() {
    unloadTextures();
    m_game->reloadShaders().transform_error([&](auto e) { error() << "Failed to reload shaders: " << e.msg; });
    loadTextures();
}
...
void MapDocument::loadTextures() {
    try {
        if (const auto *wadStr = m_world->entity().property(Model::EntityPropertyKeys::Wad)) {
            const auto wadPaths = kdl::vec_transform(kdl::str_split(*wadStr, ";"), [](const auto &str) { return std::filesystem::path{str}; });
...
static auto makeSetTexturesVisitor(Assets::TextureManager &manager) {
    return kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::EntityNode *entity) {
        entity->visitChildren(thisLambda);
    }, [&](Model::BrushNode *brushNode) {
        const Model::Brush &brush = brushNode->brush();
...
static auto makeSetEntityDefinitionsVisitor(Assets::EntityDefinitionManager &manager) {
    ...
    return kdl::overload([=](auto &&thisLambda, Model::WorldNode *world) {
        setEntityDefinition(world);
        world->visitChildren(thisLambda);
    }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [=](Model::EntityNode *entity) { setEntityDefinition(entity); }, [](Model::BrushNode *) {}, [](Model::PatchNode *) {});
}
...
static auto makeUnsetEntityDefinitionsVisitor() {
    return kdl::overload([](auto &&thisLambda, Model::WorldNode *world) {
        ...
        world->visitChildren(thisLambda);
    }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [](Model::EntityNode *entity) { entity->setDefinition(nullptr); }, [](Model::BrushNode *) {}, [](Model::PatchNode *) {});
}
...
static auto makeSetEntityModelsVisitor(Logger &logger, Assets::EntityModelManager &manager) {
    return kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [&](Model::EntityNode *entityNode) {
...
static auto makeUnsetEntityModelsVisitor() {
    return kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [](Model::EntityNode *entity) { entity->setModelFrame(nullptr); }, [](Model::BrushNode *) {}, [](Model::PatchNode *) {});
}
...
void MapDocument::updateGameSearchPaths() {
    m_game->setAdditionalSearchPaths(kdl::vec_transform(mods(), [](const auto &mod) { return std::filesystem::path{mod}; }), logger());
}
...
void MapDocument::registerValidators() {
    ...
    m_world->registerValidator(std::make_unique<Model::LongPropertyKeyValidator>(m_game->maxPropertyLength()));
    m_world->registerValidator(std::make_unique<Model::LongPropertyValueValidator>(m_game->maxPropertyLength()));
    m_world->registerValidator(std::make_unique<Model::PropertyKeyWithDoubleQuotationMarksValidator>());
    m_world->registerValidator(std::make_unique<Model::PropertyValueWithDoubleQuotationMarksValidator>());
...
static auto makeInitializeNodeTagsVisitor(Model::TagManager &tagManager) {
    return kdl::overload([&](auto &&thisLambda, Model::WorldNode *world) {
        ...
        entity->visitChildren(thisLambda);
    }, [&](Model::BrushNode *brush) { brush->initializeTags(tagManager); }, [&](Model::PatchNode *patch) { patch->initializeTags(tagManager); });
}
...
static auto makeClearNodeTagsVisitor() {
    return kdl::overload([](auto &&thisLambda, Model::WorldNode *world) {
        ...
        entity->clearTags();
        entity->visitChildren(thisLambda);
    }, [](Model::BrushNode *brush) { brush->clearTags(); }, [](Model::PatchNode *patch) { patch->clearTags(); });
}
...
void MapDocument::updateAllFaceTags() {
    m_world->accept(kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::GroupNode *group) { group->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::EntityNode *entity) {
        entity->visitChildren(thisLambda);
    }, [&](Model::BrushNode *brush) { brush->initializeTags(*m_tagManager); }, [](Model::PatchNode *) {}));
}
...
void MapDocument::preferenceDidChange(const std::filesystem::path &path) {
    if (isGamePathPreference(path)) {
        ...
        reloadTextures();
        setTextures();
    } else if (path == Preferences::TextureMinFilter.path() || path == Preferences::TextureMagFilter.path()) {
...
Transaction::Transaction(std::weak_ptr<MapDocument> document, std::string name) : Transaction{kdl::mem_lock(document), std::move(name)} {
}

Transaction::Transaction(std::shared_ptr<MapDocument> document, std::string name) : Transaction{*document, std::move(name)} {
}

Transaction::Transaction(MapDocument &document, std::string name) : m_document{document}, m_name{std::move(name)}, m_state{State::Running} {
    begin();
}

Transaction::~Transaction() {
    if (m_state == State::Running) {
        m_document.error() << "Cancelling unfinished transaction with name '" << m_name << "' - please report this on github!";
        cancel();
    }
}
