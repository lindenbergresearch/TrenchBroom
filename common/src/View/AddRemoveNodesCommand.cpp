...
AddRemoveNodesCommand::AddRemoveNodesCommand(const Action action, const std::map<Model::Node *, std::vector<Model::Node *>> &nodes) : UpdateLinkedGroupsCommandBase{makeName(action), true}, m_action{action} {
    switch (m_action) {
        case Action::Add:
            m_nodesToAdd = nodes;
            break;
