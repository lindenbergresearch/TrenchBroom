...
AddRemoveNodesCommand::AddRemoveNodesCommand(const Action action, const std::map<Model::Node *, std::vector<Model::Node *>> &nodes) :
  ...
  switch (m_action) {
  case Action::Add:m_nodesToAdd = nodes;
    break;
  case Action::Remove:m_nodesToRemove = nodes;
    break;
    switchDefault();
  }
}
...
std::string AddRemoveNodesCommand::makeName(const Action action) {
  switch (action) {
  case Action::Add:return "Add Objects";
  case Action::Remove:return "Remove Objects";
    switchDefault();
  }
}
...
void AddRemoveNodesCommand::doAction(MapDocumentCommandFacade *document) {
  switch (m_action) {
  case Action::Add:document->performAddNodes(m_nodesToAdd);
    break;
  case Action::Remove:document->performRemoveNodes(m_nodesToRemove);
    break;
  }
...
void AddRemoveNodesCommand::undoAction(MapDocumentCommandFacade *document) {
  switch (m_action) {
  case Action::Add:document->performRemoveNodes(m_nodesToRemove);
    break;
  case Action::Remove:document->performAddNodes(m_nodesToAdd);
    break;
  }
