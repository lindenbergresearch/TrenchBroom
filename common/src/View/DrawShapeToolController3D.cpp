...
namespace View {
DrawShapeToolController3D::DrawShapeToolController3D(DrawShapeTool &tool, std::weak_ptr<MapDocument> document)
...
class CreateSimpleBrushDragDelegate : public HandleDragTrackerDelegate {
  ...
  public:
    CreateSimpleBrushDragDelegate(DrawShapeTool &tool, const vm::bbox3 &worldBounds) : m_tool{tool}, m_worldBounds{worldBounds} {
    }
    ...
    HandlePositionProposer start(const InputState &inputState, const vm::vec3 &initialHandlePosition, const vm::vec3 &handleOffset) {
        ...
        return makeHandlePositionProposer(makePlaneHandlePicker(vm::horizontal_plane(initialHandlePosition), handleOffset), makeIdentityHandleSnapper());
    }
    ...
    std::optional<UpdateDragConfig> modifierKeyChange(const InputState &inputState, const DragState &dragState) {
        if (inputState.modifierKeys() == ModifierKeys::MKAlt) {
            return UpdateDragConfig{
                makeHandlePositionProposer(makeLineHandlePicker(vm::line3{dragState.currentHandlePosition, vm::vec3::pos_z()}, dragState.handleOffset), makeIdentityHandleSnapper()), ResetInitialHandlePosition::Keep
            };
        }

        return UpdateDragConfig{
            makeHandlePositionProposer(makePlaneHandlePicker(vm::horizontal_plane(dragState.currentHandlePosition), dragState.handleOffset), makeIdentityHandleSnapper()), ResetInitialHandlePosition::Keep
        };
    }
  ...
  private:
    ...
    vm::bbox3 makeBounds(const InputState &inputState, const vm::vec3 &initialHandlePosition, const vm::vec3 &currentHandlePosition) const {
        const auto bounds = vm::bbox3{
            vm::min(initialHandlePosition, currentHandlePosition), vm::max(initialHandlePosition, currentHandlePosition)
        };
...
std::unique_ptr<DragTracker> DrawShapeToolController3D::acceptMouseDrag(const InputState &inputState) {
    ...
    return createHandleDragTracker(CreateSimpleBrushDragDelegate{m_tool, document->worldBounds()}, inputState, initialHandlePosition, initialHandlePosition);
}
