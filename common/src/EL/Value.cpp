...
Value::Value(const BooleanType value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(value)}, m_expression{std::move(expression)} {
}

Value::Value(StringType value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(std::move(value))}, m_expression{std::move(expression)} {
}

Value::Value(const char *value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(StringType(value))}, m_expression{std::move(expression)} {
}

Value::Value(const NumberType value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(value)}, m_expression{std::move(expression)} {
}

Value::Value(const int value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(static_cast<NumberType>(value))}, m_expression{std::move(expression)} {
}

Value::Value(const long value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(static_cast<NumberType>(value))}, m_expression{std::move(expression)} {
}

Value::Value(const size_t value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(static_cast<NumberType>(value))}, m_expression{std::move(expression)} {
}

Value::Value(ArrayType value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(std::move(value))}, m_expression{std::move(expression)} {
}

Value::Value(MapType value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(std::move(value))}, m_expression{std::move(expression)} {
}

Value::Value(RangeType value, std::optional<Expression> expression) : m_value{std::make_shared<VariantType>(std::move(value))}, m_expression{std::move(expression)} {
}
...
const BooleanType &Value::booleanValue() const {
    return std::visit(
        kdl::overload(
            [&](const BooleanType &b) -> const BooleanType & { return b; }, [&](const StringType &) -> const BooleanType & { throw DereferenceError{describe(), type(), ValueType::String}; }, [&](const NumberType &) -> const BooleanType & { throw DereferenceError{describe(), type(), ValueType::Number}; }, [&](const ArrayType &) -> const BooleanType & { throw DereferenceError{describe(), type(), ValueType::Array}; }, [&](const MapType &) -> const BooleanType & { throw DereferenceError{describe(), type(), ValueType::Map}; }, [&](const RangeType &) -> const BooleanType & { throw DereferenceError{describe(), type(), ValueType::Range}; }, [&](const NullType &) -> const BooleanType & {
            static const BooleanType b = false;
            return b; }, [&](const UndefinedType &) -> const BooleanType & { throw DereferenceError{describe(), type(), ValueType::Undefined}; }),
        *m_value);
}
...
const StringType &Value::stringValue() const {
    return std::visit(
        kdl::overload(
            ...
            [&](const StringType &s) -> const StringType & { return s; }, [&](const NumberType &) -> const StringType & { throw DereferenceError{describe(), type(), ValueType::Number}; }, [&](const ArrayType &) -> const StringType & { throw DereferenceError{describe(), type(), ValueType::Array}; }, [&](const MapType &) -> const StringType & { throw DereferenceError{describe(), type(), ValueType::Map}; }, [&](const RangeType &) -> const StringType & { throw DereferenceError{describe(), type(), ValueType::Range}; }, [&](const NullType &) -> const StringType & {
            static const StringType s;
            return s; }, [&](const UndefinedType &) -> const StringType & { throw DereferenceError{describe(), type(), ValueType::Undefined}; }),
        *m_value);
}
...
const NumberType &Value::numberValue() const {
    return std::visit(
        kdl::overload(
            ...
            [&](const NumberType &n) -> const NumberType & { return n; }, [&](const ArrayType &) -> const NumberType & { throw DereferenceError{describe(), type(), ValueType::Array}; }, [&](const MapType &) -> const NumberType & { throw DereferenceError{describe(), type(), ValueType::Map}; }, [&](const RangeType &) -> const NumberType & { throw DereferenceError{describe(), type(), ValueType::Range}; }, [&](const NullType &) -> const NumberType & {
            static const NumberType n = 0.0;
            return n; }, [&](const UndefinedType &) -> const NumberType & { throw DereferenceError{describe(), type(), ValueType::Undefined}; }),
        *m_value);
}
...
const ArrayType &Value::arrayValue() const {
    return std::visit(
        kdl::overload(
            ...
            [&](const ArrayType &a) -> const ArrayType & { return a; }, [&](const MapType &) -> const ArrayType & { throw DereferenceError{describe(), type(), ValueType::Map}; }, [&](const RangeType &) -> const ArrayType & { throw DereferenceError{describe(), type(), ValueType::Range}; }, [&](const NullType &) -> const ArrayType & {
            static const ArrayType a(0);
            return a; }, [&](const UndefinedType &) -> const ArrayType & { throw DereferenceError{describe(), type(), ValueType::Undefined}; }),
        *m_value);
}
...
const MapType &Value::mapValue() const {
    return std::visit(
        kdl::overload(
            ...
            [&](const MapType &m) -> const MapType & { return m; }, [&](const RangeType &) -> const MapType & { throw DereferenceError{describe(), type(), ValueType::Range}; }, [&](const NullType &) -> const MapType & {
            static const MapType m;
            return m; }, [&](const UndefinedType &) -> const MapType & { throw DereferenceError{describe(), type(), ValueType::Undefined}; }),
        *m_value);
}
...
const RangeType &Value::rangeValue() const {
    return std::visit(
        kdl::overload(
            ...
            [&](const RangeType &r) -> const RangeType & { return r; }, [&](const NullType &) -> const RangeType & { throw DereferenceError{describe(), type(), ValueType::Null}; }, [&](const UndefinedType &) -> const RangeType & { throw DereferenceError{describe(), type(), ValueType::Undefined}; }),
        *m_value);
}
...
void Value::appendToStream(std::ostream &str, const bool multiline, const std::string &indent) const {
    std::visit(
        kdl::overload(
            ...
            str << "\"" << kdl::str_escape(s, "\\\"") << "\""; }, [&](const NumberType &n) {
            ...
            str << "]"; }, [&](const NullType &) { str << "null"; }, [&](const UndefinedType &) { str << "undefined"; }),
        *m_value);
}
...
bool operator==(const Value &lhs, const Value &rhs) {
    return std::visit(
        kdl::overload(
            ...
            [](const MapType &lhsMap, const MapType &rhsMap) { return lhsMap == rhsMap; }, [](const RangeType &lhsRange, const RangeType &rhsRange) { return lhsRange == rhsRange; }, [](const NullType &, const NullType &) { return true; }, [](const UndefinedType &, const UndefinedType &) { return true; },
