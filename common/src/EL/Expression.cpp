...
namespace EL {
Expression::Expression(std::unique_ptr<ExpressionImpl> expression, const size_t line, const size_t column)
...
Expression::Expression(ArrayExpression expression, const size_t line, const size_t column)
    : m_expression{std::make_shared<ArrayExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}

Expression::Expression(MapExpression expression, const size_t line, const size_t column)
    : m_expression{std::make_shared<MapExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}

Expression::Expression(UnaryExpression expression, const size_t line, const size_t column)
    : m_expression{std::make_shared<UnaryExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}
...
void Expression::rebalanceByPrecedence() {
    ...
    if (parentPrecedence > std::min(leftPrecedence, rightPrecedence)) {
        if (leftPrecedence < rightPrecedence) {
            ...
            dynamic_cast<BinaryExpression *>(m_expression.get())->m_leftOperand = std::move(dynamic_cast<BinaryExpression *>(leftExpression.m_expression.get())->m_rightOperand);
            ...
            *this = std::move(leftExpression);
            ...
            dynamic_cast<BinaryExpression *>(m_expression.get())->m_rightOperand = std::move(dynamic_cast<BinaryExpression *>(rightExpression.m_expression.get())->m_leftOperand);
