...
namespace EL {
Expression::Expression(std::unique_ptr<ExpressionImpl> expression, const size_t line, const size_t column) : m_expression{std::move(expression)}, m_line{line}, m_column{column} {
}

Expression::Expression(LiteralExpression expression, const size_t line, const size_t column) : m_expression{std::make_shared<LiteralExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}

Expression::Expression(VariableExpression expression, const size_t line, const size_t column) : m_expression{std::make_shared<VariableExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}
...
Expression::Expression(BinaryExpression expression, const size_t line, const size_t column) : m_expression{std::make_shared<BinaryExpression>(std::move(expression))}, m_line{line}, m_column{column} {
    rebalanceByPrecedence();
}

Expression::Expression(SubscriptExpression expression, const size_t line, const size_t column) : m_expression{std::make_shared<SubscriptExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}

Expression::Expression(SwitchExpression expression, const size_t line, const size_t column) : m_expression{std::make_shared<SwitchExpression>(std::move(expression))}, m_line{line}, m_column{column} {
}
...
void Expression::rebalanceByPrecedence() {
   ...
   */
    ...
    const auto leftPrecedence = dynamic_cast<BinaryExpression *>(m_expression.get())->m_leftOperand.precedence();
    const auto rightPrecedence = dynamic_cast<BinaryExpression *>(m_expression.get())->m_rightOperand.precedence();
    ...
    if (parentPrecedence > std::min(leftPrecedence, rightPrecedence)) {
        if (leftPrecedence < rightPrecedence) {
            ...
            Expression leftExpression = std::move(dynamic_cast<BinaryExpression *>(m_expression.get())->m_leftOperand);
            ...
            dynamic_cast<BinaryExpression *>(leftExpression.m_expression.get())->m_rightOperand = std::move(*this);
            *this = std::move(leftExpression);
            ...
            Expression rightExpression = std::move(dynamic_cast<BinaryExpression *>(m_expression.get())->m_rightOperand);
            ...
            dynamic_cast<BinaryExpression *>(rightExpression.m_expression.get())->m_leftOperand = std::move(*this);
