...
LiteralExpression::LiteralExpression(Value value) : m_value{std::move(value)} {
}
...
VariableExpression::VariableExpression(std::string variableName) : m_variableName{std::move(variableName)} {
}
...
ArrayExpression::ArrayExpression(std::vector<Expression> elements) : m_elements{std::move(elements)} {
}
...
MapExpression::MapExpression(std::map<std::string, Expression> elements) : m_elements{std::move(elements)} {
}
...
static Value evaluateUnaryPlus(const Value &v) {
    ...
    case ValueType::Number:return Value{v.convertTo(ValueType::Number).numberValue()};
...
static Value evaluateLogicalNegation(const Value &v) {
    switch (v.type()) {
    case ValueType::Boolean:return Value{!v.booleanValue()};
    ...
    throw EvaluationError{
        "Cannot apply logical negation to value '" + v.describe() + "' of type '" + v.typeName()
    };
}

static Value evaluateBitwiseNegation(const Value &v) {
    switch (v.type()) {
    case ValueType::Number:return Value{~v.integerValue()};
    case ValueType::String:
        if (const auto result = v.tryConvertTo(ValueType::Number)) {
    ...
    throw EvaluationError{
        "Cannot apply bitwise negation to value '" + v.describe() + "' of type '" + v.typeName()
    };
}
...
static Value evaluateUnaryExpression(const UnaryOperator &operator_, const Value &operand) {
    ...
    switch (operator_) {
    case UnaryOperator::Plus:return evaluateUnaryPlus(operand);
    case UnaryOperator::Minus:return evaluateUnaryMinus(operand);
    case UnaryOperator::LogicalNegation:return evaluateLogicalNegation(operand);
    case UnaryOperator::BitwiseNegation:return evaluateBitwiseNegation(operand);
    case UnaryOperator::Group:return Value{operand};
        switchDefault();
    }
}
...
void UnaryExpression::appendToStream(std::ostream &str) const {
    switch (m_operator) {
    case UnaryOperator::Plus:str << "+" << m_operand;
        break;
    case UnaryOperator::Minus:str << "-" << m_operand;
        break;
    case UnaryOperator::LogicalNegation:str << "!" << m_operand;
        break;
    case UnaryOperator::BitwiseNegation:str << "~" << m_operand;
        break;
    case UnaryOperator::Group:str << "( " << m_operand << " )";
        break;
        switchDefault();
    }
}
...
template<typename Eval> static std::optional<Value> tryEvaluateAlgebraicOperator(const Value &lhs, const Value &rhs, const Eval &eval) {
    if (lhs.hasType(ValueType::Undefined) || rhs.hasType(ValueType::Undefined)) {
        return Value::Undefined;
    }

    if (lhs.hasType(ValueType::Boolean, ValueType::Number) && rhs.hasType(ValueType::Boolean, ValueType::Number)) {
        return Value{
            eval(lhs.convertTo(ValueType::Number), rhs.convertTo(ValueType::Number))
        };
    }

    if (lhs.hasType(ValueType::Boolean, ValueType::Number) && rhs.hasType(ValueType::String)) {
        if (const auto rhsAsNumber = rhs.tryConvertTo(ValueType::Number)) {
            return Value{eval(lhs.convertTo(ValueType::Number), *rhsAsNumber)};
        }
    }

    if (lhs.hasType(ValueType::String) && rhs.hasType(ValueType::Boolean, ValueType::Number)) {
...
static Value evaluateAddition(const Value &lhs, const Value &rhs) {
    ...
    if (lhs.hasType(ValueType::String) && rhs.hasType(ValueType::String)) {
        return Value{
            lhs.convertTo(ValueType::String).stringValue() + rhs.convertTo(ValueType::String).stringValue()
        };
    }
...
static Value evaluateModulus(const Value &lhs, const Value &rhs) {
    ...
    throw EvaluationError{
        "Cannot apply operator % to '" + lhs.describe() + "' of type '" + typeName(lhs.type()) + " and '" + rhs.describe() + "' of type '" + typeName(rhs.type()) + "'"
    };
}

template<typename EvaluateLhs, typename EvaluateRhs> static Value evaluateLogicalAnd(const EvaluateLhs &evaluateLhs, const EvaluateRhs &evaluateRhs) {
...
template<typename EvaluateLhs, typename EvaluateRhs> static Value evaluateLogicalOr(const EvaluateLhs &evaluateLhs, const EvaluateRhs &evaluateRhs) {
    const auto lhs = evaluateLhs();
    auto rhs = std::make_optional<Value>();
...
template<typename Eval> static std::optional<Value> tryEvaluateBitwiseOperator(const Value &lhs, const Value &rhs, const Eval &eval) {
    if (lhs.hasType(ValueType::Undefined) || rhs.hasType(ValueType::Undefined)) {
        return Value::Undefined;
    }
...
static int evaluateCompare(const Value &lhs, const Value &rhs) {
    try {
        ...
        case ValueType::Boolean:
            ...
            case ValueType::Boolean:
            case ValueType::Number:
            case ValueType::String:return compareAsBooleans(lhs, rhs);
            case ValueType::Null:
            case ValueType::Undefined:return 1;
            case ValueType::Array:
            case ValueType::Map:
            case ValueType::Range:break;
            }
            break;
        case ValueType::Number:
            switch (rhs.type()) {
            case ValueType::Boolean:return compareAsBooleans(lhs, rhs);
            case ValueType::Number:
            case ValueType::String:return compareAsNumbers(lhs, rhs);
            case ValueType::Null:
            case ValueType::Undefined:return 1;
            case ValueType::Array:
            case ValueType::Map:
        ...
        case ValueType::String:
            switch (rhs.type()) {
            case ValueType::Boolean:return compareAsBooleans(lhs, rhs);
            case ValueType::Number:return compareAsNumbers(lhs, rhs);
            case ValueType::String:return lhs.stringValue().compare(rhs.convertTo(ValueType::String).stringValue());
            case ValueType::Null:
            case ValueType::Undefined:return 1;
            case ValueType::Array:
            case ValueType::Map:
        ...
        case ValueType::Range:
            switch (rhs.type()) {
            case ValueType::Range:return kdl::col_lexicographical_compare(lhs.rangeValue(), rhs.rangeValue());
            case ValueType::Null:
            case ValueType::Undefined:return 1;
            case ValueType::Boolean:
            case ValueType::Number:
        ...
        throw EvaluationError{
            "Cannot compare value '" + lhs.describe() + "' of type '" + typeName(lhs.type()) + " to value '" + rhs.describe() + "' of type '" + typeName(rhs.type()) + "'"
        };
    } catch (const ConversionError &c) {
        throw EvaluationError{
            "Cannot compare value '" + lhs.describe() + "' of type '" + typeName(lhs.type()) + " to value '" + rhs.describe() + "' of type '" + typeName(rhs.type()) + "': " + c.what()
        };
    }
}
...
template<typename EvaluateLhs, typename EvaluateRhs> static Value evaluateCase(const EvaluateLhs &evaluateLhs, const EvaluateRhs &evaluateRhs) {
    const auto lhs = evaluateLhs();
...
template<typename EvalualateLhs, typename EvaluateRhs> static Value evaluateBinaryExpression(const BinaryOperator operator_, const EvalualateLhs &evaluateLhs, const EvaluateRhs &evaluateRhs) {
    switch (operator_) {
    case BinaryOperator::Addition:return evaluateAddition(evaluateLhs(), evaluateRhs());
    case BinaryOperator::Subtraction:return evaluateSubtraction(evaluateLhs(), evaluateRhs());
    case BinaryOperator::Multiplication:return evaluateMultiplication(evaluateLhs(), evaluateRhs());
    case BinaryOperator::Division:return evaluateDivision(evaluateLhs(), evaluateRhs());
    case BinaryOperator::Modulus:return evaluateModulus(evaluateLhs(), evaluateRhs());
    case BinaryOperator::LogicalAnd:return evaluateLogicalAnd(evaluateLhs, evaluateRhs);
    case BinaryOperator::LogicalOr:return evaluateLogicalOr(evaluateLhs, evaluateRhs);
    case BinaryOperator::BitwiseAnd:return evaluateBitwiseAnd(evaluateLhs(), evaluateRhs());
    case BinaryOperator::BitwiseXOr:return evaluateBitwiseXOr(evaluateLhs(), evaluateRhs());
    case BinaryOperator::BitwiseOr:return evaluateBitwiseOr(evaluateLhs(), evaluateRhs());
    case BinaryOperator::BitwiseShiftLeft:return evaluateBitwiseShiftLeft(evaluateLhs(), evaluateRhs());
    case BinaryOperator::BitwiseShiftRight:return evaluateBitwiseShiftRight(evaluateLhs(), evaluateRhs());
    case BinaryOperator::Less:return Value{evaluateCompare(evaluateLhs(), evaluateRhs()) < 0};
    case BinaryOperator::LessOrEqual:return Value{evaluateCompare(evaluateLhs(), evaluateRhs()) <= 0};
    case BinaryOperator::Greater:return Value{evaluateCompare(evaluateLhs(), evaluateRhs()) > 0};
    case BinaryOperator::GreaterOrEqual:return Value{evaluateCompare(evaluateLhs(), evaluateRhs()) >= 0};
    case BinaryOperator::Equal:return Value{evaluateCompare(evaluateLhs(), evaluateRhs()) == 0};
    case BinaryOperator::NotEqual:return Value{evaluateCompare(evaluateLhs(), evaluateRhs()) != 0};
    case BinaryOperator::Range:return Value{evaluateRange(evaluateLhs(), evaluateRhs())};
    case BinaryOperator::Case:return evaluateCase(evaluateLhs, evaluateRhs);
        switchDefault();
    };
}
...
std::unique_ptr<ExpressionImpl> BinaryExpression::optimize() const {
    ...
    return std::make_unique<BinaryExpression>(m_operator, std::move(optimizedLeftOperand).value_or(m_leftOperand.optimize()), std::move(optimizedRightOperand).value_or(m_rightOperand.optimize()));
}
...
size_t BinaryExpression::precedence() const {
    ...
    case BinaryOperator::Modulus:return 12;
    case BinaryOperator::Addition:
    case BinaryOperator::Subtraction:return 11;
    case BinaryOperator::BitwiseShiftLeft:
    case BinaryOperator::BitwiseShiftRight:return 10;
    case BinaryOperator::Less:
    case BinaryOperator::LessOrEqual:
    case BinaryOperator::Greater:
    case BinaryOperator::GreaterOrEqual:return 9;
    case BinaryOperator::Equal:
    case BinaryOperator::NotEqual:return 8;
    case BinaryOperator::BitwiseAnd:return 7;
    case BinaryOperator::BitwiseXOr:return 6;
    case BinaryOperator::BitwiseOr:return 5;
    case BinaryOperator::LogicalAnd:return 4;
    case BinaryOperator::LogicalOr:return 3;
    case BinaryOperator::Range:return 2;
    case BinaryOperator::Case:return 1;
        switchDefault();
    };
}
...
bool BinaryExpression::operator==(const BinaryExpression &rhs) const {
    return m_operator == rhs.m_operator && m_leftOperand == rhs.m_leftOperand && m_rightOperand == rhs.m_rightOperand;
}

void BinaryExpression::appendToStream(std::ostream &str) const {
    switch (m_operator) {
    case BinaryOperator::Addition:str << m_leftOperand << " + " << m_rightOperand;
        break;
    case BinaryOperator::Subtraction:str << m_leftOperand << " - " << m_rightOperand;
        break;
    case BinaryOperator::Multiplication:str << m_leftOperand << " * " << m_rightOperand;
        break;
    case BinaryOperator::Division:str << m_leftOperand << " / " << m_rightOperand;
        break;
    case BinaryOperator::Modulus:str << m_leftOperand << " % " << m_rightOperand;
        break;
    case BinaryOperator::LogicalAnd:str << m_leftOperand << " && " << m_rightOperand;
        break;
    case BinaryOperator::LogicalOr:str << m_leftOperand << " || " << m_rightOperand;
        break;
    case BinaryOperator::BitwiseAnd:str << m_leftOperand << " & " << m_rightOperand;
        break;
    case BinaryOperator::BitwiseXOr:str << m_leftOperand << " ^ " << m_rightOperand;
        break;
    case BinaryOperator::BitwiseOr:str << m_leftOperand << " | " << m_rightOperand;
        break;
    case BinaryOperator::BitwiseShiftLeft:str << m_leftOperand << " << " << m_rightOperand;
        break;
    case BinaryOperator::BitwiseShiftRight:str << m_leftOperand << " >> " << m_rightOperand;
        break;
    case BinaryOperator::Less:str << m_leftOperand << " < " << m_rightOperand;
        break;
    case BinaryOperator::LessOrEqual:str << m_leftOperand << " <= " << m_rightOperand;
        break;
    case BinaryOperator::Greater:str << m_leftOperand << " > " << m_rightOperand;
        break;
    case BinaryOperator::GreaterOrEqual:str << m_leftOperand << " >= " << m_rightOperand;
        break;
    case BinaryOperator::Equal:str << m_leftOperand << " == " << m_rightOperand;
        break;
    case BinaryOperator::NotEqual:str << m_leftOperand << " != " << m_rightOperand;
        break;
    case BinaryOperator::Range:str << m_leftOperand << ".." << m_rightOperand;
        break;
    case BinaryOperator::Case:str << m_leftOperand << " -> " << m_rightOperand;
        break;
        switchDefault();
    };
}
...
SwitchExpression::SwitchExpression(std::vector<Expression> cases) : m_cases{std::move(cases)} {
}
