...
OrthographicCamera::OrthographicCamera(const float nearPlane, const float farPlane, const Viewport &viewport, const vm::vec3f &position,
...
void OrthographicCamera::doUpdateZoom() {
    ...
    m_zoomedViewport = Viewport(unzoomedViewport.x, unzoomedViewport.y, static_cast<int>(vm::round(static_cast<float>(unzoomedViewport.width) / zoom())),
