...
void OrthographicCamera::doUpdateZoom() {
    ...
    m_zoomedViewport = Viewport(unzoomedViewport.x, unzoomedViewport.y, static_cast<int>(vm::round(static_cast<float>(unzoomedViewport.width) / zoom())), static_cast<int>(vm::round(static_cast<float>(unzoomedViewport.height) / zoom())));
}
} // namespace Renderer
} // namespace TrenchBroom
