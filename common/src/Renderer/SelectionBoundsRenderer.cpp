...
public:
    SizeTextAnchor2D(const vm::bbox3 &bounds, const vm::axis::type axis, const Renderer::Camera &camera) : m_bounds(
        bounds
    ), m_axis(axis), m_camera(camera) {
    }
...
public:
    SizeTextAnchor3D(const vm::bbox3 &bounds, const vm::axis::type axis, const Renderer::Camera &camera) : m_bounds(
        bounds
    ), m_axis(axis), m_camera(camera) {
    }
...
void SelectionBoundsRenderer::renderMinMax(RenderContext &renderContext, RenderBatch &renderBatch) {
    ...
    renderService.renderString(buffer.str(),
