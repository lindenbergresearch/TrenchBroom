...
EntityLinkRenderer::EntityLinkRenderer(std::weak_ptr<View::MapDocument> document)
    : m_document(document), m_defaultColor(0.5f, 1.0f, 0.5f, 1.0f), m_selectedColor(1.0f, 0.0f, 0.0f, 1.0f) {
}
    ...
    void addSources(
        ...
        for (const Model::EntityNodeBase *source: sources) {
            if (
                ...
                && m_editorContext.visible(source))
                addLink(source, target);
        }
    }
...
static void collectSelectedLinks(
    ...
    for (auto *node: selectedNodes) {
        node->accept(kdl::overload(
            ...
            [](Model::LayerNode *) {},
            [](Model::GroupNode *) {},
            [&](Model::EntityNode *entity) { collectLinks.visit(entity); },
