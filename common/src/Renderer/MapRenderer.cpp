...
class SelectedBrushRendererFilter : public BrushRenderer::DefaultFilter {
  public:
    explicit SelectedBrushRendererFilter(const Model::EditorContext &context) : DefaultFilter{context} {
    }
...
class LockedBrushRendererFilter : public BrushRenderer::DefaultFilter {
  public:
    explicit LockedBrushRendererFilter(const Model::EditorContext &context) : DefaultFilter{context} {
    }
...
class UnselectedBrushRendererFilter : public BrushRenderer::DefaultFilter {
  public:
    explicit UnselectedBrushRendererFilter(const Model::EditorContext &context) : DefaultFilter{context} {
    }
    ...
    RenderSettings markFaces(const Model::BrushNode &brushNode) const override {
        ...
        return {
            renderFaces ? FaceRenderPolicy::RenderMarked : FaceRenderPolicy::RenderNone, renderEdges ? EdgeRenderPolicy::RenderAll : EdgeRenderPolicy::RenderNone
        };
    }
};
} // namespace
...
MapRenderer::MapRenderer(std::weak_ptr<View::MapDocument> document)
    : m_document{std::move(document)}, m_defaultRenderer{createDefaultRenderer(m_document)}, m_selectionRenderer{createSelectionRenderer(m_document)}, m_lockedRenderer{createLockRenderer(m_document)}, m_entityDecalRenderer{createEntityDecalRenderer(m_document)}, m_entityLinkRenderer{std::make_unique<EntityLinkRenderer>(m_document)}, m_groupLinkRenderer{std::make_unique<GroupLinkRenderer>(m_document)} {
    connectObservers();
    setupRenderers();
}
...
std::unique_ptr<ObjectRenderer> MapRenderer::createDefaultRenderer(std::weak_ptr<View::MapDocument> document) {
    return std::make_unique<ObjectRenderer>(*kdl::mem_lock(document), kdl::mem_lock(document)->entityModelManager(), kdl::mem_lock(document)->editorContext(), UnselectedBrushRendererFilter{kdl::mem_lock(document)->editorContext()});
}

std::unique_ptr<ObjectRenderer> MapRenderer::createSelectionRenderer(std::weak_ptr<View::MapDocument> document) {
    return std::make_unique<ObjectRenderer>(*kdl::mem_lock(document), kdl::mem_lock(document)->entityModelManager(), kdl::mem_lock(document)->editorContext(), SelectedBrushRendererFilter{kdl::mem_lock(document)->editorContext()});
}

std::unique_ptr<ObjectRenderer> MapRenderer::createLockRenderer(std::weak_ptr<View::MapDocument> document) {
    return std::make_unique<ObjectRenderer>(*kdl::mem_lock(document), kdl::mem_lock(document)->entityModelManager(), kdl::mem_lock(document)->editorContext(), LockedBrushRendererFilter{kdl::mem_lock(document)->editorContext()});
}

std::unique_ptr<EntityDecalRenderer> MapRenderer::createEntityDecalRenderer(std::weak_ptr<View::MapDocument> document) {
...
void MapRenderer::renderEntityDecals(RenderContext &renderContext, RenderBatch &renderBatch) {
...
void MapRenderer::updateAndInvalidateNodeRecursive(Model::Node *node) {
    node->accept(kdl::overload([](auto &&thisLambda, Model::WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, Model::LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, Model::GroupNode *group) {
