...
MapRenderer::MapRenderer(std::weak_ptr<View::MapDocument> document) :
    m_document{std::move(document)}, m_defaultRenderer{createDefaultRenderer(m_document)}, m_selectionRenderer{createSelectionRenderer(m_document)},
    m_lockedRenderer{
        createLockRenderer(m_document)
    }, m_entityDecalRenderer{createEntityDecalRenderer(m_document)}, m_entityLinkRenderer{std::make_unique<EntityLinkRenderer>(m_document)},
    m_groupLinkRenderer{
...
std::unique_ptr<ObjectRenderer> MapRenderer::createDefaultRenderer(std::weak_ptr<View::MapDocument> document) {
  return std::make_unique<ObjectRenderer>(
      *kdl::mem_lock(document), kdl::mem_lock(document)->entityModelManager(), kdl::mem_lock(document)->editorContext(),
