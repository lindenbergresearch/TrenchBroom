...
namespace detail {
namespace {
[[maybe_unused]] bool is_valid_address(const int16_t x, const int16_t y, const int16_t z, const uint16_t size) {
...
node_address::node_address(const int16_t i_x, const int16_t i_y, const int16_t i_z, const uint16_t i_size) : x{i_x}, y{i_y}, z{i_z}, size{i_size} {
...
node_address get_parent(const node_address &a) {
    ...
    return {
        int16_t(((a.x >= 0 ? a.x : a.x - p_s + 1) / p_s) * p_s), int16_t(((a.y >= 0 ? a.y : a.y - p_s + 1) / p_s) * p_s), int16_t(((a.z >= 0 ? a.z : a.z - p_s + 1) / p_s) * p_s), uint16_t(a.size + 1)
    };
}
...
node_address get_child(const node_address &a, const size_t quadrant) {
    ...
    return {
        int16_t(a.x + ((quadrant & 1) ? (1 << (a.size - 1)) : 0)), int16_t(a.y + ((quadrant & 2) ? (1 << (a.size - 1)) : 0)), int16_t(a.z + ((quadrant & 4) ? (1 << (a.size - 1)) : 0)), uint16_t(a.size - 1)
    };
}
