...
template<typename T, typename U> class octree {
  public:
    ...
    struct inner_node {
      ...
      inner_node(const detail::node_address i_address, std::vector<U> i_data) : inner_node{
          i_address, std::move(i_data), kdl::vec_from(node{leaf_node{get_child(i_address, 0), {}}}, node{leaf_node{get_child(i_address, 1), {}}}, node{leaf_node{get_child(i_address, 2), {}}}, node{leaf_node{get_child(i_address, 3), {}}}, node{leaf_node{get_child(i_address, 4), {}}}, node{leaf_node{get_child(i_address, 5), {}}}, node{leaf_node{get_child(i_address, 6), {}}}, node{leaf_node{get_child(i_address, 7), {}}})
      } {
      }
  ...
  private:
    ...
    static bool is_empty(const node &node) {
        ...
        }, [](const leaf_node &l) { return l.data.empty(); }), node);
    }
