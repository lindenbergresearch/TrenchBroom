...
EntityModelFrame::EntityModelFrame(const size_t index) : m_index{index}, m_skinOffset{0} {
}
...
) : EntityModelFrame{index}, m_name{name}, m_bounds{
    bounds
}, m_pitchType{pitchType}, m_orientation{orientation}, m_spacialTree{std::make_unique<SpacialTree>(16.0f)} {
}
...
void EntityModelLoadedFrame::addToSpacialTree(
  ...
  case Renderer::PrimType::LineLoop:break;
...
public:
   ...
   */
  explicit EntityModelUnloadedFrame(const size_t index) : EntityModelFrame{index} {
  }
...
protected:
   ...
   */
  explicit EntityModelMesh(std::vector<EntityModelVertex> vertices) : m_vertices{std::move(vertices)} {
  }
...
EntityModelSurface::EntityModelSurface(std::string name, const size_t frameCount) :
    m_name{std::move(name)}, m_meshes{frameCount}, m_skins{std::make_unique<TextureCollection>()} {
}
...
std::unique_ptr<Renderer::TexturedRenderer> EntityModel::buildRenderer(const size_t skinIndex, const size_t frameIndex) const {
  ...
  for (const auto &surface : m_surfaces) {
    ...
    const auto correctedSkinIndex = actualSkinIndex < surface->skinCount() ? actualSkinIndex : 0;
