...
const PropertyDefinition *EntityDefinition::safeGetPropertyDefinition(const EntityDefinition *entityDefinition, const std::string &propertyKey) {
    ...
    const auto it = std::find_if(propertyDefinitions.begin(), propertyDefinitions.end(), [&](const auto &propertyDefinition) {
...
std::vector<EntityDefinition *> EntityDefinition::filterAndSort(const std::vector<EntityDefinition *> &definitions, const EntityDefinitionType type, const EntityDefinitionSortOrder order) {
    auto result = kdl::vec_filter(definitions, [&](const auto &definition) { return definition->type() == type; });
    ...
    if (order == EntityDefinitionSortOrder::Usage) {
        return kdl::vec_sort(std::move(result), kdl::combine_cmp(compareUsageCount, compareNames));
    }
    return kdl::vec_sort(std::move(result), kdl::combine_cmp(compareNames, compareUsageCount));
}

EntityDefinition::EntityDefinition(std::string name, const Color &color, std::string description, std::vector<std::shared_ptr<PropertyDefinition>> propertyDefinitions)
    : m_index{0}, m_name{std::move(name)}, m_color{color}, m_description{std::move(description)}, m_usageCount{0}, m_propertyDefinitions{std::move(propertyDefinitions)} {
}

PointEntityDefinition::PointEntityDefinition(std::string name, const Color &color, const vm::bbox3 &bounds, std::string description, std::vector<std::shared_ptr<PropertyDefinition>> propertyDefinitions, ModelDefinition modelDefinition, DecalDefinition decalDefinition)
    : EntityDefinition{std::move(name), color, std::move(description), std::move(propertyDefinitions)}, m_bounds{bounds}, m_modelDefinition{std::move(modelDefinition)}, m_decalDefinition{std::move(decalDefinition)} {
}
...
BrushEntityDefinition::BrushEntityDefinition(std::string name, const Color &color, std::string description, std::vector<std::shared_ptr<PropertyDefinition>> propertyDefinitions)
    : EntityDefinition{
