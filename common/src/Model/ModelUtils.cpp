...
LayerNode *findContainingLayer(Node *node) {
    return node->accept(kdl::overload([](WorldNode *) -> LayerNode * { return nullptr; }, [](LayerNode *layer) -> LayerNode * { return layer; }, [](auto &&thisLambda, GroupNode *group) -> LayerNode * {
...
GroupNode *findContainingGroup(Node *node) {
    return node->visitParent(kdl::overload([](WorldNode *) -> GroupNode * { return nullptr; }, [](LayerNode *) -> GroupNode * { return nullptr; }, [](GroupNode *group) -> GroupNode * { return group; }, [](auto &&thisLambda, EntityNode *entity) -> GroupNode * {
        return entity->visitParent(thisLambda).value_or(nullptr);
...
GroupNode *findOutermostClosedGroup(Node *node) {
    return node->visitParent(kdl::overload([](WorldNode *) -> GroupNode * { return nullptr; }, [](LayerNode *) -> GroupNode * { return nullptr; }, [](auto &&thisLambda, GroupNode *group) -> GroupNode * {
 ...
 */
template<typename P> static std::vector<Node *> collectMatchingNodes(const std::vector<Node *> &nodes, const std::vector<BrushNode *> &brushes, const P &predicate) {
    ...
    for (auto *node : nodes) {
        node->accept(kdl::overload([](auto &&thisLambda, WorldNode *world) { world->visitChildren(thisLambda); }, [](auto &&thisLambda, LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, GroupNode *group) {
...
std::vector<Node *> collectSelectedNodes(const std::vector<Node *> &nodes) {
    return collectNodesAndDescendants(nodes, [](const auto *node) { return node->selected(); });
}
...
std::vector<BrushFaceHandle> collectSelectableBrushFaces(const std::vector<Node *> &nodes, const EditorContext &editorContext) {
    return collectBrushFaces(nodes, [&](const BrushNode &brushNode, const BrushFace &brushFace) {
...
vm::bbox3 computeLogicalBounds(const std::vector<Node *> &nodes, const vm::bbox3 &defaultBounds) {
    ...
    Node::visitAll(nodes, kdl::overload([](const WorldNode *) {}, [](const LayerNode *) {}, [&](const GroupNode *group) { builder.add(group->logicalBounds()); }, [&](const EntityNode *entity) { builder.add(entity->logicalBounds()); }, [&](const BrushNode *brush) { builder.add(brush->logicalBounds()); }, [&](const PatchNode *patch) { builder.add(patch->logicalBounds()); }));
...
vm::bbox3 computePhysicalBounds(const std::vector<Node *> &nodes, const vm::bbox3 &defaultBounds) {
    vm::bbox3::builder builder;
    Node::visitAll(nodes, kdl::overload([](const WorldNode *) {}, [](const LayerNode *) {}, [&](const GroupNode *group) { builder.add(group->physicalBounds()); }, [&](const EntityNode *entity) { builder.add(entity->physicalBounds()); }, [&](const BrushNode *brush) { builder.add(brush->physicalBounds()); }, [&](const PatchNode *patch) { builder.add(patch->physicalBounds()); }));
    return builder.initialized() ? builder.bounds() : defaultBounds;
}
...
std::vector<BrushNode *> filterBrushNodes(const std::vector<Node *> &nodes) {
    ...
    for (Node *node : nodes) {
        node->accept(kdl::overload([](WorldNode *) {}, [](LayerNode *) {}, [](GroupNode *) {}, [](EntityNode *) {}, [&](BrushNode *brushNode) { result.push_back(brushNode); }, [](PatchNode *) {}));
    }
    return result;
}
...
std::vector<EntityNode *> filterEntityNodes(const std::vector<Node *> &nodes) {
    ...
    result.reserve(nodes.size());
    for (Node *node : nodes) {
        node->accept(kdl::overload([](WorldNode *) {}, [](LayerNode *) {}, [](GroupNode *) {}, [&](EntityNode *entityNode) { result.push_back(entityNode); }, [](BrushNode *) {}, [](PatchNode *) {}));
    }
    return result;
}
