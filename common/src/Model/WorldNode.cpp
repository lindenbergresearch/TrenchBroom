...
WorldNode::WorldNode(EntityPropertyConfig entityPropertyConfig, Entity entity, const MapFormat mapFormat)
    : m_entityPropertyConfig{std::move(entityPropertyConfig)}, m_mapFormat{mapFormat}, m_defaultLayer{nullptr}, m_entityNodeIndex{std::make_unique<EntityNodeIndex>()}, m_validatorRegistry{std::make_unique<ValidatorRegistry>()}, m_nodeTree{std::make_unique<NodeTree>(256.0)}, m_updateNodeTree{true} {
...
WorldNode::WorldNode(EntityPropertyConfig entityPropertyConfig, std::initializer_list<EntityProperty> properties, const MapFormat mapFormat) : WorldNode{
...
std::vector<LayerNode *> WorldNode::allLayers() {
    ...
    visitChildren(kdl::overload([](WorldNode *) {}, [&](LayerNode *layer) { layers.push_back(layer); }, [](GroupNode *) {}, [](EntityNode *) {}, [](BrushNode *) {}, [](PatchNode *) {}));
    return layers;
}
...
std::vector<LayerNode *> WorldNode::customLayers() {
    ...
    for (auto it = std::next(std::begin(children)); it != std::end(children); ++it) {
        (*it)->accept(kdl::overload([](WorldNode *) {}, [&](LayerNode *layer) { layers.push_back(layer); }, [](GroupNode *) {}, [](EntityNode *) {}, [](BrushNode *) {}, [](PatchNode *) {}));
    }
...
void WorldNode::rebuildNodeTree() {
    ...
    accept(kdl::overload([&](auto &&thisLambda, WorldNode *world) {
        addNode(world);
    ...
    }, [&](BrushNode *brush) { addNode(brush); }, [&](PatchNode *patch) { addNode(patch); }));
...
Node *WorldNode::doClone(const vm::bbox3 & /* worldBounds */, const SetLinkId /* setLinkIds */) const {
...
Node *WorldNode::doCloneRecursively(const vm::bbox3 &worldBounds, const SetLinkId setLinkIds) const {
    ...
    worldNode->defaultLayer()->addChildren(cloneRecursively(worldBounds, m_defaultLayer->children(), setLinkIds));

    if (myChildren.size() > 1) {
        auto childClones = std::vector<Node *>{};
        childClones.reserve(myChildren.size() - 1);
        cloneRecursively(worldBounds, setLinkIds, std::next(std::begin(myChildren)), std::end(myChildren), std::back_inserter(childClones));
...
bool WorldNode::doCanAddChild(const Node *child) const {
    return child->accept(kdl::overload([](const WorldNode *) { return false; }, [](const LayerNode *) { return true; }, [](const GroupNode *) { return false; }, [](const EntityNode *) { return false; }, [](const BrushNode *) { return false; }, [](const PatchNode *) { return false; }));
}

bool WorldNode::doCanRemoveChild(const Node *child) const {
    return child->accept(kdl::overload([](const WorldNode *) { return false; }, [&](const LayerNode *layer) { return (layer != defaultLayer()); }, [](const GroupNode *) { return false; }, [](const EntityNode *) { return false; }, [](const BrushNode *) { return false; }, [](const PatchNode *) { return false; }));
}
...
void WorldNode::doDescendantWasAdded(Node *node, const size_t /* depth */) {
    ...
    if (m_updateNodeTree) {
        node->accept(kdl::overload([&](auto &&thisLambda, WorldNode *world) { world->visitChildren(thisLambda); }, [&](auto &&thisLambda, LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, GroupNode *group) { group->visitChildren(thisLambda); }, [&](auto &&thisLambda, EntityNode *entity) {
            m_nodeTree->insert(entity->physicalBounds(), entity);
            entity->visitChildren(thisLambda);
        }, [&](BrushNode *brush) { m_nodeTree->insert(brush->physicalBounds(), brush); }, [&](PatchNode *patch) { m_nodeTree->insert(patch->physicalBounds(), patch); }));
    }
    ...
    node->accept(kdl::overload([&](auto &&thisLambda, WorldNode *world) { world->visitChildren(thisLambda); }, [&](auto &&thisLambda, LayerNode *layer) {
        layer->visitChildren(thisLambda);
...
void WorldNode::doDescendantWillBeRemoved(Node *node, const size_t /* depth */) {
    if (m_updateNodeTree) {
        ...
        node->accept(kdl::overload([&](auto &&thisLambda, WorldNode *world) { world->visitChildren(thisLambda); }, [&](auto &&thisLambda, LayerNode *layer) { layer->visitChildren(thisLambda); }, [&](auto &&thisLambda, GroupNode *group) { group->visitChildren(thisLambda); }, [&](auto &&thisLambda, EntityNode *entity) {
            doRemove(entity);
            entity->visitChildren(thisLambda);
        }, [&](BrushNode *brush) { doRemove(brush); }, [&](PatchNode *patch) { doRemove(patch); }));
    }
}

void WorldNode::doDescendantPhysicalBoundsDidChange(Node *node) {
    if (m_updateNodeTree) {
        node->accept(kdl::overload([](WorldNode *) {}, [](LayerNode *) {}, [](GroupNode *) {}, [&](EntityNode *entity) { m_nodeTree->update(entity->physicalBounds(), entity); }, [&](BrushNode *brush) { m_nodeTree->update(brush->physicalBounds(), brush); }, [&](PatchNode *patch) { m_nodeTree->update(patch->physicalBounds(), patch); }));
    }
}
...
void WorldNode::doFindEntityNodesWithProperty(const std::string &name, const std::string &value, std::vector<Model::EntityNodeBase *> &result) const {
    result = kdl::vec_concat(std::move(result), m_entityNodeIndex->findEntityNodes(EntityNodeIndexQuery::exact(name), value));
}
...
void WorldNode::doFindEntityNodesWithNumberedProperty(const std::string &prefix, const std::string &value, std::vector<Model::EntityNodeBase *> &result) const {
    result = kdl::vec_concat(std::move(result), m_entityNodeIndex->findEntityNodes(EntityNodeIndexQuery::numbered(prefix), value));
}
