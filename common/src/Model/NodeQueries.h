...
template<typename T = Node *, typename Predicate = TrueNodePredicate> auto collectNodes(const std::vector<T> &nodes, const Predicate &predicate = Predicate{}) {
    ...
    auto visitor = kdl::overload([&]([[maybe_unused]] WorldNode *worldNode) {
...
template<typename T = Node *, typename Predicate = TrueNodePredicate> auto collectAncestors(const std::vector<T> &nodes, const Predicate &predicate = Predicate{}) {
    auto result = std::vector<Node *>{};
    auto visitor = kdl::overload([&]([[maybe_unused]] WorldNode *worldNode) {
        if constexpr (std::is_invocable_r_v<bool, Predicate, WorldNode *>) {
            if (predicate(worldNode)) {
...
template<typename T = Node *, typename Predicate = TrueNodePredicate> auto collectNodesAndAncestors(const std::vector<T> &nodes, const Predicate &predicate = Predicate{}) {
    auto result = std::vector<Node *>{};
    auto visitor = kdl::overload([&]([[maybe_unused]] WorldNode *worldNode) {
        if constexpr (std::is_invocable_r_v<bool, Predicate, WorldNode *>) {
            if (predicate(worldNode)) {
...
template<typename T = Node *, typename Predicate = TrueNodePredicate> auto collectDescendants(const std::vector<T> &nodes, const Predicate &predicate = Predicate{}) {
    auto result = std::vector<Node *>{};
    auto visitor = kdl::overload([&](auto &&thisLambda, [[maybe_unused]] WorldNode *worldNode) {
        if constexpr (std::is_invocable_r_v<bool, Predicate, WorldNode *>) {
            if (predicate(worldNode)) {
                result.push_back(worldNode);
            }
...
template<typename T = Node *, typename Predicate = TrueNodePredicate> auto collectNodesAndDescendants(const std::vector<T> &nodes, const Predicate &predicate = Predicate{}) {
    auto result = std::vector<Node *>{};
    auto visitor = kdl::overload([&](auto &&thisLambda, [[maybe_unused]] WorldNode *worldNode) {
        if constexpr (std::is_invocable_r_v<bool, Predicate, WorldNode *>) {
            if (predicate(worldNode)) {
...
template<typename T = Node, typename Predicate = TrueBrushFacePredicate> std::vector<BrushFaceHandle> collectBrushFaces(const std::vector<T *> &nodes, const Predicate &predicate = Predicate{}) {
