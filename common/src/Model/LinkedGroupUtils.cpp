...
std::vector<Node *> collectNodesWithLinkId(const std::vector<Node *> &nodes, const std::string &linkId) {
    return collectNodesAndDescendants(nodes, kdl::overload([&](const GroupNode *groupNode) { return groupNode->linkId() == linkId; }, [&](const EntityNode *entityNode) { return entityNode->linkId() == linkId; }, [&](const BrushNode *brushNode) { return brushNode->linkId() == linkId; }, [&](const PatchNode *patchNode) { return patchNode->linkId() == linkId; }));
}

std::vector<GroupNode *> collectGroupsWithLinkId(const std::vector<Node *> &nodes, const std::string &linkId) {
    return kdl::vec_static_cast<GroupNode *>(collectNodesAndDescendants(nodes, kdl::overload([&](const GroupNode *groupNode) {
...
std::vector<std::string> collectLinkedGroupIds(const std::vector<Node *> &nodes) {
    ...
    Node::visitAll(nodes, kdl::overload([](auto &&thisLambda, const WorldNode *worldNode) {
...
SelectionResult nodeSelectionWithLinkedGroupConstraints(WorldNode &world, const std::vector<Node *> &nodes) {
    ...
    for (auto *node : nodes) {
        ...
        const bool isNodeInGroupsToLock = kdl::any_of(containingGroupNodes, [&](auto *groupNode) { return groupsToLock.count(groupNode) == 1u; });
        ...
        const auto areAncestorGroupsHandled = kdl::all_of(containingGroupNodes, [&](auto *groupNode) { return groupsToKeepUnlocked.count(groupNode) == 1u; });
...
FaceSelectionResult faceSelectionWithLinkedGroupConstraints(WorldNode &world, const std::vector<BrushFaceHandle> &faces) {
    const std::vector<Node *> nodes = kdl::vec_transform(faces, [](auto handle) -> Node * { return handle.node(); });
...
namespace {
Result<std::unique_ptr<Node>> cloneAndTransformRecursive(const Node *nodeToClone, std::unordered_map<const Node *, NodeContents> &origNodeToTransformedContents, const vm::bbox3 &worldBounds) {
    // First, clone `n`, and move in the new (transformed) content which was
    // prepared for it above
    auto clone = nodeToClone->accept(kdl::overload([](const WorldNode *) -> std::unique_ptr<Node> {
        ...
        auto &patch = std::get<BezierPatch>(origNodeToTransformedContents.at(patchNode).get());
    ...
    return kdl::fold_results(kdl::vec_transform(nodeToClone->children(), [&](const auto *childNode) {
        return cloneAndTransformRecursive(childNode, origNodeToTransformedContents, worldBounds);
 ...
 */
Result<std::vector<std::unique_ptr<Node>>> cloneAndTransformChildren(const Node &node, const vm::bbox3 &worldBounds, const vm::mat4x4 &transformation) {
    ...
    // `nodesToClone`
    auto transformResults = kdl::vec_parallel_transform(nodesToClone, [&](const Node *nodeToTransform) {
        return nodeToTransform->accept(kdl::overload([](const WorldNode *) -> TransformResult {
            ensure(false, "Linked group structure is valid");
            ...
            return brush.transform(worldBounds, transformation, true).and_then([&]() -> TransformResult {
    ...
    return kdl::fold_results(std::move(transformResults)).or_else([](const auto &) -> Result<std::vector<std::pair<const Node *, NodeContents>>> {
        return Error{"Failed to transform a linked node"};
    }).and_then([&](auto origNodeAndTransformedContents) -> Result<std::vector<std::unique_ptr<Node>>> {
        ...
        return kdl::fold_results(kdl::vec_transform(node.children(), [&](const auto *childNode) {
...
auto makeLinkIdToNodeMap(const std::vector<Node *> &nodes) {
    ...
    }, [&](const BrushNode *brushNode) { result[brushNode->linkId()] = brushNode; }, [&](const PatchNode *patchNode) { result[patchNode->linkId()] = patchNode; }));
    return result;
}

template<typename N> const N *getCorrespondingNode(const std::unordered_map<std::string_view, const Node *> &correspondingNodes, const std::string_view linkId) {
    auto it = correspondingNodes.find(linkId);
    return it != correspondingNodes.end() ? dynamic_cast<const N *>(it->second) : nullptr;
}

template<typename T> void preserveGroupNames(const std::vector<T> &clonedNodes, const std::unordered_map<std::string_view, const Node *> &correspondingNodes) {
    return Node::visitAll(clonedNodes, kdl::overload([](auto &&thisLambda, const WorldNode *worldNode) {
        worldNode->visitChildren(thisLambda);
    }, [](auto &&thisLambda, const LayerNode *layerNode) {
        layerNode->visitChildren(thisLambda);
    }, [&](auto &&thisLambda, GroupNode *groupNode) {
        if (const auto *correspondingNode = getCorrespondingNode<GroupNode>(correspondingNodes, groupNode->linkId())) {
            auto group = groupNode->group();
...
void preserveEntityProperties(EntityNode &clonedEntityNode, const EntityNode &correspondingEntityNode) {
    if (clonedEntityNode.entity().protectedProperties().empty() && correspondingEntityNode.entity().protectedProperties().empty()) {
        return;
    }
    ...
    const auto allProtectedProperties = kdl::vec_sort_and_remove_duplicates(kdl::vec_concat(clonedEntity.protectedProperties(), correspondingEntity.protectedProperties()));
...
template<typename T> void preserveEntityProperties(const std::vector<T> &clonedNodes, const std::unordered_map<std::string_view, const Node *> &correspondingNodes) {
    return Node::visitAll(clonedNodes, kdl::overload([](auto &&thisLambda, const WorldNode *worldNode) {
        worldNode->visitChildren(thisLambda);
    }, [](auto &&thisLambda, const LayerNode *layerNode) {
        layerNode->visitChildren(thisLambda);
        ...
        if (const auto *correspondingNode = getCorrespondingNode<EntityNode>(correspondingNodes, entityNode->linkId())) {
            preserveEntityProperties(*entityNode, *correspondingNode);
        }
    }, [](const BrushNode *) {}, [](const PatchNode *) {}));
}
} // namespace

Result<UpdateLinkedGroupsResult> updateLinkedGroups(const GroupNode &sourceGroupNode, const std::vector<GroupNode *> &targetGroupNodes, const vm::bbox3 &worldBounds) {
    ...
    return kdl::fold_results(kdl::vec_transform(targetGroupNodesToUpdate, [&](auto *targetGroupNode) {
        const auto transformation = targetGroupNode->group().transformation() * _invertedSourceTransformation;
        return cloneAndTransformChildren(sourceGroupNode, worldBounds, transformation).transform([&](auto newChildren) {
...
template<typename N1, typename N2> Result<N1 *> tryCast(N2 &targetNode) {
    auto *targetNodeCasted = dynamic_cast<N1 *>(&targetNode);
    return targetNodeCasted ? Result<N1 *>{targetNodeCasted} : Result<N1 *>{Error{"Inconsistent linked group structure"}};
}

template<typename SourceNode, typename TargetNode, typename F> Result<void> visitNodesPerPosition(const SourceNode &sourceNode, TargetNode &targetNode, const F &f) {
    return sourceNode.accept(kdl::overload([&](const WorldNode *sourceWorldNode) {
        ...
        return tryCast<EntityNode>(targetNode).and_then([&](EntityNode *targetEntityNode) {
...
template<typename SourceNode, typename TargetNode, typename F> Result<void> visitChildrenPerPosition(SourceNode &sourceNode, TargetNode &targetNode, const F &f) {
    if (sourceNode.childCount() != targetNode.childCount()) {
        return Error{"Inconsistent linked group structure"};
    }

    return kdl::fold_results(kdl::vec_transform(kdl::make_zip_range(sourceNode.children(), targetNode.children()), [&](auto childPair) {
        auto &[sourceChild, targetChild] = childPair;
        return visitNodesPerPosition(*sourceChild, *targetChild, f);
    }));
}

Result<void> copyLinkIds(const GroupNode &sourceRootNode, GroupNode &targetRootNode, std::unordered_map<Node *, std::string> &linkIds) {
    return visitNodesPerPosition(sourceRootNode, targetRootNode, kdl::overload([&](const WorldNode &, const WorldNode &) { return Result<bool>{true}; }, [&](const LayerNode &, const LayerNode &) { return Result<bool>{true}; }, [&](const GroupNode &sourceGroupNode, GroupNode &targetGroupNode) {
...
template<typename R> Result<std::unordered_map<Node *, std::string>> copyLinkIds(const GroupNode &sourceGroupNode, const R &targetGroupNodes) {
    auto linkIds = std::unordered_map<Node *, std::string>{};
    return kdl::fold_results(kdl::vec_transform(targetGroupNodes, [&](auto *targetGroupNode) {
        return copyLinkIds(sourceGroupNode, *targetGroupNode, linkIds);
    })).transform([&]() { return std::move(linkIds); });
}

template<typename R> Result<std::unordered_map<Node *, std::string>> copyLinkIds(const R &groupNodes) {
    if (groupNodes.empty()) {
        return Error{"Link set must contain at least one group"};
    }

    return copyLinkIds(*groupNodes.front(), kdl::range{std::next(groupNodes.begin()), groupNodes.end()});
}

template<typename R> void setLinkIds(Result<std::unordered_map<Node *, std::string>> linkIdResult, const R &groups, std::vector<Error> &errors) {
    std::move(linkIdResult).transform([&](auto linkIds) {
        for (auto &[node, linkId] : linkIds) {
            node->accept(kdl::overload([](const WorldNode *) {}, [](const LayerNode *) {}, [&linkId = linkId](Object *object) { object->setLinkId(std::move(linkId)); }));
        }
...
std::vector<Error> initializeLinkIds(const std::vector<Node *> &nodes) {
    const auto allGroupNodes = kdl::vec_sort(collectGroups(nodes), compareGroupNodesByLinkId);
    const auto groupNodesByLinkId = kdl::make_grouped_range(allGroupNodes, [](const auto *lhs, const auto *rhs) { return lhs->linkId() == rhs->linkId(); });

    auto errors = std::vector<Error>{};
    for (const auto groupNodesWithId : groupNodesByLinkId) {
        // skip any link IDs with only one group
        if (groupNodesWithId.begin() != groupNodesWithId.end() && std::next(groupNodesWithId.begin()) != groupNodesWithId.end()) {
...
Result<std::unordered_map<Node *, std::string>> copyAndReturnLinkIds(const GroupNode &sourceGroupNode, const std::vector<GroupNode *> &targetGroupNodes) {
    return copyLinkIds(sourceGroupNode, targetGroupNodes);
}

std::vector<Error> copyAndSetLinkIds(const GroupNode &sourceGroupNode, const std::vector<GroupNode *> &targetGroupNodes) {
    auto errors = std::vector<Error>{};
