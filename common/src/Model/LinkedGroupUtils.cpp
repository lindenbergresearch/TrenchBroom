...
std::vector<Node *> collectNodesWithLinkId(const std::vector<Node *> &nodes, const std::string &linkId) {
  return collectNodesAndDescendants(
      nodes, kdl::overload(
          ...
          [&](const EntityNode *entityNode) { return entityNode->linkId() == linkId; }, [&](const BrushNode *brushNode) {
            return brushNode->linkId() == linkId;
...
SelectionResult nodeSelectionWithLinkedGroupConstraints(WorldNode &world, const std::vector<Node *> &nodes) {
  ...
  for (auto *node : nodes) {
    ...
    const bool isNodeInGroupsToLock = kdl::any_of(
        containingGroupNodes, [&](auto *groupNode) { return groupsToLock.count(groupNode) == 1u; }
    );
    ...
    const auto areAncestorGroupsHandled = kdl::all_of(
        containingGroupNodes, [&](auto *groupNode) { return groupsToKeepUnlocked.count(groupNode) == 1u; }
    );
...
Result<std::unique_ptr<Node>> cloneAndTransformRecursive(
    const Node *nodeToClone, std::unordered_map<const Node *, NodeContents> &origNodeToTransformedContents, const vm::bbox3 &worldBounds
) {
  // First, clone `n`, and move in the new (transformed) content which was
  // prepared for it above
  auto clone = nodeToClone->accept(
      kdl::overload(
  ...
  return kdl::fold_results(
      kdl::vec_transform(
          nodeToClone->children(), [&](const auto *childNode) {
...
Result<std::vector<std::unique_ptr<Node>>> cloneAndTransformChildren(const Node &node, const vm::bbox3 &worldBounds, const vm::mat4x4 &transformation) {
  ...
  auto transformResults = kdl::vec_parallel_transform(
      nodesToClone, [&](const Node *nodeToTransform) {
        return nodeToTransform->accept(
            kdl::overload(
                [](const WorldNode *) -> TransformResult {
                  ...
                  return brush.transform(worldBounds, transformation, true).and_then(
                      [&]() -> TransformResult {
  ...
  return kdl::fold_results(std::move(transformResults)).or_else(
      [](const auto &) -> Result<std::vector<std::pair<const Node *, NodeContents>>> {
        return Error{"Failed to transform a linked node"};
      }
  ).and_then(
      [&](auto origNodeAndTransformedContents) -> Result<std::vector<std::unique_ptr<Node>>> {
        // Move into map for easier lookup
        auto resultsMap = std::unordered_map<const Node *, NodeContents>{origNodeAndTransformedContents.begin(), origNodeAndTransformedContents.end()};
        ...
        return kdl::fold_results(
            kdl::vec_transform(
                node.children(), [&](const auto *childNode) {
...
template<typename N> const N *getCorrespondingNode(
    const std::unordered_map<std::string_view, const Node *> &correspondingNodes, const std::string_view linkId
) {
...
template<typename T> void preserveEntityProperties(
    const std::vector<T> &clonedNodes, const std::unordered_map<std::string_view, const Node *> &correspondingNodes
) {
  return Node::visitAll(
      clonedNodes, kdl::overload(
          [](auto &&thisLambda, const WorldNode *worldNode) {
...
Result<UpdateLinkedGroupsResult> updateLinkedGroups(
    const GroupNode &sourceGroupNode, const std::vector<GroupNode *> &targetGroupNodes, const vm::bbox3 &worldBounds
) {
  ...
  return kdl::fold_results(
      kdl::vec_transform(
          targetGroupNodesToUpdate, [&](auto *targetGroupNode) {
            const auto transformation = targetGroupNode->group().transformation() * _invertedSourceTransformation;
            return cloneAndTransformChildren(sourceGroupNode, worldBounds, transformation).transform(
                [&](auto newChildren) {
...
template<typename SourceNode, typename TargetNode, typename F> Result<void> visitNodesPerPosition(
    const SourceNode &sourceNode, TargetNode &targetNode, const F &f
) {
  return sourceNode.accept(
      kdl::overload(
          [&](const WorldNode *sourceWorldNode) {
            return tryCast<WorldNode>(targetNode).and_then(
                [&](WorldNode *targetWorldNode) {
                  return f(*sourceWorldNode, *targetWorldNode);
                }
            );
          }, [&](const LayerNode *sourceLayerNode) {
            return tryCast<LayerNode>(targetNode).and_then(
                [&](LayerNode *targetLayerNode) {
                  return f(*sourceLayerNode, *targetLayerNode);
                }
            );
          }, [&](const GroupNode *sourceGroupNode) {
            return tryCast<GroupNode>(targetNode).and_then(
                [&](GroupNode *targetGroupNode) {
                  return f(*sourceGroupNode, *targetGroupNode);
                }
            );
          }, [&](const EntityNode *sourceEntityNode) {
            return tryCast<EntityNode>(targetNode).and_then(
                [&](EntityNode *targetEntityNode) {
                  return f(*sourceEntityNode, *targetEntityNode);
                }
            );
          }, [&](const BrushNode *sourceBrushNode) {
            return tryCast<BrushNode>(targetNode).and_then(
                [&](BrushNode *targetBrushNode) {
                  return f(*sourceBrushNode, *targetBrushNode);
                }
            );
          }, [&](const PatchNode *sourcePatchNode) {
            return tryCast<PatchNode>(targetNode).and_then(
                [&](PatchNode *targetPatchNode) {
...
template<typename SourceNode, typename TargetNode, typename F> Result<void> visitChildrenPerPosition(
  ...
  return kdl::fold_results(
      kdl::vec_transform(
          kdl::make_zip_range(sourceNode.children(), targetNode.children()), [&](auto childPair) {
...
Result<void> copyLinkIds(const GroupNode &sourceRootNode, GroupNode &targetRootNode, std::unordered_map<Node *, std::string> &linkIds) {
  return visitNodesPerPosition(
      sourceRootNode, targetRootNode, kdl::overload(
          [&](const WorldNode &, const WorldNode &) { return Result<bool>{true}; }, [&](const LayerNode &, const LayerNode &) { return Result<bool>{true}; },
...
template<typename R> Result<std::unordered_map<Node *, std::string>> copyLinkIds(const GroupNode &sourceGroupNode, const R &targetGroupNodes) {
  ...
  return kdl::fold_results(
      kdl::vec_transform(
          targetGroupNodes, [&](auto *targetGroupNode) {
            return copyLinkIds(
...
template<typename R> void setLinkIds(Result<std::unordered_map<Node *, std::string>> linkIdResult, const R &groups, std::vector<Error> &errors) {
  std::move(linkIdResult).transform(
      [&](auto linkIds) {
        for (auto &[node, linkId] : linkIds) {
          node->accept(
              kdl::overload(
                  [](const WorldNode *) {}, [](const LayerNode *) {}, [&linkId = linkId](Object *object) { object->setLinkId(std::move(linkId)); }
              ));
        }
      }
  ).transform_error(
...
std::vector<Error> initializeLinkIds(const std::vector<Node *> &nodes) {
  ...
  const auto groupNodesByLinkId = kdl::make_grouped_range(
      allGroupNodes, [](const auto *lhs, const auto *rhs) { return lhs->linkId() == rhs->linkId(); }
  );
