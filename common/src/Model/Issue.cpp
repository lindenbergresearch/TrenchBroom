...
bool Issue::addSelectableNodes(std::vector<Model::Node *> &nodes) const {
    ...
    m_node.accept(kdl::overload([](WorldNode *) {}, [](LayerNode *) {}, [&](GroupNode *group) { nodes.push_back(group); }, [&](auto &&thisLambda, EntityNode *entity) {
...
BrushFaceIssue::BrushFaceIssue(const IssueType type, BrushNode &node, const size_t faceIndex, std::string description)
...
EntityPropertyIssue::EntityPropertyIssue(const IssueType type, EntityNodeBase &entityNode, std::string propertyKey, std::string description)
