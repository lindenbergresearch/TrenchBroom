...
template<typename T, typename FP, typename VP> void Polyhedron<T, FP, VP>::addPoints(std::vector<vm::vec<T, 3>> points) {
    if (!points.empty()) {
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addPoint(const vm::vec<T, 3> &position, const T planeEpsilon) {
    ...
    switch (vertexCount()) {
    case 0:result = addFirstPoint(position);
        m_bounds.min = m_bounds.max = position;
        break;
    case 1:result = addSecondPoint(position);
        m_bounds = vm::merge(m_bounds, position);
        break;
    case 2:result = addThirdPoint(position);
        m_bounds = vm::merge(m_bounds, position);
        break;
    default:result = addFurtherPoint(position, planeEpsilon);
        if (result != nullptr) {
            m_bounds = vm::merge(m_bounds, position);
        }
        break;
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addFirstPoint(const vm::vec<T, 3> &position) {
    assert(empty());
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addSecondPoint(const vm::vec<T, 3> &position) {
    assert(point());
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addThirdPoint(const vm::vec<T, 3> &position) {
    assert(edge());
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addColinearThirdPoint(const vm::vec<T, 3> &position) {
    assert(edge());
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addNonColinearThirdPoint(const vm::vec<T, 3> &position) {
    assert(edge());
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addFurtherPoint(const vm::vec<T, 3> &position, const T planeEpsilon) {
    assert(faceCount() > 0u);
    if (faceCount() == 1u) {
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addFurtherPointToPolygon(const vm::vec<T, 3> &position, const T planeEpsilon) {
    Face *face = m_faces.front();
    const vm::plane_status status = face->pointStatus(position, planeEpsilon);
    switch (status) {
    case vm::plane_status::inside:return addPointToPolygon(position, planeEpsilon);
    case vm::plane_status::above:face->flip();
        switchFallthrough();
    case vm::plane_status::below:return makePolyhedron(position, planeEpsilon);
    }
    // will never be reached
    return nullptr;
}

template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addPointToPolygon(const vm::vec<T, 3> &position, const T planeEpsilon) {
    assert(polygon());
    ...
    for (HalfEdge *curEdge : face->boundary()) {
        ...
        if (curStatus == vm::plane_status::inside && vm::segment<T, 3>(curEdge->origin()->position(), curEdge->destination()->position()).contains(position, vm::constants<T>::almost_zero())) {
            return nullptr;
        }
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::makePolyhedron(const vm::vec<T, 3> &position, const T planeEpsilon) {
    assert(polygon());
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Vertex *Polyhedron<T, FP, VP>::addFurtherPointToPolyhedron(const vm::vec<T, 3> &position, const T planeEpsilon) {
    assert(polyhedron());
...
template<typename T, typename FP, typename VP> class Polyhedron<T, FP, VP>::Seam {
  ...
  public:
     ...
     */
    const_iterator end() const { return std::end(m_edges); }
...
template<typename T, typename FP, typename VP> std::optional<typename Polyhedron<T, FP, VP>::Seam> Polyhedron<T, FP, VP>::createSeamForHorizon(const vm::vec<T, 3> &position, const T planeEpsilon) {
...
template<typename T, typename FP, typename VP> void Polyhedron<T, FP, VP>::visitFace(const vm::vec<T, 3> &position, HalfEdge *initialBoundaryEdge, std::unordered_set<Face *> &visitedFaces, Seam &seam, const T planeEpsilon) {
...
template<typename T, typename FP, typename VP> void Polyhedron<T, FP, VP>::split(const Seam &seam) {
    assert(seam.size() >= 3);
...
template<typename T, typename FP, typename VP> template<typename FaceSet> void Polyhedron<T, FP, VP>::deleteFaces(HalfEdge *first, FaceSet &visitedFaces, VertexList &verticesToDelete) {
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Face *Polyhedron<T, FP, VP>::sealWithSinglePolygon(const Seam &seam, const vm::plane<T, 3> &plane) {
    assert(seam.size() >= 3);
    assert(!seam.hasMultipleLoops());
...
template<typename T, typename FP, typename VP> bool Polyhedron<T, FP, VP>::checkSeamForWeaving(const Seam &seam, const vm::vec<T, 3> &position) const {
    assert(seam.size() >= 3);
    assert(!seam.hasMultipleLoops());
...
template<typename T, typename FP, typename VP> std::optional<typename Polyhedron<T, FP, VP>::WeaveConeResult> Polyhedron<T, FP, VP>::weaveCone(const Seam &seam, const vm::vec<T, 3> &position) {
    assert(seam.size() >= 3);
    assert(!seam.hasMultipleLoops());
...
template<typename T, typename FP, typename VP> void Polyhedron<T, FP, VP>::sealWithCone(WeaveConeResult cone, const Seam &seam) {
...
template<typename T, typename FP, typename VP> bool Polyhedron<T, FP, VP>::mergeCoplanarIncidentFaces(Vertex *vertex, const T planeEpsilon) {
    ...
    const auto checkAndMergeIncidentNeighbours = [&](auto *leaving) -> HalfEdge * {
        ...
        if (firstFace->coplanar(secondFace, planeEpsilon)) {
            ...
            if (firstFace->maximumVertexDistance(secondFace->plane()) < secondFace->maximumVertexDistance(firstFace->plane())) {
