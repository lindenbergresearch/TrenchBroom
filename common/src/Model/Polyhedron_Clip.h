...
namespace Model {
template<typename T, typename FP, typename VP> Polyhedron<T, FP, VP>::ClipResult::ClipResult(Face *face)
    : m_value(face) {
}

template<typename T, typename FP, typename VP> Polyhedron<T, FP, VP>::ClipResult::ClipResult(const FailureReason reason)
    : m_value(reason) {
}

template<typename T, typename FP, typename VP> bool Polyhedron<T, FP, VP>::ClipResult::unchanged() const {
    return std::holds_alternative<FailureReason>(m_value) && std::get<FailureReason>(m_value) == FailureReason::Unchanged;
}

template<typename T, typename FP, typename VP> bool Polyhedron<T, FP, VP>::ClipResult::empty() const {
    return std::holds_alternative<FailureReason>(m_value) && std::get<FailureReason>(m_value) == FailureReason::Empty;
}

template<typename T, typename FP, typename VP> bool Polyhedron<T, FP, VP>::ClipResult::success() const {
    return std::holds_alternative<Face *>(m_value);
}

template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Face *Polyhedron<T, FP, VP>::ClipResult::face() const {
    return success() ? std::get<Face *>(m_value) : nullptr;
}

template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::ClipResult Polyhedron<T, FP, VP>::clip(const vm::plane<T, 3> &plane) {
...
template<typename T, typename FP, typename VP> std::optional<typename Polyhedron<T, FP, VP>::ClipResult::FailureReason> Polyhedron<T, FP, VP>::checkIntersects(const vm::plane<T, 3> &plane) const {
    std::size_t above = 0u;
    std::size_t below = 0u;
    ...
    for (const Vertex *currentVertex : m_vertices) {
        ...
        switch (status) {
        case vm::plane_status::above:++above;
            break;
        case vm::plane_status::below:++below;
            break;
        case vm::plane_status::inside:++inside;
            break;
            switchDefault();
        }
    }
...
template<typename T, typename FP, typename VP> class Polyhedron<T, FP, VP>::NoSeamException : public Exception {
  private:
    std::vector<Edge *> m_splitFaces;

  public:
    NoSeamException(std::vector<Edge *> splitFaces) : m_splitFaces(std::move(splitFaces)) {
    }

    const std::vector<Edge *> &splitFaces() const { return m_splitFaces; }
};

template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::Seam Polyhedron<T, FP, VP>::intersectWithPlane(const vm::plane<T, 3> &plane) {
    Seam seam;
...
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::HalfEdge *Polyhedron<T, FP, VP>::findInitialIntersectingEdge(const vm::plane<T, 3> &plane) const {
    for (const Edge *currentEdge : m_edges) {
        ...
        if ((os == vm::plane_status::above && ds == vm::plane_status::inside) || (os == vm::plane_status::above && ds == vm::plane_status::below)) {
            return halfEdge;
        }
...
template<typename T, typename FP, typename VP> std::tuple<typename Polyhedron<T, FP, VP>::HalfEdge *, bool> Polyhedron<T, FP, VP>::intersectWithPlane(HalfEdge *firstBoundaryEdge, const vm::plane<T, 3> &plane) {
    ...
    do {
        ...
        } else if ((os == vm::plane_status::below && ds == vm::plane_status::above) || (os == vm::plane_status::above && ds == vm::plane_status::below)) {
...
template<typename T, typename FP, typename VP> void Polyhedron<T, FP, VP>::intersectWithPlane(HalfEdge *oldBoundaryFirst, HalfEdge *newBoundaryFirst) {
...
/*
 ...
 the given plane.
 */
template<typename T, typename FP, typename VP> typename Polyhedron<T, FP, VP>::HalfEdge *Polyhedron<T, FP, VP>::findNextIntersectingEdge(HalfEdge *searchFrom, const vm::plane<T, 3> &plane) const {
