...
std::ostream &operator<<(std::ostream &lhs, const EntityRotationType &rhs) {
    switch (rhs) {
    case EntityRotationType::None:lhs << "None";
        break;
    case EntityRotationType::Angle:lhs << "Angle";
        break;
    case EntityRotationType::AngleUpDown:lhs << "AngleUpDown";
        break;
    case EntityRotationType::Euler:lhs << "Euler";
        break;
    case EntityRotationType::Euler_PositivePitchDown:lhs << "Euler_PositivePitchDown";
        break;
    case EntityRotationType::Mangle:lhs << "Mangle";
        break;
        switchDefault();
    }
    return lhs;
...
std::ostream &operator<<(std::ostream &lhs, const EntityRotationUsage &rhs) {
    switch (rhs) {
    case EntityRotationUsage::Allowed:lhs << "Allowed";
        break;
    case EntityRotationUsage::BlockRotation:lhs << "BlockRotation";
        break;
        switchDefault();
    }
    return lhs;
...
namespace {
std::optional<std::tuple<std::string, EntityRotationType>> selectEntityRotationType(const Entity &entity, const std::vector<std::tuple<std::string, EntityRotationType>> &propertyToEntityRotationTypeMapping) {
    ...
    if (classname != EntityPropertyValues::NoClassname) {
        if (kdl::cs::str_is_prefix(classname, "light")) {
            ...
            if (!entity.pointEntity()) {
                // brush entity
                std::tie(propertyKey, type) = selectEntityRotationType(entity, {{EntityPropertyKeys::Angles, eulerType}, {EntityPropertyKeys::Mangle, eulerType}, {EntityPropertyKeys::Angle, EntityRotationType::AngleUpDown}}).value_or(std::make_tuple(propertyKey, type));
            } else {
                // point entity
                ...
                std::tie(propertyKey, type) = selectEntityRotationType(entity, {{EntityPropertyKeys::Angles, eulerType}, {EntityPropertyKeys::Mangle, eulerType}, {EntityPropertyKeys::Angle, EntityRotationType::AngleUpDown}}).value_or(std::make_tuple(EntityPropertyKeys::Angle, EntityRotationType::AngleUpDown));
            }
        }
    }
...
vm::mat4x4 entityRotation(const std::vector<EntityProperty> &properties, const EntityRotationInfo &info) {
    ...
    case EntityRotationType::Euler: {
        const auto it = findEntityProperty(properties, info.propertyKey);
        const auto angles = it != std::end(properties) ? vm::parse<FloatType, 3>(it->value()).value_or(vm::vec3::zero()) : vm::vec3::zero();
    ...
    case EntityRotationType::None:return vm::mat4x4::identity();
        switchDefault();
    }
}
...
std::optional<EntityProperty> applyEntityRotation(const std::vector<EntityProperty> &properties, const EntityRotationInfo &info, const vm::mat4x4 &transformation) {
    ...
    case EntityRotationType::None:return std::nullopt;
        switchDefault();
    }
}
