...
std::vector<vm::vec3> computeGridNormals(const std::vector<BezierPatch::Point> patchGrid, const size_t pointRowCount, const size_t pointColumnCount) {
    ...
    for (size_t row = 1u; row < b; ++row) {
        for (size_t col = 1u; col < r; ++col) {
            normals[index(row, col)] = (normalForQuadrant(row, col, RowOffset::Above, ColOffset::Left) + normalForQuadrant(row, col, RowOffset::Above, ColOffset::Right) + normalForQuadrant(row, col, RowOffset::Below, ColOffset::Left) + normalForQuadrant(row, col, RowOffset::Below, ColOffset::Right)) / static_cast<FloatType>(4);
        }
    }
