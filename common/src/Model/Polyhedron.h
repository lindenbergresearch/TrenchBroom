...
struct Polyhedron_GetVertexLink {
  kdl::intrusive_circular_link<Polyhedron_Vertex<T, FP, VP>> &operator()(
      Polyhedron_Vertex<T, FP, VP> *vertex
  ) const;
 ...
 *
 * Each vertex of a polyhedron has a position, a leaving half edge, a link to its previous
 * and next neighbours in the containing intrusive circular list, and a payload.
 *
 * The leaving half edge of a vertex is any half edge that has the vertex as its origin.
 * It is used to find the incident faces of a vertex.
 *
...
public:
    ...
    /**
     * Sets the leaving half edge for this vertex. The given edge must not be null and its
     * origin must be this vertex.
     *
    ...
    /**
     * Rounds each component of this vertices' position to the nearest integer if the
     * distance of the component's value to that integer is less  than the given epsilon
     * value. Furthermore, the component value is rounded such that at most the given number
     * of decimals are retained.
     *
 ...
 *
 * Each edge consists of two half edges with opposite directions. These half edges belong
 * to adjacent faces that share the edge. During execution of some algorithms, edges may
 * be underspecified, meaning that they have only one half edge. If both half edges are
 * set, then this edge is called fully specified.
 *
 * The naming of the contained half edges as first or second does not imply any
 * precedence, but it is sometimes used to distinguish the half edges in algorithms. For
 * example, when splitting a polyhedron along a seam, the first and second half edge will
 * be used to determine which part of the polyhedron is deleted and which part is
 * retained.
 *
...
public:
    ...
    /**
     * Returns the origin of the second half edge if one is set. Otherwise returns the
     * destination of the first half edge.
     */
    ...
    /**
     * Returns the second half edge. Behavior is unspecified if the second half edge is
     * unset.
     */
    HalfEdge *secondEdge() const;

    /**
     * Returns the twin of the given half edge, which must be one of the half edges of this
     * edge. If a half edge is passed that doesn't belong to this edge, the behavior is
     * unspecified.
     *
     * @param halfEdge the half edge of which the twin should be returned
     * @return the second half edge if given the first half edge, and the first half edge if
     * given the second half edge
     */
    HalfEdge *twin(const HalfEdge *halfEdge) const;

    /**
     * Returns the vector from the position of the first vertex to the position of the
     * second vertex. The behavior is unspecified if this edge is not fully specified.
     */
    ...
    /**
     * Returns the center between the position of the first vertex and the position of the
     * second vertex. The behavior is unspecified if this edge is not fully specified.
     */
    ...
    /**
     * Returns the face to which the second half edge belongs. The behavior is unspecified
     * if no second half edge is set.
     */
    Face *secondFace() const;

    /**
     * Indicates whether the given vertex is either the first or the second vertex of this
     * edge.
     *
     * @param vertex the vertex to check, which must not be null
     * @return true if the given vertex is either the first or the second vertex of this
     * edge, and false otherwise
     */
    bool hasVertex(const Vertex *vertex) const;

    /**
     * Indicates whether this edge has a vertex with the given position, up to the given
     * epsilon value.
     *
    ...
    /**
     * Indicates whether this edge has the given positions for its first and second vertex,
     * up to the given epsilon value. The order of the given positions does not matter.
     *
     ...
     */
    bool hasPositions(
        const vm::vec<T, 3> &position1, const vm::vec<T, 3> &position2, T epsilon = static_cast<T>(0.0)) const;

    /**
     * Computes the maximum of the minimal distances of each of the positions this edge's
     * vertices and the given two points. The result is the maximum of two distances d1 and
     * d2, where
     *
     * - d1 is the minimum of the distances between the positions of this edge's vertices
     * and position1
     * - d2 is the minimum of the distances between the positions of this edge's vertices
     * and position2
     *
...
private:
    ...
    /**
     * Ensures that the given half edge is the first half edge of this edge. If the given
     * edge is null, or if it is neither the first nor the second half edge of this edge,
     * the behavior is unspecfied. If the given half edge already is the first half edge of
     * this edge, then nothing happens. Otherwise, this edge is flipped and thereby the
     * given half edge is made the first half edge of this edge.
     *
    ...
    /**
     * Ensures that the given half edge is the second half edge of this edge. If the given
     * edge is null, or if it is neither the first nor the second half edge of this edge,
     * the behavior is unspecfied. If the given half edge already is the second half edge of
     * this edge, then nothing happens. Otherwise, this edge is flipped and thereby the
     * given half edge is made the second half edge of this edge.
     *
    ...
    /**
     * Ensures that the first half edge of this edge is the leaving half edge of its origin
     * vertex.
     */
    void setFirstAsLeaving();

    /**
     * Sets the second half edge of this edge to null. Assumes that this edge is fully
     * specified, otherwise the behavior is unspecified.
     */
    void unsetSecondEdge();

    /**
     * Sets the second half edge of this edge. Assumes that this edge is not fully
     * specified, otherwise the behavior is undefined. Furthermore, the given half edge must
     * not be associated with any other edge.
     *
     * @param second the half edge to set as the second half edge of this edge, must not be
     * null
     */
...
struct Polyhedron_GetHalfEdgeLink {
  kdl::intrusive_circular_link<Polyhedron_HalfEdge<T, FP, VP>> &operator()(
      Polyhedron_HalfEdge<T, FP, VP> *halfEdge
  ) const;
 ...
 * each of which belongs to the two faces containing the edge. The half edges belonging to
 * a face make up its boundary.
 *
 * Each half edge has an origin vertex, the edge to which the half edge belongs, and the
 * face to which it belongs. The origin vertex may have a pointer to this half edge if it
 * was set as the leaving half edge of that vertex.
 *
 * Furthermore, an edge has a link to its previous and next neighbours in the containing
 * intrusive circular list.
 *
 * The destination vertex of a half edge is the vertex at which the half edge ends and
 * where its successor in the boundary of the containing face originates.
 *
 * If this half edge is part of a fully specified edge, then the other half edge of that
 * edge is called the twin of this half edge.
 *
...
private:
    /**
     * Creates a new half edge with the given vertex as its origin. This half edge will be
     * set as the leaving half edge of the given vertex.
     *
...
public:
    ...
    /**
     * Returns the destination vertex of this half edge, which is the vertex where its
     * successor originates.
     */
    ...
    /**
     * Returns the twin of this half edge, that is, the other half edge of the edge to which
     * this half edge belongs.
     *
     * If this half edge does not belong to an edge, the behavior is unspecified. If the
     * corresponding edge is not fully specified, then null is returned.
     */
    HalfEdge *twin() const;

    /**
     * Returns the next incident half edge, that is, the next half edge, in counter
     * clockwise order, that originates at the origin of this half edge.
     *
    ...
    /**
     * Returns the previous incident half edge, that is, the previous half edge, in counter
     * clockwise order, that originates at the origin of this half edge.
     *
     * This function is used to enumerate the faces incident to a vertex.
     *
    ...
    /**
     * Indicates whether the positions of the origin vertices of this half edge and its
     * successors have the given positions. The first position is compared to the position
     * of this half edge's origin, the next given position is compared to the position of
     * this half edge's successor's origin, and so on.
     *
     * @param positions the positions to check
     * @param epsilon an epsilon value for comparing the vertex positions agains the given
     * positions
...
private:
    /**
     * Determines the relative location of the given point and a plane p that is defined as
     * follows.
     *
    ...
    vm::plane_status pointStatus(
        const vm::vec<T, 3> &normal, const vm::vec<T, 3> &point, T epsilon
    ) const;

    /**
     * Determines whether this half edge is colinear to the given half edge, which is
     * expected to have this half edge's destination as its origin.
     *
     * @param other the half edge to check against, must not be null and must not be
     * identical to this half edge
     * @return true if this half edge is colinear with the given half edge
     */
    bool colinear(const HalfEdge *other) const;

    /**
     * Sets the origin of this half edge and sets this as the leaving half edge of the given
     * vertex.
     *
    ...
    /**
     * Sets the edge to which this half edge belongs. This half edge must not already belong
     * to an edge when this function is called.
     *
    ...
    /**
     * Sets the edge of this half edge to null. Assumes that this half edge does belong to
     * an edge when this function is called.
     */
    void unsetEdge();

    /**
     * Sets the face whose boundary this half edge belongs to. This half edge must not
     * already belong to the boundary of any other face when this function is called.
     *
    ...
    /**
     * Sets the face to which this half edge belongs to null. Assumes that this half edge
     * does belong to a face when this function is called.
     */
    void unsetFace();
...
private:
    /**
     * The boundary of this face. The boundary of a face is a circular list of half edges,
     * usually three or more (but in some cases less if the face is degenerate).
     */
    HalfEdgeList m_boundary;

    /**
     * The plane that contains this face, i.e., the plane that contains all vertices of this
     * face.
     */
...
private:
    /**
     * Creates a new face with the given boundary and sets the face of each of the
     * boundary's half edges to this. The given boundary is moved into this face.
     *
     * @param boundary the boundary of the newly created face, which must contain at least
     * three half edges
...
public:
     ...
     * given origin, up to the given epsilon.
     *
     ...
     * given positions, up to the given epsilon.
     *
    ...
    /**
     * Indicates whether this face has a vertex with the given position, up to the given
     * epsilon.
     *
     * @param position the position to check
     * @param epsilon the epsilon value to use for comparison
     ...
     * position is compared to the position of the origin of the first half edge of this
     * face's boundary, the next given position is compared to its successor, and so on.
     *
     * @param positions the positions to check
     * @param epsilon the epsilon value to use for comparison
     ...
     * First, the algorithm finds the vertex with the minimal distance to the first given
     * position.
     *
     * If no vertex is within the given max distance of the given points, then the given max
     * distance is returned.
     *
     * Then it proceeds to compute the distance of the succeeding vertices (in the order in
     * which the vertices appear in the boundary) to the succeeding given positions, and
     * takes the maximum of all computed distances and returns it.
     *
     ...
     */
    T distanceTo(
        const std::vector<vm::vec<T, 3>> &positions, T maxDistance = std::numeric_limits<T>::max()) const;

    /**
     * Returns the normal of this face. The normal is computed by finding three non colinear
     * vertices in the boundary of this face, and taking the cross product of the vectors
    ...
    /**
     * Intersects this face with the given ray and returns the distance along the given
     * ray's direction, from the given ray's origin to the point of intersection, or NaN if
     * the given reay does not intersect this face.
     *
     * The given side determines whether hits from above or below this face should be
     * returned. A hit is considered from above if the ray's origin is above this face (the
     * face normal points towards it) and it is considered from below if the ray's origin is
     * below this face (the face normal points away from it).
     *
     * @param ray the ray
     ...
     * lies. This plane is determined by the position of the origin of the first half edge
     * of this face and the face normal.
     *
     * @param point the point to check
...
private:
    /**
     * Checks whether this face is coplanar with the given face, that is, if both faces lie
     * in the same plane.
     *
    ...
    /**
     * Inserts the given circular list of edges into the boundary of this face after the
     * given half edge. After insertion, the given list of edges will be empty.
     *
     * If the given half edge does not belong to this face's boundary, the behavior is
     * unspecified.
     *
     ...
     * the given half edges does not belong to this face's boundary, the behavior is
     * unspecified.
     *
    ...
    /**
     * Replaces the range [from, to] of half edges of this face's boundary with the half
     * edges in the given circular list. After replacement, the given circular list will be
     * empty.
     *
     * If the given range of half edges does not belong to this face's boundary, the
     ...
     * Two faces share a vertex if this vertex is the origin of a half edge of both faces.
     *
     * @param other the other face, which must not be null and it must not be identical to
     * this face
...
public:
    /**
     * Helper that maps a vertex to its position or a half edge to the position of its
     * origin.
     */
...
public: // Constructors
    ...
    /**
     * Copy constructor with callback. The callback can be used to set up the face and
     * vertex payloads.
     *
...
public: // comparison operators
     ...
     *
     * - they have an identical vertex count and for every vertex of one polyhedron, the
     * other has a vertex with an identical position
     * - they have an identical edge count and for every edge of one polyhedron, the other
     * has an edge with identical positions
     * - the have an identical face count and for every face of one polyhedron, the other
     * has a face with identical vertex positions
     */
...
public: // Accessors
    ...
    /**
     * Checks whether this polyhedron has any edge with the given vertex positions, up to
     * the given epsilon.
     *
    ...
    /**
     * Checks whether this polyhedron has any face with the given vertex positions, up to
     * the given epsilon.
     *
     ...
     *
     * A polyhedron is closed if the sum of its vertex and face count is equal to its edge
     * count plus two (Euler criterion for convex polyhedra).
     *
    ...
    /**
     * Clears this polyhedron. Afterwards, this polyhedron will have 0 vertices, 0 edges, 0
     * faces and its bounds will have NaN vectors for its min and max values.
     */
    void clear();
...
public: // General purpose methods
    /**
     * Checks whether this polyhedron has a vertex with the given position, up to the given
     * epsilon.
     *
     * @param position the position to check
     * @param epsilon the epsilon value to use for comparison
     * @return true if this polyhedron has a vertex with the given position and false
     * otherwise
     */
    bool hasVertex(const vm::vec<T, 3> &position, T epsilon = static_cast<T>(0.0)) const;

    /**
     * Checks whether any of the given positions corresponds to the position of a vertex of
     * this polyhedron.
     *
     * @param positions the positions to check
     * @param epsilon the epsilon value to use for comparison
     ...
     * - its distance to the given position is minimal among all vertices of this
     * polyhedron.
     *
    ...
    Edge *findClosestEdge(
        const vm::vec<T, 3> &pos1, const vm::vec<T, 3> &pos2, T maxDistance = std::numeric_limits<T>::max()) const;
...
private:
    /**
     * Updates the bounds to the smallest bounding box that contains the positions of all
     * vertices of this polyhedron. If this polyhedron does not have any vertices, the min
     * and max of the bounds are set to NaN vectors.
     */
    void updateBounds();

public: // Vertex correction and edge healing
    /**
     * Rounds each component of position of every vertex to the nearest integer if the
     * distance of the component's value to that integer is less  than the given epsilon
     * value. Furthermore, the component value is rounded such that at most the given number
     * of decimals are retained.
     *
     ...
     * removing an edge leads to degenerate faces, these degenerate faces are removed, too.
     *
     * Updates the bounds of this polyhedron afterwards.
     *
     * @param minLength the minimum edge length, edges shorter than this length will be
     * removed
...
private:
     ...
     * @return removes the successor of the given edge in the containing circular list or
     ...
     *
     * If this is not a polyhedron (has less than four faces), then the outcome is
     * undefined.
     *
     * Let f1 be the face to which the given half edge borderFirst belongs. Let f2 be the
     * face to which the twin of the given half edge belongs, and let e be the edge to which
     * the given half edge belongs. Then f1 and f2 are adjacent and should be merged.
     *
     * In some cases, f1 and f2 can share more edges than e. In such a case, the shared
     * edges preceede and / or succeed e, so this algorithm will search for all shared edges
     * and handle them accordingly. The effects of merging f1 and f2 are
     *
     * - that f2 is deleted
     ...
     * - f2 is deleted
     *
     * After the faces are merged, topological errors are checked and fixed. Let v1, ..., vn
     * be the vertices that f1 and f2 shared before the merge. Then v2, ..., vn-1 have been
     * deleted by the merge, but we need to inspect v1 and vn further. Let v' be v1 or vn
     * without loss of generality. If v' has only two incident edges, then v' is redundant.
     * We inspect the two incident faces of v'. One of them is f1, which has more than three
     * vertices now. Let f' be the other incident face. If f' is a triangle, then f' gets
     * merged into f1, whereby f' is deleted and f1 is retained. If f' is not a triangle,
     * then v' is removed and f1 and f' are now separated by a single edge.
     *
     * Finally, if the given validEdge is deleted by this algorithm, then it is set to its
     * first successor that is not deleted by this algorithm. This value can be used by the
     * caller during iteration over all edges of this polyhedron.
     *
     * The merge process can fail if it turns this polyhedron into an invalid polyhedron
     * with less than four faces.
     *
     * @param borderFirst a half edge that belongs to the edge to which the faces to be
     * merged are incident
     * @param validEdge an edge that is used for iteration of all edges (see result), can be
     * null
    ...
    /**
     * Merges the two incident edges of the given vertex. If either of the two incident
     * faces is a triangle, the faces will be merged.
     *
     * Preconditions:
     * - The given vertex has exactly two incident edges.
     * - The given vertex has exactly two incident faces.
     *
     * If neither of the given faces is a triangle, then the given vertex, its leaving half
     * edge and its twin and the corresponding edge will be removed. Otherwise, the faces
     * will be merged.
     *
...
private: // Convex hull; adding and removing points
    /**
     * Adds the given points to this polyhedron. The effect of adding the given points to a
     * polyhedron is that the resulting polyhedron is the convex hull of the union of the
     * polyhedron's vertices and the given points.
     *
     * Duplicates in the given vector are discarded. Furthermore, the remaining points are
     * sorted in descending order of their distance from the center of their bounding box.
     * Therefore, the result of calling this method is different from the result of
     * repeatedly calling addPoint() for every point in the given vector.
     *
     ...
     * polyhedron is that the resulting polyhedron is the convex hull of the union of the
     * polyhedron's vertices and the given point.
     *
     ...
     * @return the newly created vertex, or null if the given point was not added to this
     * polyhedron
     */
...
private:
    /**
     * Helper function that adds the given point to an empty polyhedron. Afterwards, this
     * polyhedron will be a point.
     *
     * @param position the point to add
    ...
    /**
     * Helper function that adds the given point to a point polyhedron. Afterwards, this
     * polyhedron will be a point or an edge.
     *
     * If the given point is identical to this polyhedron's single vertices' position, then
     * nothing happens.
     *
    ...
    /**
     * Helper function that adds the given point to an edge polyhedron. Afterwards, this
     * polyhedron will be an edge or a triangle.
     *
     * If the given point is contained in the edge formed by this polyhedron, nothing
     * happens. Otherwise, if the given point and this polyhedron's vertices are linearly
     * dependent, then this polyhedron is modified, but remains an edge.
     *
    ...
    /**
     * Helper function that adds the given point to an edge polyhedron. Afterwards, this
     * polyhedron is an edge.
     *
     * Assumes that this is an edge polyhedron and that the given point and this
     * polyhedron's vertices are linearly dependent.
     *
     * @param position the point to add
     * @return the newly created vertex or null if no vertex was created
     */
     ...
     *
     * Assumes that this is an edge polyhedron and that the given point and this
     * polyhedron's vertices are linearly independent. If an error occurs while adding the
     * point, the polyhedron remains unchanged.
     *
     * @param position the point to add
    ...
    /**
     * Helper function that adds the given point to a polyhedron that is either a polygon or
     * a convex volume.
     *
     ...
     * - if this is a polygon, and the given point lies on the same plane as the polygon,
     * then it is added, but the polyhedron remains a polygon
     * - if this is a polygon, and the given point does not lie on the same plane as the
     * polygon, then it is added and the polyhedron becomes a convex volume
     * - if this is a convex volume, then it is updated to the convex hull of the union of
     * its vertices and the given point
     *
     ...
     *
     * Adding a point to a polygon either turns it into a convex volume or it remains a
     * polygon, depending on whether the given point lies on the same plane as the polygon
     * itself.
     *
     ...
     *
     * Assumes that this polyhedron is a polygon and that the given position lies in the
     * same plane as the polygon.
     *
     * @param position the point to add
     * @param planeEpsilon the plane epsilon to use for point status checks
     * @return the newly created vertex or null if no vertex was created
    ...
    /**
     * Helper function that adds the given non coplanar point to a polygon, turning it into
     * a convex volume.
     *
     * Assumes that this polyhedron is a polygon and that the given point does not lie on
     * the same plane as the polygon.
     *
     * @param position the point to add
     * @param planeEpsilon the plane epsilon to use for point status checks
     * @return the newly created vertex or null if no vertex was created
    ...
    /**
     * A seam is a circular sequence of consecutive edges. For each edge of a seam, it must
     * hold that its first vertex is identical to the second vertex of its predecessor.
     */
    class Seam;
     ...
     *
     * The horizon of a position is a closed loop of edges of this polyhedron such that it
     * separates those faces which are visible from the given position from those faces
     * which are not visible from the given position.
     *
     * Suppose that the faces of a polyhedron are the vertices of a graph G, and two graph
     * vertices are connected in if and only if the corresponding faces are adjacent.
     *
     * Given such a graph, a seam divides the faces of a graph into two disjoint sets such
     * that, for every pair of faces f1, f2 from the same set, there is a graph in G that
     * connects them.
     *
     * In other words, a seam separates the faces of a polyhedron in such a way that the
     * polyhedron can be pulled apart into two "open" polyhedra along the seam.
     *
     * Thereby, the returned seam contains those edges where the incident faces belong to
     * different sets, i.e. the edges of the seam are oriented such that the first faces
     * matches the splitting criterion and the other faces does not.
     *
     * @param position the vertex position
     * @param planeEpsilon the plane epsilon to use for point status checks
     * @return a seam that separates the faces that are visible from the given position from
     * those that do not, or an empty optional if no seam could be created
     */
    std::optional<Seam> createSeamForHorizon(const vm::vec<T, 3> &position, T planeEpsilon);

    void visitFace(
        const vm::vec<T, 3> &position, HalfEdge *initialBoundaryEdge, std::unordered_set<Face *> &visitedFaces, Seam &seam, T planeEpsilon
    );

    /**
     * Splits this polyhedron along the given seam. The edges of the seam must be oriented
     * in such a way that the portion of this polyhedron that is connected to the first
     * faces of the given edges should be retained and the portion of this polyhedron that
     * is connected to the second faces of the given edges should be removed.
     *
     * The removed faces, edges and vertices are deleted. The seam edges will have their
     * second half edges unset.
     *
    ...
    /**
     * Helper that recursively deletes faces and edges, and adds vertices to be deleted to
     * the given list.
     *
     * This function will delete all faces and edges which are somehow connected to the
     * given half edge. Furthermore, it will delete all vertices it encounters where the
     * leaving half edge is to be deleted also.
     *
     * To protect a vertex from being deleted, it should therefore have a leaving half edge
     * that is not going to be deleted here, i.e. to protect a vertex from being deleted,
     * its leaving edge should belong to a face which will not be visited by this function.
     *
     * Therefore, a caller should make sure that the portion of this polyhedron that should
     * be retained is not connected to the portion that should be deleted. This can be
     * ensured by unsetting the second half edge of every seam edge before calling this
     * function.
     *
     ...
     * function assumes that this polygon is opened along the given seam, and all edges of
     * the given seam lie on a plane.
     *
     * The edges of the given seam are expected to be oriented such that their second edges
     * are unset. The new polygon will then be the first face of every edge in the given
     * seam.
     *
     * Assumes that this polyhedron is neither empty, nor a point, nor an edge, nor a
     * polygon.
     *
     ...
     *
     * The returned cone may have coplanar adjacent faces. The caller is responsible for
     * merging those.
     *
     * @param seam the seam to weave a cone onto
     * @param position the position of the cone's tip
     ...
     *
     * This polyhedron is expected to be open such that the second edges of the given seam
     * are unset. The given cone must fit onto the given seam, i.e., the cone is expected to
     * be open also and its "open" edges have the vertices of the seam edges as their
     * origins.
     *
    ...
    /**
     * Inspects all incident faces of the given vertex and merges those which are coplanar.
     * If all the faces are coplanar, then the given vertex is deleted and a single face
     * remains that includes all other vertices of the incident faces.
     *
     * Furthermore, the faces are merged with their coplanar neighbours which are not
     * incident to the given vertex, which might remove even more vertices and edges. In
     * this case, the neighbouring faces are retained and the faces incident to the given
     * vertex are removed.
     *
     * @param vertex the vertex whose incident faces should be merged
     * @param planeEpsilon the plane epsilon to use for point status checks
     * @return true if the given vertex remains or false if all incident faces were merged
     * and the vertex was deleted
     */
...
private:
    ...
    /**
     May throw a NoSeamException if the polyhedron cannot be intersected with the given
     plane due.
     */

    /**
     * Creates a seam along the intersection of this polyhedron with the given plane. This
     * will create
     *
     ...
     *
     * The newly created edges are oriented such that their second edge is incident to the
     * faces which are above the given plane. If a face is split, then the portion that is
     * above the given plane is newly created and the portion that is below the given plane
     * will be the original face, modified so that it does not intersect with the given
     * plane.
     *
     * Afterwards, this polyhedron will be modified in such a way that it has no edge or
     * face that intersects with the given plane. The edges of the returned seam will lie
     * entirely inside of the given plane.
     *
     * This function assumes that the given plane does intersect with this polyhedron.
     *
     * In some corner cases, it is impossible to construct a seam due to floating point
     * inaccuracies. If this function detects such a case, it throws a NoSeamException.
     *
    ...
    /**
     * This function finds the starting edge for intersecting a polyhedron with a plane. It
     * returns a half edge such that one of the following conditions hold:
     *
     * - The half edge is split in two by the given plane, and its origin is be above the
     * given plane or its destination is be below the given plane.
     * - The half edge lies entirely within the given plane, and it belongs to a face that
     * lies above the given plane.
     *
     * This function may fail to find such a half edge in certain corner cases. If such a
     * case is detected, the function returns null.
     *
     * @param plane the intersecting plane
     * @return the starting edge for intersecting this polyhedron with the plane, or null if
     ...
     *
     * 1. The plane just touches the face, i.e. one vertex of the face is inside the plane.
     * In this case, the face is left untouched and the half edge whose destination is
     * inside the plane is returned.
     * 2. The plane just touches an edge of the face, i.e. two consecutive vertices of the
     * face are inside the plane. In that case, the face is left untouched and the touched
     * half edge is returned.
     * 3. The plane intersects the face. In this case, there are two points where the given
     * plane intersects the boundary of the face, and any number of these points of
     * intersection may coincide with a vertex of the face.
     *
     *    In this case, the algorithm will insert vertices at those points of intersection
     * where there isn't a vertex already, splitting the intersected edges in the process.
     * Finally, the algorithm will split the face into two such that the newly created face
     * coincides with the portion of the given face that lies above the given plane, and the
     * given face is modified so that it lies entirely below the given plane.
     *
     *    Finally, the algorithm then returns the newly created half edge which was inserted
     * into the given face.
     *
     * The returned half edge can be used by the caller to continue splitting this
     * polyhedron with the given plane.
     *
     ...
     * half edges oldBoundaryFirst and newBoundaryFirst belong to the same face, which is
     * the face to be split.
     *
     * The following diagram illustrates how the face is split. Given a face with the
     * following shape:
     *
     *      oldBoundaryFirst
     *   *<------------------*<------------------*
     ...
     *                         newBoundaryFirst
     *
     * @param oldBoundaryFirst the first half edge of the boundary of the remaining portion
     * of the face
    ...
    /**
     * Searches all half edges leaving the destination of half edge searchFrom for a half
     * edge that is intersected by the given plane under the assumption that searchFrom was
     * also intersected.
     *
     * A half edge leaving searchEdge's destination is considered to be intersected by the
     * given plane if either of the following conditions hold:
     *
     * - its destination is inside the plane
     * - its destination and the origin of its predecessor lie on different sides of the
     * plane.
     *
     * The search stops if such a half edge is found or if the search encounters
     * searchFrom's twin, which is already known to be intersected by the given plane. In
     * the first case, the found half edge is returned, and in the latter case, the function
     * returns null.
     *
...
public: // Intersection
    /**
     * Clips this polyhedron with all faces of the given polyhedron. The result is the
     * intersection of this and the given polyhedron. This polyhedron remains unchanged.
     *
...
public: // Subtraction
    /**
     * Subtracts the given polyhedron from this polyhedron and returns the resulting
     * fragments. This polyhedron remains unchanged.
     *
     * In general, the result of a CSG subtraction is concave and can therefore not be
     * represented by one single polyhedron. This algorithm represents the concave shape as
     * a disjoint union of convex polyhedra. In the context of subtraction, these polyhedra
     * are called fragments.
     *
     * If the given polyhedron and this polyhedron are disjoint, the result of the
     * subtraction is identical to this polyhedron.
     *
...
public: // geometrical queries
    /**
     * Checks whether this polyhedron contains the given point. A point is considered to be
     * contained in this polyhedron if it isn't above any of its faces' planes.
     *
     * @param point the point to check
    ...
    /**
     * Checks whether this polyhedron contains the given polyhedron. A polyhedron is
     * considered to be contained in this polyhedron if the CSG intersection of the
     * polyhedra is identical to the given polyhedron.
     *
     * @param other the polyhedron to check
     * @return true if the given polyhedron is contained in this polyhedron and false
     * otherwise
     */
    bool contains(const Polyhedron &other) const;

    /**
     * Checks whether this polyhedron intersects with the given polyhedron, i.e. whether
     * their CSG intersection is an empty polyhedron.
     *
     * @param other the polyhedron to check
...
private: // helper functions for all cases of polygon / polygon intersection
     ...
     * vertices are above that face's plane.
     *
     * @param faces the faces to check
     * @param vertices the vertices to check against each face plane
     * @return true if a face was found such that all of the given vertices have their
     * position above the face plane and false otherwise
     */
