...
bool EditorContext::visible(const Model::Node *node) const {
    return node->accept(kdl::overload([&](const WorldNode *world) { return visible(world); }, [&](const LayerNode *layer) { return visible(layer); }, [&](const GroupNode *group) { return visible(group); }, [&](const EntityNode *entity) { return visible(entity); }, [&](const BrushNode *brush) { return visible(brush); }, [&](const PatchNode *patch) { return visible(patch); }));
}
...
bool EditorContext::selectable(const Model::Node *node) const {
    return node->accept(kdl::overload([&](const WorldNode *world) { return selectable(world); }, [&](const LayerNode *layer) { return selectable(layer); }, [&](const GroupNode *group) { return selectable(group); }, [&](const EntityNode *entity) { return selectable(entity); }, [&](const BrushNode *brush) { return selectable(brush); }, [&](const PatchNode *patch) { return selectable(patch); }));
}
...
bool EditorContext::selectable(const Model::GroupNode *groupNode) const {
    return visible(groupNode) && editable(groupNode) && !groupNode->opened() && inOpenGroup(groupNode);
}

bool EditorContext::selectable(const Model::EntityNode *entityNode) const {
    return visible(entityNode) && editable(entityNode) && !entityNode->hasChildren() && inOpenGroup(entityNode);
}
