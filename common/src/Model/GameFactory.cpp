                                             ...
                                             [&](auto backupPath) {
                                               return m_configFs->makeAbsolute(profilesPath).join(m_configFs->makeAbsolute(backupPath)).transform(
                                                                    [&](auto absProfilesPath, auto absBackupPath) {
                                                                      logger
                                                                          .warn() << "Backed up malformed compilation config " << absProfilesPath << " to " << absBackupPath;
                                                                    }
                                                                );
                                             }
                                         ).transform_error(
    ...
    backupFile(*m_configFs, profilesPath).and_then(
                                             [&](auto backupPath) {
                                               return m_configFs->makeAbsolute(profilesPath).join(m_configFs->makeAbsolute(backupPath)).transform(
                                                                    [&](auto absProfilesPath, auto absBackupPath) {
                                                                      logger
                                                                          .warn() << "Backed up malformed game engine config " << absProfilesPath << " to " << absBackupPath;
                                                                    }
                                                                );
                                             }
                                         ).transform_error(
