...
void GameFileSystem::initialize(const GameConfig &config, const std::filesystem::path &gamePath, const std::vector<std::filesystem::path> &additionalSearchPaths, Logger &logger) {
    unmountAll();
    m_shaderFS = nullptr;
...
void GameFileSystem::reloadWads(const std::filesystem::path &rootPath, const std::vector<std::filesystem::path> &wadSearchPaths, const std::vector<std::filesystem::path> &wadPaths, Logger &logger) {
    unmountWads();
...
void GameFileSystem::addDefaultAssetPaths(const GameConfig &config, Logger &logger) {
    ...
    auto defaultFolderPaths = IO::SystemPaths::findResourceDirectories(std::filesystem::path("defaults"));
...
void GameFileSystem::addGameFileSystems(const GameConfig &config, const std::filesystem::path &gamePath, const std::vector<std::filesystem::path> &additionalSearchPaths, Logger &logger) {
...
void GameFileSystem::addShaderFileSystem(const GameConfig &config, Logger &logger) {
    ...
    if (!textureConfig.shaderSearchPath.empty()) {
        ...
        auto shaderFs = IO::createImageFileSystem<IO::Quake3ShaderFileSystem>(*this, std::move(shaderSearchPath), std::move(textureSearchPaths), logger).value();
        m_shaderFS = shaderFs.get();
        mount(std::filesystem::path{}, std::move(shaderFs));
    }
}

void GameFileSystem::mountWads(const std::filesystem::path &rootPath, const std::vector<std::filesystem::path> &wadSearchPaths, const std::vector<std::filesystem::path> &wadPaths, Logger &logger) {
