...
FgdParser::FgdParser(const std::string_view str, const Color &defaultEntityColor, const std::filesystem::path &path) : EntityDefinitionParser{defaultEntityColor}, m_tokenizer{FgdTokenizer{str}} {
...
Assets::ModelDefinition FgdParser::parseModel(ParserStatus &status, const bool allowEmptyExpression) {
    ...
    try {
        ...
        try {
            ...
            expression.optimize();
            status.warn(
                line, column, fmt::format("Legacy model expressions are deprecated, replace with '{}'", expression.asString()));
            return Assets::ModelDefinition{std::move(expression)};
...
std::unique_ptr<Assets::PropertyDefinition> FgdParser::parsePropertyDefinition(
    ...
    status.debug(
        line, column, fmt::format("Unknown property definition type '{}' for property '{}'", typeName, propertyKey));
