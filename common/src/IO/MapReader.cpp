...
MapReader::MapReader(std::string_view str, const Model::MapFormat sourceMapFormat, const Model::MapFormat targetMapFormat,
    Model::EntityPropertyConfig entityPropertyConfig, std::vector<std::string> linkedGroupsToKeep
) : StandardMapParser(std::move(str), sourceMapFormat, targetMapFormat), m_entityPropertyConfig{
...
void MapReader::onStandardBrushFace(const size_t line, const Model::MapFormat targetMapFormat, const vm::vec3 &point1, const vm::vec3 &point2,
    const vm::vec3 &point3, const Model::BrushFaceAttributes &attribs, ParserStatus &status
) {
    Model::BrushFace::createFromStandard(point1, point2, point3, attribs, targetMapFormat).transform([&](auto face) {
...
void
MapReader::onValveBrushFace(const size_t line, const Model::MapFormat targetMapFormat, const vm::vec3 &point1, const vm::vec3 &point2, const vm::vec3 &point3,
    const Model::BrushFaceAttributes &attribs, const vm::vec3 &texAxisX, const vm::vec3 &texAxisY, ParserStatus &status
) {
    Model::BrushFace::createFromValve(point1, point2, point3, attribs, texAxisX, texAxisY, targetMapFormat).transform([&](Model::BrushFace &&face) {
          face.setFilePosition(line, 1u);
          onBrushFace(std::move(face), status);
        }
    ).transform_error([&](auto e) { status.error(line, "Skipping face: " + e.msg); });
...
void MapReader::onPatch(const size_t startLine, const size_t lineCount, Model::MapFormat, const size_t rowCount, const size_t columnCount,
    std::vector<vm::vec<FloatType, 5>> controlPoints, std::string textureName, ParserStatus &
) {
    m_objectInfos.emplace_back(PatchInfo{
            rowCount, columnCount, std::move(controlPoints), std::move(textureName), startLine, lineCount, m_currentEntityInfo
        }
    );
}
...
static std::optional<ContainerInfo> extractContainerInfo(const std::vector<Model::EntityProperty> &properties, std::vector<NodeIssue> &nodeIssues) {
    if (const std::string &parentLayerIdStr = findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::Layer);
    ...
    else if (const std::string &parentGroupIdStr = findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::Group);
 ...
 */
static CreateNodeResult
createWorldNode(MapReader::EntityInfo entityInfo, const Model::EntityPropertyConfig &entityPropertyConfig, const Model::MapFormat mapFormat) {
    ...
    return NodeInfo{
        std::move(worldNode), {}, // parent info
        {}  // issues
    };
}
...
static CreateNodeResult createLayerNode(const MapReader::EntityInfo &entityInfo) {
    ...
    const auto &name = findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::LayerName);
    if (kdl::str_is_blank(name)) {
        return NodeError{entityInfo.startLine, "Skipping layer entity: missing name"};
    }

    const auto &idStr = findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::LayerId);
    ...
    if (!rawId || *rawId <= 0u) {
        return NodeError{
            entityInfo.startLine, kdl::str_to_string("Skipping layer entity: '", idStr, "' is not a valid id")
        };
    }

    auto layer = Model::Layer{name};
    // This is optional (not present on maps saved in TB 2020.1 and earlier)
    if (const auto layerSortIndex = kdl::str_to_int(findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::LayerSortIndex))) {
        layer.setSortIndex(*layerSortIndex);
    }

    if (findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::LayerOmitFromExport) == Model::EntityPropertyValues::LayerOmitFromExportValue) {
    ...
    if (findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::LayerLocked) == Model::EntityPropertyValues::LayerLockedValue) {
        layerNode->setLockState(Model::LockState::Locked);
    }

    if (findEntityPropertyOrDefault(properties, Model::EntityPropertyKeys::LayerHidden) == Model::EntityPropertyValues::LayerHiddenValue) {
        layerNode->setVisibilityState(Model::VisibilityState::Hidden);
    }

    return NodeInfo{
        std::move(layerNode), {}, // parent info
        {}  // issues
    };
}
...
static CreateNodeResult createGroupNode(const MapReader::EntityInfo &entityInfo) {
    const auto &name = findEntityPropertyOrDefault(entityInfo.properties, Model::EntityPropertyKeys::GroupName);
    if (kdl::str_is_blank(name)) {
        return NodeError{entityInfo.startLine, "Skipping group entity: missing name"};
    }

    const auto &idStr = findEntityPropertyOrDefault(entityInfo.properties, Model::EntityPropertyKeys::GroupId);
    if (kdl::str_is_blank(idStr)) {
    ...
    if (!rawId || *rawId <= 0) {
        return NodeError{
            entityInfo.startLine, kdl::str_to_string("Skipping group entity: '", idStr, "' is not a valid id")
        };
    }
    ...
    const auto &linkedGroupId = findEntityPropertyOrDefault(entityInfo.properties, Model::EntityPropertyKeys::LinkedGroupId);
    if (!linkedGroupId.empty()) {
        const auto &transformationStr = findEntityPropertyOrDefault(entityInfo.properties, Model::EntityPropertyKeys::GroupTransformation);
    ...
    return NodeInfo{std::move(groupNode), std::move(containerInfo), std::move(nodeIssues),};
}
 ...
 */
static CreateNodeResult
createNodeFromEntityInfo(const Model::EntityPropertyConfig &entityPropertyConfig, MapReader::EntityInfo entityInfo, const Model::MapFormat mapFormat) {
    const auto &classname = findEntityPropertyOrDefault(entityInfo.properties, Model::EntityPropertyKeys::Classname);
...
static CreateNodeResult createBrushNode(MapReader::BrushInfo brushInfo, const vm::bbox3 &worldBounds) {
    return Model::Brush::create(worldBounds, std::move(brushInfo.faces)).transform([&](auto brush) {
...
static CreateNodeResult createPatchNode(MapReader::PatchInfo patchInfo) {
    auto patchNode = std::make_unique<Model::PatchNode>(Model::BezierPatch{
            patchInfo.rowCount, patchInfo.columnCount, std::move(patchInfo.controlPoints), std::move(patchInfo.textureName)
        }
    );
 ...
 * nodes later.
 */
static std::vector<std::optional<NodeInfo>>
createNodesFromObjectInfos(const Model::EntityPropertyConfig &entityPropertyConfig, std::vector<MapReader::ObjectInfo> objectInfos,
    const vm::bbox3 &worldBounds, const Model::MapFormat mapFormat, ParserStatus &status
) {
    ...
    auto createNodeResults = kdl::vec_parallel_transform(std::move(objectInfos), [&](MapReader::ObjectInfo &&objectInfo) -> CreateNodeResult {
          return std::visit(kdl::overload([&](MapReader::EntityInfo &&entityInfo) {
    ...
    return kdl::vec_transform(std::move(createNodeResults), [&](std::optional<CreateNodeResult> &&createNodeResult) -> std::optional<NodeInfo> {
          ...
          return std::move(*createNodeResult).transform([&](NodeInfo &&nodeInfo) -> std::optional<NodeInfo> {
...
static void validateDuplicateLayersAndGroups(std::vector<std::optional<NodeInfo>> &nodeInfos, ParserStatus &status) {
    auto layerIds = std::unordered_set<Model::IdType>{};
    auto groupIds = std::unordered_set<Model::IdType>{};

    for (auto &nodeInfo: nodeInfos) {
        if (nodeInfo) {
            nodeInfo->node->accept(kdl::overload([](Model::WorldNode *) {}, [&](Model::LayerNode *layerNode) {
...
static void
validateOrphanedLinkedGroups(std::vector<std::optional<NodeInfo>> &nodeInfos, const std::vector<std::string> linkedGroupsToKeep, ParserStatus &status) {
    ...
    for (const auto &nodeInfo: nodeInfos) {
        if (nodeInfo) {
            nodeInfo->node->accept(kdl::overload([](const Model::WorldNode *) {}, [](const Model::LayerNode *) {}, [&](const Model::GroupNode *groupNode) {
    ...
    for (auto &nodeInfo: nodeInfos) {
        if (nodeInfo) {
            nodeInfo->node->accept(kdl::overload([](const Model::WorldNode *) {}, [](const Model::LayerNode *) {}, [&](Model::GroupNode *groupNode) {
                  if (const auto linkedGroupId = groupNode->group().linkedGroupId()) {
                      if (linkedGroupCounts[*linkedGroupId] == 1) {
                          status.error(groupNode->lineNumber(), kdl::str_to_string("Unlinking orphaned linked group with ID '", *linkedGroupId, "'"));
...
static void logValidationIssues(std::vector<std::optional<NodeInfo>> &nodeInfos, ParserStatus &status) {
    for (auto &nodeInfo: nodeInfos) {
        if (nodeInfo) {
            // log issues
            for (const auto &issue: nodeInfo->issues) {
                std::visit(kdl::overload([&](const MalformedTransformationIssue &m) {
                          ...
                          status.warn(nodeInfo->node->lineNumber(), kdl::str_to_string("Adding object to default layer: Invalid ", c.type, " ID '", c.idStr, "'"));
                        }
                    ), issue
                );
            }
...
static bool isRecursiveLinkedGroup(const std::string &nestedLinkedGroupId, Model::Node *parentNode) {
...
static void
validateRecursiveLinkedGroups(std::vector<std::optional<NodeInfo>> &nodeInfos, const std::unordered_map<Model::Node *, Model::Node *> &nodeToParentMap,
    ParserStatus &status
) {
    for (auto &nodeInfo: nodeInfos) {
        if (nodeInfo) {
                ...
                if (const auto groupNodeLinkedGroupId = groupNode->group().linkedGroupId()) {
                    ...
                    while (iParent != nodeToParentMap.end()) {
                        if (isRecursiveLinkedGroup(*groupNodeLinkedGroupId, iParent->second)) {
                            status.error(groupNode->lineNumber(),
                                kdl::str_to_string("Unlinking recursive linked group with ID '", *groupNode->persistentId(), "'"));
 ...
 */
static std::unordered_map<Model::Node *, Model::Node *> buildNodeToParentMap(std::vector<std::optional<NodeInfo>> &nodeInfos, ParserStatus &status) {
    ...
    for (const auto &nodeInfo: nodeInfos) {
        if (nodeInfo && nodeInfo->parentInfo) {
            std::visit(kdl::overload([&](const size_t parentIndex) {
                      ...
                      if (auto *containerNode = findContainerNode(containerInfo)) {
                          ...
                          if (containerInfo.type == ContainerType::Layer) {
                              status.warn(nodeInfo->node->lineNumber(),
                                  kdl::str_to_string("Entity references missing layer '", containerInfo.id, "', adding to default layer"));
                          }
                          else {
                              status.warn(nodeInfo->node->lineNumber(),
                                  kdl::str_to_string("Entity references missing group '", containerInfo.id, "', adding to default layer"));
                          }
                      }
                    }
                ), *nodeInfo->parentInfo
...
void MapReader::createNodes(ParserStatus &status) {
    ...
    for (auto &nodeInfo: nodeInfos) {
        if (nodeInfo) {
            ...
            node->accept(kdl::overload([&](Model::WorldNode *) {
                  // this should not happen since we already cleared out any world nodes
                }, [&](Model::LayerNode *) { onLayerNode(std::move(node), status); }, [&](Model::GroupNode *) { onNode(parentNode, std::move(node), status); },
