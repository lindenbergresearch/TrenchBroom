...
MapReader::MapReader(std::string_view str, const Model::MapFormat sourceMapFormat, const Model::MapFormat targetMapFormat, Model::EntityPropertyConfig entityPropertyConfig)
    : StandardMapParser{std::move(str), sourceMapFormat, targetMapFormat}, m_entityPropertyConfig{std::move(entityPropertyConfig)} {
}
...
[[maybe_unused]] std::ostream &operator<<(std::ostream &lhs, const ContainerType rhs) {
    switch (rhs) {
    case ContainerType::Layer:return lhs << "layer";
    case ContainerType::Group:return lhs << "group";
        switchDefault();
    }
}
 ...
 */
std::optional<ContainerInfo> extractContainerInfo(const std::vector<Model::EntityProperty> &properties, std::vector<NodeIssue> &nodeIssues) {
 ...
 */
CreateNodeResult createWorldNode(MapReader::EntityInfo entityInfo, const Model::EntityPropertyConfig &entityPropertyConfig, const Model::MapFormat mapFormat) {
...
CreateNodeResult createGroupNode(const MapReader::EntityInfo &entityInfo) {
    ...
    const auto linkId = findEntityPropertyOrDefault(entityInfo.properties, Model::EntityPropertyKeys::LinkId);
    if (!linkId.empty()) {
    ...
    return NodeInfo{
        std::move(groupNode), std::move(containerInfo), std::move(nodeIssues),
    };
}
 ...
 */
CreateNodeResult createEntityNode(const Model::EntityPropertyConfig &entityPropertyConfig, MapReader::EntityInfo entityInfo) {
    auto entity = Model::Entity{entityPropertyConfig, std::move(entityInfo.properties)};
 ...
 */
CreateNodeResult createNodeFromEntityInfo(const Model::EntityPropertyConfig &entityPropertyConfig, MapReader::EntityInfo entityInfo, const Model::MapFormat mapFormat) {
 ...
 * be created.
 */
CreateNodeResult createBrushNode(MapReader::BrushInfo brushInfo, const vm::bbox3 &worldBounds) {
 ...
 * nodes later.
 */
std::vector<std::optional<NodeInfo>> createNodesFromObjectInfos(const Model::EntityPropertyConfig &entityPropertyConfig, std::vector<MapReader::ObjectInfo> objectInfos, const vm::bbox3 &worldBounds, const Model::MapFormat mapFormat, ParserStatus &status) {
...
void validateDuplicateLayersAndGroups(std::vector<std::optional<NodeInfo>> &nodeInfos, ParserStatus &status) {
...
void logValidationIssues(std::vector<std::optional<NodeInfo>> &nodeInfos, ParserStatus &status) {
    for (auto &nodeInfo : nodeInfos) {
        if (nodeInfo) {
            // log issues
...
void validateRecursiveLinkedGroups(std::vector<std::optional<NodeInfo>> &nodeInfos, const std::unordered_map<Model::Node *, Model::Node *> &nodeToParentMap, ParserStatus &status) {
    for (auto &nodeInfo : nodeInfos) {
        if (nodeInfo) {
            if (auto *groupNode = dynamic_cast<Model::GroupNode *>(nodeInfo->node.get())) {
                ...
                while (iParent != nodeToParentMap.end()) {
                    if (isRecursiveLinkedGroup(groupNodeLinkId, iParent->second)) {
                        status.error(groupNode->lineNumber(), kdl::str_to_string("Unlinking recursive linked group with ID '", *groupNode->persistentId(), "'"));
 ...
 */
std::unordered_map<Model::Node *, Model::Node *> buildNodeToParentMap(std::vector<std::optional<NodeInfo>> &nodeInfos, ParserStatus &status) {
...
void MapReader::createNodes(ParserStatus &status) {
    ...
    for (auto &nodeInfo : nodeInfos) {
        if (nodeInfo) {
            ...
            }, [&](Model::LayerNode *) { onLayerNode(std::move(node), status); }, [&](Model::GroupNode *) { onNode(parentNode, std::move(node), status); }, [&](Model::EntityNode *) { onNode(parentNode, std::move(node), status); }, [&](Model::BrushNode *) { onNode(parentNode, std::move(node), status); }, [&](Model::PatchNode *) { onNode(parentNode, std::move(node), status); }));
        }
    }
}
