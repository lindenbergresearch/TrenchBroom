...
Assets::Texture loadSkin(
    ...
    return fs.openFile(path).and_then(
        [&](auto file) -> Result<Assets::Texture, ReadTextureError> {
...
Assets::Texture loadShader(
    ...
    return fs.openFile(actualPath).and_then([&](auto file) { return readQuake3ShaderTexture(name, *file, fs); }).transform_error(
        [&](auto e) {
