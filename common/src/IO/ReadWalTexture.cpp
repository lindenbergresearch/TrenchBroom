...
size_t readMipOffsets(const size_t maxMipLevels, size_t offsets[], const size_t width, const size_t height, Reader &reader) {
    const auto mipLevels = std::min(std::min(size_t(std::log2(width)), size_t(std::log2(height))) + 1, maxMipLevels);
...
std::tuple<Assets::TextureBufferList, bool> readMips(const Assets::Palette &palette, const size_t mipLevels, const size_t offsets[], const size_t width, const size_t height, Reader &reader, Color &averageColor, const Assets::PaletteTransparency transparency) {
    ...
    for (size_t i = 0; i < mipLevels; ++i) {
        ...
        hasTransparency = hasTransparency || (palette.indexedToRgba(reader, size, buffers[i], transparency, tempColor) && i == 0);
        if (i == 0) {
...
Result<Assets::Texture, ReadTextureError> readQ2Wal(std::string name, Reader &reader, const std::optional<Assets::Palette> &palette) {
    ...
    try {
        ...
        auto [buffers, hasTransparency] = readMips(*palette, mipLevels, offsets, width, height, reader, averageColor, Assets::PaletteTransparency::Opaque);

        unused(hasTransparency);

        return Assets::Texture{
            std::move(name), width, height, averageColor, std::move(buffers), GL_RGBA, Assets::TextureType::Opaque, gameData
        };
...
Result<Assets::Texture, ReadTextureError> readDkWal(std::string name, Reader &reader) {
    ...
    try {
        ...
        return Assets::loadPalette(paletteReader, Assets::PaletteColorFormat::Rgb).transform([&](const auto &palette) {
            auto [buffers, hasTransparency] = readMips(palette, mipLevels, offsets, width, height, reader, averageColor, Assets::PaletteTransparency::Index255Transparent);

            return Assets::Texture{
                std::move(name), width, height, averageColor, std::move(buffers), GL_RGBA, hasTransparency ? Assets::TextureType::Masked : Assets::TextureType::Opaque, gameData
            };
