...
void GameEngineConfigWriter::writeConfig() {
    m_stream << EL::Value{EL::MapType{{"version",  EL::Value{1.0}},
                                      {"profiles", writeProfiles(m_config)},}} << "\n";
}

EL::Value GameEngineConfigWriter::writeProfiles(
    const Model::GameEngineConfig &config
) const {
...
EL::Value GameEngineConfigWriter::writeProfile(
    const Model::GameEngineProfile &profile
) const {
    return EL::Value{EL::MapType{{"name",       EL::Value{profile.name}},
