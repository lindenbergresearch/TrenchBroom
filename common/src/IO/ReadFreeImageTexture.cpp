...
Result<Assets::Texture, ReadTextureError> readFreeImageTextureFromMemory(std::string name, const uint8_t *begin, const size_t size) {
    try {
        ...
        auto imageMemory = kdl::resource{
            FreeImage_OpenMemory(const_cast<uint8_t *>(begin), static_cast<DWORD>(size)), FreeImage_CloseMemory
        };
...
std::vector<std::string> getSupportedFreeImageExtensions() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (FreeImage_IsPluginEnabled(format)) {
            ...
            result = kdl::vec_concat(std::move(result), kdl::vec_transform(kdl::str_split(extensionListStr, ","), [](const auto &extension) { return "." + extension; }));
        }
    }
