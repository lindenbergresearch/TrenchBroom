...
Bsp29Parser::Bsp29Parser(std::string name, const Reader &reader, Assets::Palette palette, const FileSystem &fs) : m_name{std::move(name)}, m_reader{reader}, m_palette{std::move(palette)}, m_fs{fs} {
}
...
void Bsp29Parser::doLoadFrame(const size_t frameIndex, Assets::EntityModel &model, Logger & /* logger */) {
    ...
    parseFrame(
        ...
        frameIndex, model, textureInfos, vertices, edgeInfos, faceInfos, faceEdges);
}
...
std::vector<Assets::Texture> Bsp29Parser::parseTextures(Reader reader, Logger &logger) {
    ...
    for (size_t i = 0; i < textureCount; ++i) {
        ...
        result.push_back(
            readIdMipTexture(textureName, textureReader, m_palette).or_else(makeReadTextureErrorHandler(m_fs, logger)).value());
    }
...
void Bsp29Parser::parseFrame(
    Reader reader, const size_t frameIndex, Assets::EntityModel &model, const TextureInfoList &textureInfos, const std::vector<vm::vec3f> &vertices,
