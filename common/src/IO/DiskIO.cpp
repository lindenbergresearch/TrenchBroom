...
Result<std::vector<std::filesystem::path>> find(const std::filesystem::path &path, const TraversalMode traversalMode, const PathMatcher &pathMatcher) {
  ...
  case TraversalMode::Flat:
    std::transform(
        std::filesystem::directory_iterator{fixedPath, error}, std::filesystem::directory_iterator{}, std::back_inserter(result),
        [&](const auto &entry) { return entry.path(); }
    );
    break;
  case TraversalMode::Recursive:
    std::transform(
        std::filesystem::recursive_directory_iterator{fixedPath, error}, std::filesystem::recursive_directory_iterator{}, std::back_inserter(result),
        [&](const auto &entry) { return entry.path(); }
    );
    break;
  }
...
Result<bool> deleteFile(const std::filesystem::path &path) {
  ...
  case PathInfo::Directory: return Error{"Failed to delete '" + fixedPath.string() + "': path denotes a directory"};
  case PathInfo::File: {
    auto error = std::error_code{};
