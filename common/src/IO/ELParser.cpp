...
ELTokenizer::ELTokenizer(std::string_view str, const size_t line, const size_t column) : Tokenizer{std::move(str), "\"", '\\', line, column} {
}
...
ELParser::ELParser(const ELParser::Mode mode, std::string_view str, const size_t line, const size_t column) : m_mode{mode}, m_tokenizer{str, line, column} {
}
...
EL::Expression ELParser::parseGroupedTerm() {
    ...
    auto lhs = EL::Expression{
        EL::UnaryExpression{EL::UnaryOperator::Group, std::move(expression)}, token.line(), token.column()
    };
...
EL::Expression ELParser::parseSubscript(EL::Expression lhs) {
    ...
    auto rhs = elements.size() == 1u ? std::move(elements.front()) : EL::Expression{EL::ArrayExpression{std::move(elements)}, line, column};
...
EL::Expression ELParser::parseLiteral() {
    ...
    if (token.hasType(ELToken::Number)) {
        m_tokenizer.nextToken();
        return {
            EL::LiteralExpression{EL::Value{token.toFloat<EL::NumberType>()}}, token.line(), token.column()
        };
    }
    if (token.hasType(ELToken::Boolean)) {
        m_tokenizer.nextToken();
        return {
            EL::LiteralExpression{EL::Value{token.data() == "true"}}, token.line(), token.column()
        };
    }
...
EL::Expression ELParser::parseUnaryOperator() {
    static const auto TokenMap = std::unordered_map<ELToken::Type, EL::UnaryOperator>{{ELToken::Addition, EL::UnaryOperator::Plus}, {ELToken::Subtraction, EL::UnaryOperator::Minus}, {ELToken::LogicalNegation, EL::UnaryOperator::LogicalNegation}, {ELToken::BitwiseNegation, EL::UnaryOperator::BitwiseNegation},
    };
...
EL::Expression ELParser::parseCompoundTerm(EL::Expression lhs) {
    static const auto TokenMap = std::unordered_map<ELToken::Type, EL::BinaryOperator>{{ELToken::Addition, EL::BinaryOperator::Addition}, {ELToken::Subtraction, EL::BinaryOperator::Subtraction}, {ELToken::Multiplication, EL::BinaryOperator::Multiplication}, {ELToken::Division, EL::BinaryOperator::Division}, {ELToken::Modulus, EL::BinaryOperator::Modulus}, {ELToken::LogicalAnd, EL::BinaryOperator::LogicalAnd}, {ELToken::LogicalOr, EL::BinaryOperator::LogicalOr}, {ELToken::BitwiseAnd, EL::BinaryOperator::BitwiseAnd}, {ELToken::BitwiseXOr, EL::BinaryOperator::BitwiseXOr}, {ELToken::BitwiseOr, EL::BinaryOperator::BitwiseOr}, {ELToken::BitwiseShiftLeft, EL::BinaryOperator::BitwiseShiftLeft}, {ELToken::BitwiseShiftRight, EL::BinaryOperator::BitwiseShiftRight}, {ELToken::Less, EL::BinaryOperator::Less}, {ELToken::LessOrEqual, EL::BinaryOperator::LessOrEqual}, {ELToken::Greater, EL::BinaryOperator::Greater}, {ELToken::GreaterOrEqual, EL::BinaryOperator::GreaterOrEqual}, {ELToken::Equal, EL::BinaryOperator::Equal}, {ELToken::NotEqual, EL::BinaryOperator::NotEqual}, {ELToken::Range, EL::BinaryOperator::Range}, {ELToken::Case, EL::BinaryOperator::Case},
    };
    ...
    while (m_tokenizer.peekToken().hasType(ELToken::CompoundTerm)) {
        ...
        if (const auto it = TokenMap.find(token.type()); it != TokenMap.end()) {
            const auto op = it->second;
            lhs = EL::Expression{
                EL::BinaryExpression{op, std::move(lhs), parseSimpleTermOrSwitch()}, token.line(), token.column()
            };
        } else {
            throw ParserException{
                token.line(), token.column(), "Unhandled binary operator: " + tokenName(token.type())
            };
        }
    }
...
ELParser::TokenNameMap ELParser::tokenNames() const {
    return {{ELToken::Name, "variable"}, {ELToken::String, "string"}, {ELToken::Number, "number"}, {ELToken::Boolean, "boolean"}, {ELToken::OBracket, "'['"}, {ELToken::CBracket, "']'"}, {ELToken::OBrace, "'{'"}, {ELToken::CBrace, "'}'"}, {ELToken::OParen, "'('"}, {ELToken::CParen, "')'"}, {ELToken::Addition, "'+'"}, {ELToken::Subtraction, "'-'"}, {ELToken::Multiplication, "'*'"}, {ELToken::Division, "'/'"}, {ELToken::Modulus, "'%'"}, {ELToken::Colon, "':'"}, {ELToken::Comma, "','"}, {ELToken::Range, "'..'"}, {ELToken::LogicalNegation, "'!'"}, {ELToken::LogicalAnd, "'&&'"}, {ELToken::LogicalOr, "'||'"}, {ELToken::Less, "'<'"}, {ELToken::LessOrEqual, "'<='"}, {ELToken::Equal, "'=='"}, {ELToken::NotEqual, "'!='"}, {ELToken::GreaterOrEqual, "'>='"}, {ELToken::Greater, "'>'"}, {ELToken::Case, "'->'"}, {ELToken::BitwiseNegation, "'~'"}, {ELToken::BitwiseAnd, "'&'"}, {ELToken::BitwiseOr, "'|'"}, {ELToken::BitwiseShiftLeft, "'<<'"}, {ELToken::BitwiseShiftRight, "'>>'"}, {ELToken::DoubleOBrace, "'{{'"}, {ELToken::DoubleCBrace, "'}}'"}, {ELToken::Null, "'null'"}, {ELToken::Eof, "end of file"},
    };
}
