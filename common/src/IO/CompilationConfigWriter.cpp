...
namespace IO {
CompilationConfigWriter::CompilationConfigWriter(const Model::CompilationConfig &config, std::ostream &stream) : m_config{config}, m_stream{stream} {
    assert(!m_stream.bad());
}
...
EL::Value CompilationConfigWriter::writeProfiles(const Model::CompilationConfig &config) const {
    return EL::Value{
        kdl::vec_transform(config.profiles, [&](const auto &profile) { return writeProfile(profile); })
    };
}

EL::Value CompilationConfigWriter::writeProfile(const Model::CompilationProfile &profile) const {
    return EL::Value{
        EL::MapType{{"name", EL::Value{profile.name}}, {"workdir", EL::Value{profile.workDirSpec}}, {"tasks", writeTasks(profile)},
        }};
}

EL::Value CompilationConfigWriter::writeTasks(const Model::CompilationProfile &profile) const {
    return EL::Value{
        kdl::vec_transform(profile.tasks, [](const auto &task) {
            return std::visit(kdl::overload([](const Model::CompilationExportMap &exportMap) {
                auto map = EL::MapType{};
