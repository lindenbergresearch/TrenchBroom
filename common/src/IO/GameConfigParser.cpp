...
std::vector<std::string> prependDot(const std::vector<std::string> &extensions) {
    return kdl::vec_transform(extensions, [](const auto &extension) { return prependDot(extension); });
}

void checkVersion(const EL::Value &version) {
    const auto validVsns = std::vector<EL::IntegerType>{7, 8};
    const auto isValidVersion = version.convertibleTo(EL::ValueType::Number) && std::find(validVsns.begin(), validVsns.end(), version.integerValue()) != validVsns.end();

    if (!isValidVersion) {
        throw ParserException{
            version.line(), version.column(), " Unsupported game configuration version " + version.convertTo(EL::ValueType::String).stringValue() + "; valid versions are: " + kdl::str_join(validVsns, ", ")
        };
    }
}
...
std::vector<Model::CompilationTool> parseCompilationTools(const EL::Value &value) {
    ...
    for (size_t i = 0; i < value.length(); ++i) {
        ...
        if (value[i]["description"] != EL::Value::Null) {
            ...
            result.push_back(Model::CompilationTool{
                value[i]["name"].stringValue(), std::nullopt,
            });
        }
    }
...
std::optional<vm::bbox3> parseSoftMapBounds(const EL::Value &value) {
    ...
    if (!bounds.has_value()) {
        ...
        throw ParserException{
            value.line(), value.column(), "Can't parse soft map bounds '" + value.asString() + "'"
        };
    }
    return bounds;
}
...
void parseSurfaceParmTag(std::string name, const EL::Value &value, std::vector<Model::SmartTag> &result) {
...
void parseFaceTags(const EL::Value &value, const Model::FaceAttribsConfig &faceAttribsConfig, std::vector<Model::SmartTag> &result) {
    if (value == EL::Value::Null) {
        return;
    }
    ...
    for (size_t i = 0; i < value.length(); ++i) {
        ...
        if (match == "texture") {
            ...
            result.emplace_back(entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::TextureNameTagMatcher>(entry["pattern"].stringValue()));
            ...
            result.emplace_back(entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::ContentFlagsTagMatcher>(parseFlagValue(entry["flags"], faceAttribsConfig.contentFlags)));
        } else if (match == "surfaceflag") {
            expectMapEntry(entry, "flags", EL::ValueType::Array);
            result.emplace_back(entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::SurfaceFlagsTagMatcher>(parseFlagValue(entry["flags"], faceAttribsConfig.surfaceFlags)));
        } else {
            throw ParserException{
...
void parseBrushTags(const EL::Value &value, std::vector<Model::SmartTag> &result) {
    ...
    for (size_t i = 0; i < value.length(); ++i) {
        ...
        const auto match = entry["match"].stringValue();
        if (match == "classname") {
            result.emplace_back(entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::EntityClassNameTagMatcher>(entry["pattern"].stringValue(), entry["texture"].stringValue()));
        } else {
            throw ParserException{
                entry.line(), entry.column(), "Unexpected smart tag match type '" + match + "'"
            };
...
std::vector<Model::SmartTag> parseTags(const EL::Value &value, const Model::FaceAttribsConfig &faceAttribsConfig) {
    auto result = std::vector<Model::SmartTag>{};
    if (value == EL::Value::Null) {
        return result;
    }

    expectStructure(value, R"([
      {},
...
Model::BrushFaceAttributes parseFaceAttribsDefaults(const EL::Value &value, const Model::FlagsConfig &surfaceFlags, const Model::FlagsConfig &contentFlags) {
...
void parseFlag(const EL::Value &value, const size_t index, std::vector<Model::FlagConfig> &flags) {
...
Model::FaceAttribsConfig parseFaceAttribsConfig(const EL::Value &value) {
    if (value == EL::Value::Null) {
        return Model::FaceAttribsConfig{{}, {}, Model::BrushFaceAttributes{Model::BrushFaceAttributes::NoTextureName},
        };
    }
...
Model::EntityConfig parseEntityConfig(const EL::Value &value) {
    ...
    return Model::EntityConfig{
        kdl::vec_transform(value["definitions"].asStringList(), [](const auto &str) { return std::filesystem::path{str}; }), Color::parse(value["defaultcolor"].stringValue()).value_or(Color{}), value["scale"].expression(), value["setDefaultProperties"].booleanValue(),
    };
}
...
Model::PackageFormatConfig parsePackageFormatConfig(const EL::Value &value) {
    ...
    if (value["extension"] != EL::Value::Null) {
        ...
        return Model::PackageFormatConfig{{prependDot(value["extension"].stringValue())}, formatValue.stringValue(),
        };
    } else if (value["extensions"] != EL::Value::Null) {
        expectType(value["extensions"], EL::typeForName("Array"));

        return Model::PackageFormatConfig{
            prependDot(value["extensions"].asStringList()), formatValue.stringValue(),
        };
    }
    throw ParserException{
        value.line(), value.column(), "Expected map entry 'extension' of type 'String' or 'extensions' of type 'Array'"
    };
}
...
Model::TextureConfig parseTextureConfig(const EL::Value &value) {
    ...
    return Model::TextureConfig{
        std::filesystem::path{value["root"].stringValue()}, parseTextureExtensions(value), std::filesystem::path{value["palette"].stringValue()}, value["attribute"] != EL::Value::Null ? std::optional{value["attribute"].stringValue()} : std::nullopt, std::filesystem::path{value["shaderSearchPath"].stringValue()}, value["excludes"].asStringList(),
    };
}
...
Model::FileSystemConfig parseFileSystemConfig(const EL::Value &value) {
    ...
    return Model::FileSystemConfig{
        std::filesystem::path{value["searchpath"].stringValue()}, parsePackageFormatConfig(value["packageformat"]),
    };
}
...
std::vector<Model::MapFormatConfig> parseMapFormatConfigs(const EL::Value &value) {
    ...
    for (size_t i = 0; i < value.length(); ++i) {
        ...
        result.push_back(Model::MapFormatConfig{
            value[i]["format"].stringValue(), std::filesystem::path{value[i]["initialmap"].stringValue()},
        });
    }
...
GameConfigParser::GameConfigParser(std::string_view str, const std::filesystem::path &path) : ConfigParserBase{std::move(str), path}, m_version{0} {
}
...
Model::GameConfig GameConfigParser::parse() {
    ...
    return {
        root["name"].stringValue(), m_path, std::filesystem::path{root["icon"].stringValue()}, root["experimental"].booleanValue(), std::move(mapFormatConfigs), std::move(fileSystemConfig), std::move(textureConfig), std::move(entityConfig), std::move(faceAttribsConfig), std::move(tags), std::move(softMapBounds), std::move(compilationTools),
    };
}
