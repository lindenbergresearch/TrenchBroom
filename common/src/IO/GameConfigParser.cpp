...
void checkVersion(const EL::Value &version) {
    ...
    if (!isValidVersion) {
        throw ParserException{version.line(), version.column(),
                              " Unsupported game configuration version " + version.convertTo(EL::ValueType::String).stringValue() + "; valid versions are: " + kdl::str_join(validVsns, ", ")};
    }
}
...
void parseFaceTags(const EL::Value &value, const Model::FaceAttribsConfig &faceAttribsConfig, std::vector<Model::SmartTag> &result) {
    ...
    for (size_t i = 0; i < value.length(); ++i) {
        ...
        if (match == "texture") {
            ...
            result.emplace_back(
                entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::ContentFlagsTagMatcher>(parseFlagValue(entry["flags"], faceAttribsConfig.contentFlags)));
        } else if (match == "surfaceflag") {
            expectMapEntry(entry, "flags", EL::ValueType::Array);
            result.emplace_back(
                entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::SurfaceFlagsTagMatcher>(parseFlagValue(entry["flags"], faceAttribsConfig.surfaceFlags)));
        } else {
...
void parseBrushTags(const EL::Value &value, std::vector<Model::SmartTag> &result) {
    ...
    for (size_t i = 0; i < value.length(); ++i) {
        ...
        if (match == "classname") {
            result.emplace_back(
                entry["name"].stringValue(), parseTagAttributes(entry["attribs"]), std::make_unique<Model::EntityClassNameTagMatcher>(entry["pattern"].stringValue(), entry["texture"].stringValue()));
        } else {
            throw ParserException{entry.line(), entry.column(), "Unexpected smart tag match type '" + match + "'"};
        }
    }
