...
class AssimpIOStream : public Assimp::IOStream {
  ...
  public:
    ...
    size_t Write(const void * /* buffer */, const size_t /* size */, const size_t /* count */) override {
    ...
    aiReturn Seek(const size_t offset, const aiOrigin origin) override {
        try {
            switch (origin) {
            case aiOrigin_SET:m_reader.seekFromBegin(offset);
                return aiReturn_SUCCESS;
            case aiOrigin_CUR:m_reader.seekForward(offset);
                return aiReturn_SUCCESS;
            case aiOrigin_END:m_reader.seekFromEnd(offset);
                return aiReturn_SUCCESS;
            case _AI_ORIGIN_ENFORCE_ENUM_SIZE:break;
            }
...
class AssimpIOSystem : public Assimp::IOSystem {
  ...
  public:
    ...
    Assimp::IOStream *Open(const char *path, const char *mode) override {
        ...
        return m_fs.openFile(path).transform([](auto file) { return std::make_unique<AssimpIOStream>(std::move(file)); }).if_error([](auto e) { throw ParserException{e.msg}; }).value().release();
    }
};
...
std::optional<Assets::Texture> loadFallbackTexture(const FileSystem &fs) {
    ...
    static const auto texturePaths = std::vector<std::filesystem::path>{
        "textures" / kdl::path_add_extension(NoTextureName, ".png"), "textures" / kdl::path_add_extension(NoTextureName, ".jpg"), kdl::path_add_extension(NoTextureName, ".png"), kdl::path_add_extension(NoTextureName, ".jpg"),
    };
...
Assets::Texture loadFallbackOrDefaultTexture(const FileSystem &fs, const std::string &defaultMaterialName, Logger &logger) {
...
Assets::Texture loadTextureFromFileSystem(const std::filesystem::path &path, const FileSystem &fs, Logger &logger) {
    return fs.openFile(path).and_then([](auto file) {
        auto reader = file->reader().buffer();
        return readFreeImageTexture("", reader);
    }).or_else(makeReadTextureErrorHandler(fs, logger)).value();
}

Assets::Texture loadUncompressedEmbeddedTexture(std::string name, const aiTexel &data, const size_t width, const size_t height) {
    ...
    return {
        std::move(name), width, height, averageColor, std::move(buffer), GL_BGRA, Assets::TextureType::Masked
    };
}

Assets::Texture loadCompressedEmbeddedTexture(std::string name, const aiTexel &data, const size_t size, const FileSystem &fs, Logger &logger) {
    return readFreeImageTextureFromMemory(std::move(name), reinterpret_cast<const uint8_t *>(&data), size).or_else(makeReadTextureErrorHandler(fs, logger)).value();
}

Assets::Texture loadTexture(const aiTexture *texture, const std::filesystem::path &texturePath, const std::filesystem::path &modelPath, const FileSystem &fs, Logger &logger) {
    if (!texture) {
    ...
    if (texture->mHeight != 0) {
        ...
        return loadUncompressedEmbeddedTexture(texture->mFilename.C_Str(), *texture->pcData, texture->mWidth, texture->mHeight);
    }

    // The texture is embedded, but compressed. Let FreeImage load it from memory.
    return loadCompressedEmbeddedTexture(texture->mFilename.C_Str(), *texture->pcData, texture->mWidth, fs, logger);
}

std::vector<Assets::Texture> loadTexturesForMaterial(const aiScene &scene, const size_t materialIndex, const std::filesystem::path &modelPath, const FileSystem &fs, Logger &logger) {
    auto textures = std::vector<Assets::Texture>{};

    // Is there even a single diffuse texture? If not, fail and load fallback material.
    const auto textureCount = scene.mMaterials[materialIndex]->GetTextureCount(aiTextureType_DIFFUSE);
    if (textureCount > 0) {
        ...
        logger.error(fmt::format("No diffuse textures found for material {} of model '{}', loading fallback texture", materialIndex, modelPath.string()));

        // Materials aren't guaranteed to have a name.
        const auto materialName = scene.mMaterials[materialIndex]->GetName() != aiString{""} ? scene.mMaterials[materialIndex]->GetName().C_Str() : "nr. " + std::to_string(materialIndex + 1);
 ...
 */
std::tuple<size_t, aiMatrix4x4> getBoneParentChannelAndTransformation(const aiAnimation &animation, const aiNode &boneNode, const std::vector<aiMatrix4x4> &channelTransforms) {
    ...
    if (const auto index = getChannelIndex(animation, *parentNode)) {
        ...
        const auto [parentIndex, parentTransform] = getBoneParentChannelAndTransformation(animation, *parentNode, channelTransforms);
        unused(parentIndex);
 ...
 */
std::vector<AssimpBoneInformation> getAnimationInformation(const aiNode &root, const aiAnimation &animation) {
    ...
    for (unsigned int i = 0; i < animation.mNumChannels; ++i) {
        ...
        const auto position = channel.mNumPositionKeys > 0 ? channel.mPositionKeys[0].mValue : aiVector3D{0, 0, 0};
        const auto rotation = channel.mNumRotationKeys > 0 ? channel.mRotationKeys[0].mValue : aiQuaternion{1, 0, 0, 0};
        const auto scale = channel.mNumScalingKeys > 0 ? channel.mScalingKeys[0].mValue : aiVector3D{1, 1, 1};
    ...
    for (unsigned int i = 0; i < animation.mNumChannels; ++i) {
        ...
        if (const auto *boneNode = root.FindNode(channel.mNodeName)) {
            const auto [parentId, parentTransform] = getBoneParentChannelAndTransformation(animation, *boneNode, indivTransforms);

            transforms.push_back({
                                     i, int32_t(parentId), channel.mNodeName, indivTransforms[i], parentTransform * indivTransforms[i]
                                 });
        } else {
...
void processNode(std::vector<AssimpMeshWithTransforms> &meshes, const aiNode &node, const aiScene &scene, const aiMatrix4x4 &transform, const aiMatrix4x4 &axisTransform) {
    ...
    for (unsigned int i = 0; i < node.mNumChildren; ++i) {
        processNode(meshes, *node.mChildren[i], scene, transform * node.mChildren[i]->mTransformation, axisTransform);
    }
}

const auto AiMdlHl1NodeBodyparts = "<MDL_bodyparts>";

void processRootNode(std::vector<AssimpMeshWithTransforms> &meshes, const aiNode &node, const aiScene &scene, const aiMatrix4x4 &transform, const aiMatrix4x4 &axisTransform) {
...
std::optional<size_t> getBoneIndexByName(const std::vector<AssimpBoneInformation> &boneTransforms, const aiBone &bone) {
...
std::vector<Assets::EntityModelVertex> computeMeshVertices(const aiMesh &mesh, const aiMatrix4x4 &transform, const aiMatrix4x4 &axisTransform, const std::vector<AssimpBoneInformation> &boneTransforms) {
    ...
    auto weightsPerVertex = std::vector<std::vector<AssimpVertexBoneWeight>>{mesh.mNumVertices};
    for (unsigned int i = 0; i < mesh.mNumBones; ++i) {
        ...
        if (const auto index = getBoneIndexByName(boneTransforms, bone)) {
            for (unsigned int j = 0; j < bone.mNumWeights; ++j) {
                weightsPerVertex[bone.mWeights[j].mVertexId].push_back({*index, bone.mWeights[j].mWeight, bone});
            }
        }
    }
    ...
    for (unsigned int i = 0; i < numVerts; ++i) {
        ...
        vertices.emplace_back(vm::vec3f{meshVertices.x, meshVertices.y, meshVertices.z}, texcoords);
    }
...
aiMatrix4x4 getAxisTransform(const aiScene &scene) {
    if (scene.mMetaData) {
        ...
        const auto metadataPresent = scene.mMetaData->Get("UpAxis", upAxis) && scene.mMetaData->Get("UpAxisSign", upAxisSign) && scene.mMetaData->Get("FrontAxis", frontAxis) && scene.mMetaData->Get("FrontAxisSign", frontAxisSign) && scene.mMetaData->Get("CoordAxis", coordAxis) && scene.mMetaData->Get("CoordAxisSign", coordAxisSign) && scene.mMetaData->Get("UnitScaleFactor", unitScale);
...
Result<void> loadSceneFrame(const aiScene &scene, const size_t frameIndex, Assets::EntityModel &model, const std::string &name) {
    // load the animation information for the current "frame" (animation)
    const auto boneTransforms = frameIndex < scene.mNumAnimations ? getAnimationInformation(*scene.mRootNode, *scene.mAnimations[frameIndex]) : std::vector<AssimpBoneInformation>{};
    ...
    // transform matrix.
    processRootNode(meshes, *scene.mRootNode, scene, scene.mRootNode->mTransformation, getAxisTransform(scene));
    ...
    for (const auto &mesh : meshes) {
        if (const auto meshIndex = getMeshIndex(scene, *mesh.m_mesh)) {
            const auto vertices = computeMeshVertices(*mesh.m_mesh, mesh.m_transform, mesh.m_axisTransform, boneTransforms);
            ...
            auto builder = Renderer::IndexRangeMapBuilder<Assets::EntityModelVertex::Type>{numIndices, size};
            ...
            for (unsigned int i = 0; i < numTriangles; ++i) {
                ...
                if (face.mNumIndices == 3) {
                    builder.addTriangle(vertices[face.mIndices[0]], vertices[face.mIndices[1]], vertices[face.mIndices[2]]);
                }
            }
...
bool AssimpParser::canParse(const std::filesystem::path &path) {
    ...
    return kdl::vec_contains(supportedExtensions, kdl::str_to_lower(path.extension().string()));
}

std::unique_ptr<Assets::EntityModel> AssimpParser::doInitializeModel(TrenchBroom::Logger &logger) {
    constexpr auto assimpFlags = aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_FlipWindingOrder | aiProcess_SortByPType | aiProcess_FlipUVs;
    ...
    if (!scene) {
        throw ParserException{
            fmt::format("Assimp couldn't import model from '{}': {}", m_path.string(), importer.GetErrorString())
        };
    }

    // Create model.
    auto model = std::make_unique<Assets::EntityModel>(modelPath, Assets::PitchType::Normal, Assets::Orientation::Oriented);
    ...
    for (size_t i = 0; i < numMeshes; ++i) {
        ...
        surface.setSkins({loadTexturesForMaterial(*scene, mesh->mMaterialIndex, m_path, m_fs, logger)});
    }

    return model;
}

void AssimpParser::doLoadFrame(size_t frameIndex, Assets::EntityModel &model, Logger & /* logger */) {
    constexpr auto assimpFlags = aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_FlipWindingOrder | aiProcess_SortByPType | aiProcess_FlipUVs;
