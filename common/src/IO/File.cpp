...
OwningBufferFile::OwningBufferFile(std::unique_ptr<char[]> buffer, const size_t size) : m_buffer{
    std::move(buffer)
}, m_size{size} {
}
...
namespace {
Result<CFile::FilePtr> openPathAsFILE(const std::filesystem::path &path, const std::string &mode) {
...
FileView::FileView(std::shared_ptr<File> file, const size_t offset, const size_t length) : m_file{
    std::move(file)
