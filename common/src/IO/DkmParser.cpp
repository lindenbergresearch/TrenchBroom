...
namespace IO {
const vm::vec3fDkmParser::Normals[162] = {
...
std::filesystem::path DkmParser::findSkin(const std::string &skin) const {
  ...
  return m_fs.find(folder, TraversalMode::Flat, makeFilenamePathMatcher(basename.string() + ".*"))
             .transform([&](auto items) { return items.size() == 1 ? items.front() : skinPath; }).if_error(
      [](auto e) {
