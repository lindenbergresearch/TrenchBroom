...
std::optional<std::filesystem::path> findImage(const std::filesystem::path &texturePath, const FileSystem &fs) {
    ...
    if (!texturePath.empty()) {
        ...
        return fs.find(texturePath.parent_path(), TraversalMode::Flat, kdl::lift_and(makeFilenamePathMatcher(basename + ".*"), makeExtensionPathMatcher(imageExtensions))).transform([](auto candidates) -> std::optional<std::filesystem::path> {
            if (!candidates.empty()) {
...
Result<Assets::Texture, ReadTextureError> readQuake3ShaderTexture(std::string shaderName, const File &file, const FileSystem &fs) {
    ...
    return loadTextureImage(std::move(shaderName), *imagePath, fs).transform([&](auto texture) {
        ...
        case Assets::Quake3Shader::Culling::Front:texture.setCulling(Assets::TextureCulling::Back);
            break;
        case Assets::Quake3Shader::Culling::Back:texture.setCulling(Assets::TextureCulling::Front);
            break;
        case Assets::Quake3Shader::Culling::None:texture.setCulling(Assets::TextureCulling::None);
            break;
        }
