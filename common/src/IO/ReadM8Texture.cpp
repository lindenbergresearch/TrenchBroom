...
Result<Assets::Texture, ReadTextureError> readM8Texture(std::string name, Reader &reader) {
    try {
        ...
        return Assets::loadPalette(paletteReader, Assets::PaletteColorFormat::Rgb).and_then([&](const auto &palette) {
            ...
            return Result<Assets::Texture>{
                Assets::Texture{
                    std::move(name), widths[0], heights[0], mip0AverageColor, std::move(buffers), GL_RGBA, Assets::TextureType::Opaque
                }};
