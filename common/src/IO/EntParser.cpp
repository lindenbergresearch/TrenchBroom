...
std::unique_ptr<Assets::PropertyDefinition> parsePropertyDefinition(
    const tinyxml2::XMLElement &element, const PropertyDefinitionFactory &factory, ParserStatus &status) {
...
std::unique_ptr<Assets::PropertyDefinition> parseDeclaredPropertyDefinition(
    const tinyxml2::XMLElement &element, const Assets::PropertyDefinition &propertyDeclaration, ParserStatus &status) {
...
std::unique_ptr<Assets::PropertyDefinition> parseTargetNamePropertyDefinition(const tinyxml2::XMLElement &element, ParserStatus &status) {
    auto factory = [](std::string name, std::string shortDesc, std::string longDesc) {
        return std::make_unique<Assets::PropertyDefinition>(
            std::move(name), Assets::PropertyDefinitionType::TargetSourceProperty, std::move(shortDesc), std::move(longDesc), false);
    };
    return parsePropertyDefinition(element, factory, status);
}
...
std::unique_ptr<Assets::PropertyDefinition> parseTargetPropertyDefinition(const tinyxml2::XMLElement &element, ParserStatus &status) {
    auto factory = [](std::string name, std::string shortDesc, std::string longDesc) {
        return std::make_unique<Assets::PropertyDefinition>(
            std::move(name), Assets::PropertyDefinitionType::TargetDestinationProperty, std::move(shortDesc), std::move(longDesc), false);
    };
    return parsePropertyDefinition(element, factory, status);
}
        ...
        if (hasAttribute(element, "value")) {
            ...
            if (floatDefaultValue) {
                return std::make_unique<Assets::FloatPropertyDefinition>(
                    std::move(name), std::move(shortDesc), std::move(longDesc), false, std::move(floatDefaultValue));
            }

            auto strDefaultValue = parseString(element, "value");
            warn(
                element, fmt::format("Invalid default value '{}' for float property definition", strDefaultValue), status);
            return std::make_unique<Assets::UnknownPropertyDefinition>(
                std::move(name), std::move(shortDesc), std::move(longDesc), false, std::move(strDefaultValue));
        }
        return std::make_unique<Assets::FloatPropertyDefinition>(
        ...
        if (hasAttribute(element, "value")) {
            ...
            if (intDefaultValue) {
                return std::make_unique<Assets::IntegerPropertyDefinition>(
                    std::move(name), std::move(shortDesc), std::move(longDesc), false, std::move(intDefaultValue));
            }

            auto strDefaultValue = parseString(element, "value");
            warn(
                element, fmt::format("Invalid default value '{}' for integer property definition", strDefaultValue), status);
            return std::make_unique<Assets::UnknownPropertyDefinition>(
                std::move(name), std::move(shortDesc), std::move(longDesc), false, std::move(strDefaultValue));
        }
        ...
        if (hasAttribute(element, "value")) {
            if (const auto boolDefaultValue = parseBoolean(element, "value")) {
                return std::make_unique<Assets::BooleanPropertyDefinition>(
                    std::move(name), std::move(shortDesc), std::move(longDesc), false, *boolDefaultValue);
            }

            auto strDefaultValue = parseString(element, "value");
            warn(
                element, fmt::format("Invalid default value '{}' for boolean property definition", strDefaultValue), status);
            return std::make_unique<Assets::UnknownPropertyDefinition>(
                std::move(name), std::move(shortDesc), std::move(longDesc), false, std::move(strDefaultValue));
        }
...
std::unique_ptr<Assets::PropertyDefinition> parseStringPropertyDefinition(const tinyxml2::XMLElement &element, ParserStatus &status) {
    auto factory = [&](std::string name, std::string shortDesc, std::string longDesc) {
        ...
        return std::make_unique<Assets::StringPropertyDefinition>(
            std::move(name), std::move(shortDesc), std::move(longDesc), false, std::move(defaultValue));
    };
    return parsePropertyDefinition(element, factory, status);
}
...
std::unique_ptr<Assets::PropertyDefinition> parsePropertyDefinition(
    const tinyxml2::XMLElement &element, const std::vector<std::shared_ptr<Assets::PropertyDefinition>> &propertyDeclarations, ParserStatus &status) {
...
std::vector<std::shared_ptr<Assets::PropertyDefinition>> parsePropertyDefinitions(
    const tinyxml2::XMLElement &parent, const std::vector<std::shared_ptr<Assets::PropertyDefinition>> &propertyDeclarations, ParserStatus &status) {
    ...
    if (const auto *flagElement = element.FirstChildElement("flag")) {
        ...
        do {
            ...
            if (!bit) {
                ...
                warn(
                    *flagElement, fmt::format("Invalid value '{}' for bit property definition", strValue), status);
            } else {
...
void parsePropertyDefinitions(
    const tinyxml2::XMLElement &element, const std::vector<std::shared_ptr<Assets::PropertyDefinition>> &propertyDeclarations,
    EntityDefinitionClassInfo &classInfo, ParserStatus &status) {
...
Assets::ModelDefinition parseModel(const tinyxml2::XMLElement &element) {
    ...
    try {
        ...
        auto expression = EL::Expression{EL::LiteralExpression{EL::Value{EL::MapType{{
