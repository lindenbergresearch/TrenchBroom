...
bool isDirectory(const ImageEntry &entry) {
  return std::visit(
      kdl::overload(
          [](const ImageDirectoryEntry &) { return true; }, [](const ImageFileEntry &) { return false; }
      ), entry
  );
}
...
template<typename F> auto withEntry(
    const std::filesystem::path &searchPath, const ImageEntry &currentEntry, const std::filesystem::path &currentPath, const F &f, decltype(f(
    std::declval<const ImageEntry &>(), std::declval<const std::filesystem::path &>())) defaultResult
) {
  if (searchPath.empty()) {
      ...
      kdl::overload(
          [&](const ImageDirectoryEntry &directoryEntry) {
            ...
            return entryIt != directoryEntry.entries.end() ? withEntry(
                kdl::path_pop_front(searchPath), *entryIt, currentPath / name, f, defaultResult
            ) : defaultResult;
...
template<typename F> void withEntry(
    const std::filesystem::path &searchPath, const ImageEntry &currentEntry, const std::filesystem::path &currentPath, const F &f
) {
  if (searchPath.empty()) {
...
const ImageEntry *findEntry(const std::filesystem::path &path, const ImageEntry &parent) {
  return withEntry(
      path, parent, std::filesystem::path{}, [](const ImageEntry &entry, const std::filesystem::path &) { return &entry; }, nullptr
  );
}
...
ImageDirectoryEntry &findOrCreateDirectory(const std::filesystem::path &path, ImageDirectoryEntry &parent) {
  ...
  if (entryIt != parent.entries.end()) {
    ...
    return findOrCreateDirectory(
        kdl::path_pop_front(path), std::get<ImageDirectoryEntry>(
...
namespace {
void doFindImpl(
    const ImageEntry &entry, const std::filesystem::path &entryPath, const TraversalMode traversalMode, std::vector<std::filesystem::path> &result
) {
  std::visit(
      kdl::overload(
          [&](const ImageDirectoryEntry &directoryEntry) {
...
Result<std::vector<std::filesystem::path>> ImageFileSystemBase::doFind(const std::filesystem::path &path, const TraversalMode traversalMode) const {
  ...
  withEntry(
      path, m_root, {}, [&](const ImageEntry &entry, const std::filesystem::path &entryPath) {
...
Result<std::shared_ptr<File>> ImageFileSystemBase::doOpenFile(const std::filesystem::path &path) const {
  return withEntry(
      path, m_root, std::filesystem::path{}, [&](const ImageEntry &entry, const std::filesystem::path &) {
        return std::visit(
            kdl::overload(
