...
template<typename F>
auto withEntry(
    const std::filesystem::path &searchPath, const ImageEntry &currentEntry, const std::filesystem::path &currentPath, const F &f, decltype(f(std::declval<const ImageEntry &>(), std::declval<const std::filesystem::path &>())) defaultResult) {
    if (searchPath.empty()) {
