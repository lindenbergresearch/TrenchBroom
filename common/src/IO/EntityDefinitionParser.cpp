...
static void inheritAttributes(EntityDefinitionClassInfo &inheritingClass, const EntityDefinitionClassInfo &superClass) {
    ...
    for (const auto &attribute : superClass.propertyDefinitions) {
        auto it = std::find_if(std::begin(inheritingClass.propertyDefinitions), std::end(inheritingClass.propertyDefinitions), [&](const auto &a) { return a->key() == attribute->key(); });
 ...
 *
 */
template<typename F> static void inheritFromAndRecurse(ParserStatus &status, EntityDefinitionClassInfo &inheritingClass, const EntityDefinitionClassInfo &superClass, const F &findClassInfos, std::unordered_set<std::string> &visited) {
 ...
 * from the inheriting class to the given super class
 */
template<typename F> static void findSuperClassesAndInheritFrom(ParserStatus &status, EntityDefinitionClassInfo &inheritingClass, const EntityDefinitionClassInfo &classWithSuperClasses, const F &findClassInfos, std::unordered_set<std::string> &visited) {
 ...
 * super classes added
 */
template<typename F> static EntityDefinitionClassInfo resolveInheritance(ParserStatus &status, EntityDefinitionClassInfo inheritingClass, const F &findClassInfos) {
...
std::unique_ptr<Assets::EntityDefinition> createDefinition(EntityDefinitionClassInfo classInfo, const Color &defaultEntityColor) {
    ...
    case EntityDefinitionClassType::PointClass: return std::make_unique<Assets::PointEntityDefinition>(std::move(name), color, size, std::move(description), std::move(propertyDefinitions), std::move(modelDefinition), std::move(decalDefinition));
    case EntityDefinitionClassType::BrushClass: return std::make_unique<Assets::BrushEntityDefinition>(std::move(name), color, std::move(description), std::move(propertyDefinitions));
    case EntityDefinitionClassType::BaseClass:return nullptr;
        switchDefault();
    };
}

std::vector<std::unique_ptr<Assets::EntityDefinition>> createDefinitions(ParserStatus &status, const std::vector<EntityDefinitionClassInfo> &classInfos, const Color &defaultEntityColor) {
