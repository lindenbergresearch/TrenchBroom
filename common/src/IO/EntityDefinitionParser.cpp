...
static std::shared_ptr<Assets::PropertyDefinition> mergeAttributes(
  ...
  if (superClassAttribute.type() == Assets::PropertyDefinitionType::FlagsProperty
      && inheritingClassAttribute.type() == Assets::PropertyDefinitionType::FlagsProperty && superClassAttribute.key() == Model::EntityPropertyKeys::Spawnflags
 ...
 *
 */
template<typename F> static void inheritFromAndRecurse(
 ...
 * from the inheriting class to the given super class
 */
template<typename F> static void findSuperClassesAndInheritFrom(
 ...
 * super classes added
 */
template<typename F> static EntityDefinitionClassInfo resolveInheritance(
