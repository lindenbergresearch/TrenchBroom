...
Result<Assets::Palette> loadPalette(const FileSystem &gameFS, const Model::TextureConfig &textureConfig) {
  ...
  return gameFS.openFile(textureConfig.palette).and_then(
      [&](auto file) {
...
Result<Assets::Texture, ReadTextureError> readTexture(
    const File &file, const std::filesystem::path &path, const FileSystem &gameFS, const size_t prefixLength, const std::optional<Assets::Palette> &palette
) {
...
Result<ReadTextureFunc> makeReadTextureFunc(const FileSystem &gameFS, const Model::TextureConfig &textureConfig) {
  return loadPalette(gameFS, textureConfig).transform([](auto palette) { return std::optional{std::move(palette)}; })
                                           .transform_error([](auto) -> std::optional<Assets::Palette> { return std::nullopt; }).and_then(
      [&](auto palette) -> Result<ReadTextureFunc> {
...
Result<std::vector<std::filesystem::path>> findTextureCollections(const FileSystem &gameFS, const Model::TextureConfig &textureConfig) {
  return gameFS.find(
      textureConfig.root, TraversalMode::Recursive, makePathInfoPathMatcher({PathInfo::Directory})).transform(
      [&](auto paths) {
...
Result<Assets::TextureCollection> loadTextureCollection(
    const std::filesystem::path &path, const FileSystem &gameFS, const Model::TextureConfig &textureConfig, Logger &
) {
  ...
  return gameFS.find(path, TraversalMode::Flat, pathMatcher).transform(
      [&](auto texturePaths) {
  ...
  ).join(makeReadTextureFunc(gameFS, textureConfig)).and_then(
      [&](auto texturePaths, const auto &readTexture) {
        auto nullLogger = NullLogger{};
        return kdl::fold_results(
            kdl::vec_parallel_transform(
                std::move(texturePaths), [&](const auto texturePath) {
                  return gameFS.openFile(texturePath).and_then(
                      [&](const auto &file) {
                        return readTexture(*file, texturePath).transform(
                            [&](auto texture) {
                              gameFS.makeAbsolute(texturePath).transform(
                                  [&](auto absPath) {
